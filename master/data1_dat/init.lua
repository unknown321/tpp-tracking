...
AssetConfiguration.RegisterExtensionInfo({ extensions = { "sbp", "stm", "mas", "wem", "fsm" }, categories = {
	"Target",
	"Language",
} })
if GrDaemon then
...
local a
if Editor then
	t = Editor({ name = "MainEditor" })
	a = t
elseif EditorBase then
	t = EditorBase({ name = "MainEditor" })
	a = t
else
...
if CameraSelector then
	local e = CameraSelector({
		name = "MainCameraSelector",
		scene = "MainScene",
		viewport = "MainViewport",
		priorities = { "Debug", "Editor", "Demo", "Game", "GameWeakest" },
...
if a then
	...
	local t = a:CreateNewEditableBucket("NewBucket")
	a:SetCurrentEditableBucket(t)
	if e == "Windows" then
