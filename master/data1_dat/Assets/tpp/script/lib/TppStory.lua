...
e.storySequenceTable_Master = {
	...
	},
	{
		...
		condition = function()
			return (TppQuest.IsCleard("tent_q20910") and TppQuest.IsCleard("fort_q20911"))
				and TppQuest.IsCleard("sovietBase_q20912")
		end,
...
e.radioDemoTable = {
	...
	AttackedFromOtherPlayerRadio = {
		storyCondition = function(e)
			return e.demoName == "AttackedFromOtherPlayer_UnknowWhereFrom"
				or e.demoName == "AttackedFromOtherPlayer_KnowWhereFrom"
		end,
		detailCondition = function()
			return true
		end,
	...
	QuietTreatment = {
		...
		detailCondition = function(e)
			local n = not TppRadio.IsPlayed("f2000_rtrg1345")
			local i = TppBuddyService.DidObtainBuddyType(BuddyType.QUIET)
			local t = TppQuest.IsOpen("sovietBase_q99020")
			local e = not TppQuest.IsCleard("sovietBase_q99020")
	...
	QuietTreatment2 = {
		...
		detailCondition = function(e)
			...
			local i = not TppQuest.IsOpen("sovietBase_q99020")
			local e = TppQuest.IsCleard("sovietBase_q99020")
			return (t and n) and (i or e)
		end,
	...
	WalkerGear = {
		storyCondition = function()
			return e.GetCurrentStorySequence()
	...
	ZeroAndScalFace = {
		...
		detailCondition = function()
			local e = TppRadio.IsPlayed("f2000_rtrg8090")
	...
	OpenLinguaFranka = {
		storyCondition = function()
			return e.IsMissionCleard(10080)
				and e.GetCurrentStorySequence() == TppDefine.STORY_SEQUENCE.CLEARD_DESTROY_THE_FLOW_STATION
		end,
		detailCondition = function()
			return true
		end,
	...
	OpenRemovalWalkerGear = {
		storyCondition = function()
			return e.IsMissionCleard(10086)
				and e.GetCurrentStorySequence() == TppDefine.STORY_SEQUENCE.CLEARD_LINGUA_FRANKA
		end,
		detailCondition = function()
			return true
		end,
	...
	OpenCarLine = {
		storyCondition = function()
			return e.IsMissionCleard(10082)
				and e.GetCurrentStorySequence()
					== TppDefine.STORY_SEQUENCE.CLEARD_FLAG_MISSIONS_AFTER_DESTROY_THE_FLOW_STATION
		end,
		detailCondition = function()
			return true
		end,
	...
	GeneOfEli = {
		storyCondition = function()
			return (e.IsMissionCleard(10085) or e.IsMissionCleard(10200))
				and e.GetCurrentStorySequence() == TppDefine.STORY_SEQUENCE.CLEARD_WHITE_MAMBA
		end,
		detailCondition = function()
			return true
		end,
	...
	ParasiticWormCarrierQuarantine = {
		storyCondition = function()
			...
			local e = not TppMotherBaseManagement.IsPandemicEventMode()
	...
	ProgressQuestChildSoldier = {
		storyCondition = function()
			...
			local e = not (
				(TppQuest.IsOpen("tent_q20910") and TppQuest.IsOpen("fort_q20911"))
				and TppQuest.IsOpen("sovietBase_q20912")
			)
			return n and e
		end,
		detailCondition = function()
	...
	LeakRadiationInMB = {
		storyCondition = function()
			...
			local n = (TppQuest.IsCleard("tent_q20910") and TppQuest.IsCleard("fort_q20911"))
				and TppQuest.IsCleard("sovietBase_q20912")
			return e or n
		end,
		detailCondition = function()
	...
	AboutSideOps = {
		...
		detailCondition = function()
			return ((TppMission.GetMissionID() ~= 30050) and (TppMission.GetMissionID() ~= 30150))
				and (TppMission.GetMissionID() ~= 30250)
		end,
	...
	SuggestActiveSonar = {
		storyCondition = function()
			return (TppMotherBaseManagement.IsEquipDevelopableWithDevelopID({ equipDevelopID = 18030 }))
				and not (TppMotherBaseManagement.IsEquipDeveloped({ equipID = TppEquip.EQP_HAND_ACTIVESONAR }))
		end,
		detailCondition = function()
			return ((TppMission.GetMissionID() ~= 30050) and (TppMission.GetMissionID() ~= 30150))
				and (TppMission.GetMissionID() ~= 30250)
	...
	AboutCallBuddy = {
		storyCondition = function()
			return TppBuddyService.CanSortieBuddyType(BuddyType.DOG)
				or TppBuddyService.CanSortieBuddyType(BuddyType.QUIET)
		end,
		detailCondition = function()
			local t = 0
			local n = 1
			...
			if e == t or e == n then
				if
					((TppMission.GetMissionID() ~= 30050) and (TppMission.GetMissionID() ~= 30150))
					and (TppMission.GetMissionID() ~= 30250)
				then
					return true
				end
	...
	UnlockIntelSearch = {
		storyCondition = function()
			return TppMotherBaseManagement.IsActiveSectionFunc({
...
function e.CheckAllMissionCleared()
	...
	for e, t in pairs(TppDefine.MISSION_ENUM) do
		local i = TppDefine.MISSING_NUMBER_MISSION_ENUM[e]
		if not i then
			local i = tonumber(e)
			if not gvars.str_missionClearedFlag[t] then
			...
			local t = true
			local o = { [10240] = true, [10115] = true, [10030] = true }
			if o[i] then
				t = false
			end
...
function e.FailedRetakeThePlatform()
	...
	e.StartElapsedMissionEvent(
		TppDefine.ELAPSED_MISSION_EVENT.FAILED_RETAKE_THE_PLATFORM,
		TppDefine.INIT_ELAPSED_MISSION_COUNT.FAILED_RETAKE_THE_PLATFORM
	)
end
...
function e.CanOpenS10240()
	local t = e.GetClearedMissionCount({
		10010,
		10020,
		10030,
		10036,
	...
	local e = TppQuest.IsNowOccurringElapsed()
	local i = TppMotherBaseManagement.CanOpenS10240()
	if ((t >= 36) and i) and (n or e) then
		return true
	else
		return false
	end
...
function e.CanOpenS10260()
	local r = TppDemo.IsPlayedMBEventDemo("DecisionHuey")
	local e = TppBuddyService.CanSortieBuddyType(BuddyType.QUIET)
	local n = TppBuddyService.IsDeadBuddyType(BuddyType.QUIET)
	local t = TppBuddyService.GetFriendlyPoint(BuddyFriendlyType.QUIET)
	local i = not TppUiCommand.IsUsingButterflyEmblem()
	if (((r and e) and not n) and (t >= 100)) and i then
		return true
	else
		return false
	end
...
function e.CanOpenS10280()
	...
	local n = TppDemo.IsPlayedMBEventDemo("DecisionHuey")
	local e = "tp_m_10160_11"
	local t = TppMotherBaseManagement.IsGotCassetteTapeTrack(e)
	local e = TppMotherBaseManagement.IsNewCassetteTapeTrack(e)
	return (n and t) and not e
end
...
function e.UpdateStorySequence(t)
	...
	if
		TppMission.IsFOBMission(vars.missionCode) or TppMission.IsFOBMission(
			TppMission.GetNextMissionCodeForEmergency()
		)
	then
		return
	end
	if
		(gvars.str_storySequence == TppDefine.STORY_SEQUENCE.CLEARD_ENDRESS_PROXY_WAR)
		and (not TppSave.CanSaveMbMangementData())
	then
		return
	end
	local n
...
function e._UpdateS11050OpenFlag()
	...
	if
		(
			n >= TppDefine.STORY_SEQUENCE.CLEARD_RETRIEVE_CHILD_DESERTER
			and TppDemo.IsPlayedMBEventDemo("ArrivedMotherBaseAfterQuietBattle")
		) and (not e.IsMissionOpen(11050))
	then
		e.MissionOpen(11050)
	end
end
...
function e.IsOccuringBossQuiet()
	local n = e.GetClearedMissionCount({ 10041, 10044, 10052, 10054 })
	local e = TppMotherBaseManagement.IsBuiltMbMedicalClusterSpecialPlatform()
	local t = TppBuddy2BlockController.DidObtainBuddyType(BuddyType.QUIET)
	local i = TppBuddyService.CheckBuddyCommonFlag(BuddyCommonFlag.BOSS_QUIET_KILL)
	if ((e and (n >= 1)) and not t) and not i then
		return true
	else
		return false
	end
...
function e.CanArrivalLiquidInMB()
	local n = e.GetCurrentStorySequence() >= TppDefine.STORY_SEQUENCE.CLEARD_WHITE_MAMBA
	local e = not TppDemo.IsPlayedMBEventDemo("TheGreatEscapeLiquid")
	return n and e
end
...
function e.CanArrivalDDogInMB()
	local n = TppBuddyService.CanSortieBuddyType(BuddyType.DOG)
	local e = not TppBuddyService.IsDeadBuddyType(BuddyType.DOG)
	return n and e
end
function e.CanArrivalSahelanInMB()
	local n = e.GetCurrentStorySequence() >= TppDefine.STORY_SEQUENCE.CLEARD_OKB_ZERO
	local e = not TppDemo.IsPlayedMBEventDemo("TheGreatEscapeLiquid")
	return n and e
end
function e.CanArrivalChildrenInMB()
	if e.IsMissionCleard(10100) then
		...
		local e = (TppQuest.IsCleard("tent_q20910") and TppQuest.IsCleard("fort_q20911"))
			and TppQuest.IsCleard("sovietBase_q20912")
		if n and not e then
			return false
		end
...
function e.GetBattleGearDevelopLevel()
	local n = e.GetCurrentStorySequence()
	if
		gvars.forceMbRadioPlayedFlag[TppDefine.FORCE_MB_RETURN_RADIO_ENUM.CompliteDevelopBattleGearRadio]
		or TppDemo.IsPlayedMBEventDemo("DevelopedBattleGear5")
	then
		return 5
...
function e.UpdateDemoFlagQuietWishGoMission()
	if
		not TppDemo.IsPlayedMBEventDemo("QuietWishGoMission")
		and gvars.mis_quietCallCountOnMissionStart ~= vars.buddyCallCount[BuddyType.QUIET]
	then
		e.StartElapsedMissionEvent(
			TppDefine.ELAPSED_MISSION_EVENT.QUIET_WITH_GO_MISSION,
			TppDefine.INIT_ELAPSED_MISSION_COUNT.QUIET_WITH_GO_MISSION
		)
	end
end
	...
	if n == 10050 then
		if TppBuddyService.CheckBuddyCommonFlag(BuddyCommonFlag.BUDDY_QUIET_LOST) then
			...
			if n == TppDefine.ELAPSED_MISSION_COUNT.INIT then
				...
				e.StartElapsedMissionEvent(
					TppDefine.ELAPSED_MISSION_EVENT.QUIET_VISIT_MISSION,
					TppDefine.INIT_ELAPSED_MISSION_COUNT.QUIET_VISIT_MISSION
				)
			end
...
function e.DEBUG_SetStorySequence(n, o)
	...
	if TppDebugMbDevelop then
		...
		for i = TppDefine.STORY_SEQUENCE.STORY_START, gvars.str_storySequence do
			local e = e.GetStorySequenceName(i)
			if TppDebugMbDevelop[e] then
				n = TppDebugMbDevelop[e]
				t = e
			end
		end
		if n then
