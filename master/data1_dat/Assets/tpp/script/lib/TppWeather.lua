...
local o = { [TppDefine.WEATHER.SANDSTORM] = true, [TppDefine.WEATHER.FOGGY] = true }
local n = "Script"
local T = "WeatherSystem"
local a = 20
local p = 255
function e.RequestWeather(i, t, r)
	local e, t = e._GetRequestWeatherArgs(t, r)
...
function e.CancelRequestWeather(t, r, i)
	local e, r = e._GetRequestWeatherArgs(r, i)
...
function e.SetDefaultWeatherDurations()
	WeatherManager.SetWeatherDurations(s)
...
function e.RestoreMissionStartWeather()
	...
	WeatherManager.RequestWeather({ priority = WeatherManager.REQUEST_PRIORITY_NORMAL, userId = T, weatherType = r, interpTime = a })
	if t ~= nil then
		WeatherManager.RequestWeather({
			priority = WeatherManager.REQUEST_PRIORITY_EXTRA,
			userId = T,
			weatherType = t,
			interpTime = a,
		})
...
function e.OnMissionCanStart()
	if TppMission.IsHelicopterSpace(vars.missionCode) then
		TppEffectWeatherParameterMediator.SetParameters({
			addTppSkyOffsetY = 1320,
			setTppSkyScale = 0.1,
			setTppSkyScrollSpeedRate = -20,
		})
	else
