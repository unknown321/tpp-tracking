...
function e.SetBird(e)
	for n, e in ipairs(e) do
		...
		if (e.center and e.radius) and e.height then
			local a =
				{ id = "ChangeFlyingZone", name = e.name, center = e.center, radius = e.radius, height = e.height }
			GameObject.SendCommand(n, a)
			local a = nil
			if e.ground then
	...
	if (t.animalList and Tpp.IsTypeTable(t.animalList)) and next(t.animalList) then
		for t, n in pairs(t.animalList) do
			if n.animalName then
				if n.colorId then
					e.SetFova(n.animalType, n.animalName, n.colorId)
					a = true
				end
				if n.animalId then
					e.SetAnimalId(n.animalType, n.animalName, n.animalId)
					a = true
				end
				if n.kindId then
					e.SetKind(n.animalType, n.animalName, n.kindId)
					a = true
				end
				if n.routeName then
				...
				if n.isNotice then
					e.SetNotice(n.animalType, n.animalName, false)
					a = true
				end
				if n.isIgnoreNotice then
					e.SetIgnoreNotice(n.animalType, n.animalName, true)
					a = true
				end
				if n.isSleep then
					e.SetSleep(n.animalType, n.animalName, n.isSleep)
					a = true
				end
			end
			if n.birdList then
				e.SetBird(n.birdList)
				a = true
			end
		end
	end
