...
function e.DeclareSVars()
	return {
		{
			name = "chk_checkPointName",
			arraySize = TppDefine.CHECK_POINT_MAX,
...
function e.Update(n)
	...
	elseif d then
		...
		if Player.CanSaveAsCheckPoint and Player.CanSaveAsCheckPoint() then
			e.Reset()
			TppPlayer.SetInitialPositionToCurrentPosition()
			Player.NotifyCheckPointTrapName(l)
	...
	GkEventTimerManager.Start("Timer_UpdateCheckPoint", 0.01)
end
...
function e.DebugUpdate()
	...
	if mvars.debug.showCheckPointList then
		(nil).Print(i, { 0.5, 0.5, 1 }, "TppCheckPoint: showCheckPointList")
