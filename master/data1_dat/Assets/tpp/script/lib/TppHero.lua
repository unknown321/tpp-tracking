...
e.FIRE_KILL_SOLDIER_FOB_SNEAK = { heroicPoint = -40, ogrePoint = 120 }
e.KILL_SOLDIER = { heroicPoint = 0, ogrePoint = 60 }
e.KILL_SOLDIER_FOB_SNEAK = { heroicPoint = -20, ogrePoint = 60 }
...
e.STARTED_COMBAT_ON_FOB = { heroicPoint = -10, ogrePoint = 0 }
e.STARTED_COMBAT_ON_FOB_HERO = { heroicPoint = -100, ogrePoint = 0 }
e.DISCOVER_ATTACKER = { heroicPoint = 10, ogrePoint = 0 }
e.ATTAKCER_WIN_ON_FOB_PRACTICE = { heroicPoint = 50, ogrePoint = 0 }
e.DEFENCE_WIN_ELIMINATE_PRACTICE = { heroicPoint = 30, ogrePoint = 0 }
e.DEFENCE_WIN_ABORT_PRACTICE = { heroicPoint = 10, ogrePoint = 0 }
e.FULTONED_PLAYER = { heroicPoint = 50, ogrePoint = -50 }
e.FULTONED = { heroicPoint = -500, ogrePoint = 0 }
e.FULTONED_PLAYER_PRACTICE = { heroicPoint = 5, ogrePoint = -5 }
e.RETAKE_STAFF_FROM_FOB = { heroicPoint = 60, ogrePoint = 0 }
e.KILLED_PLAYER = { heroicPoint = 0, ogrePoint = 60 }
e.ATTAKCER_WIN_ON_FOB_FOR_FRIEND = { heroicPoint = 500, ogrePoint = 0 }
e.DEFENCE_WIN_FOR_FRIEND = { heroicPoint = 300, ogrePoint = 0 }
e.BREAK_PTW_MACHINEGUN = { heroicPoint = -10, ogrePoint = 10 }
e.BREAK_PTW_MORTAR = { heroicPoint = -10, ogrePoint = 10 }
e.BREAK_PTW_ANTIAIR = { heroicPoint = -30, ogrePoint = 30 }
e.FULTON_CONTAINER = { heroicPoint = -30, ogrePoint = 30 }
e.NOTICE_CRIME = { heroicPoint = -500, ogrePoint = 0 }
e.KILLED_DDS = { heroicPoint = -60, ogrePoint = 60 }
e.FOB_ABORT_BY_MENU = { heroicPoint = -500, ogrePoint = 0 }
function e.IsHero()
	return gvars.isHero
end
...
function e.OnFultonSoldier(i, o)
	local c = n(i, { id = "GetStateFlag" })
	local r = n(i, { id = "IsZombieOrMsf" })
	local n = n(i, { id = "IsChild" })
	if r then
		if o then
			TppMotherBaseManagement.AddTempLifesavingLog({ heroicPoint = 60, subOgrePoint = 60 })
		else
	...
	elseif n then
		e.AddTargetLifesavingHeroicPoint(n, o)
	else
		if t(c, StateFlag.DYING_LIFE) ~= 0 then
			if o then
				...
				if (not TppMission.IsFOBMission(vars.missionCode)) or TppServerManager.FobIsSneak() then
					...
					e.SetAndAnnounceHeroicOgrePoint({ heroicPoint = 60, ogrePoint = -60 })
				end
			end
		else
...
function e.AnnounceHeroicPoint(e, o, n)
	...
	if e.heroicPoint < 0 then
		TppUI.ShowAnnounceLog(o, -e.heroicPoint)
	elseif e.heroicPoint > 0 then
		TppUI.ShowAnnounceLog(n, e.heroicPoint)
	end
end
...
function e.OnHelicopterLostControl(n, i)
	local o = c(n)
	local n = Tpp.IsLocalPlayer(i)
...
function e.SetAndAnnounceHeroicOgrePointForAnnihilateCp(o, i)
	local n
	if i then
	...
	e.SetAndAnnounceHeroicOgrePoint(o, nil, n)
end
...
function e.OnBreakPlaced(o, n, t, i)
	...
	if TppPlayer.IsMine(n) then
		...
		e.SetAndAnnounceHeroicOgrePoint(e.BREAK_MINE, nil, "disposal_mine")
	end
end
function e.OnPickUpPlaced(i, o, t, n)
	...
	if TppPlayer.IsMine(o) then
		Tpp.IncrementPlayData("totalMineRemoveCount")
		e.SetAndAnnounceHeroicOgrePoint(e.PICK_UP_MINE, nil, "disposal_mine")
	end
end
...
function e.Messages()
	return Tpp.StrCode32Table({
				...
				func = function(o)
					if o then
						if not n(o, { id = "IsDoneHoldup" }) then
							e.SetAndAnnounceHeroicOgrePoint(e.ENEMY_HOLD_UP)
							n(o, { id = "SetDoneHoldup" })
						end
					end
				end,
			},
			...
			},
			{
				...
				func = function(n, o, i)
					...
					if mvars.ene_cpList then
						local e = mvars.ene_cpList[n]
						o = TppTrophy.DOMINATION_TARGET_CP_NAME_LIST[e]
					end
					if i == 0 then
					...
					if TppCommandPost2.SetCpDominated then
						...
						if e == "afgh" or e == "mafr" then
							...
							local o = TppCommandPost2.GetDominatedCpCount({ type = e })
							local n = TppTrophy.DOMINATION_TARGET_CP_COUNT[e]
							if i then
							end
							if o == n then
		...
		Placed = { { msg = "OnBreakPlaced", func = e.OnBreakPlaced } },
	})
end
function e.Init(n)
...
function e.OnMessage(c, P, r, o, n, i, t)
	Tpp.DoMessage(e.messageExecTable, TppMission.CheckMessageOption, c, P, r, o, n, i, t)
end
function e.DeclareSVars()
	return {
		{
			name = "her_missionOgrePoint",
