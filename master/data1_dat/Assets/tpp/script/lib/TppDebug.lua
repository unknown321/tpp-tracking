...
local r = {}
function r.SetPlayLogEnabled(e)
	if TppPlayLog then
		TppPlayLog.SetPlayLogEnabled(e)
	end
end
function r.RequestResetPlayLog()
	if TppPlayLog then
		TppPlayLog.RequestResetPlayLog()
	end
end
function r.RequestUploadPlayLog()
	if TppPlayLog then
		TppPlayLog.RequestUploadPlayLog()
	end
end
function r.ExportSavedPlayLog()
	if TppPlayLog and TppPlayLog.ExportSavedPlayLog then
		TppPlayLog.ExportSavedPlayLog()
	end
end
r.PERF_CHECK_TYPE = Tpp.Enum({ "OnUpdate", "OnMessage", "OnEnter" })
local l = {}
local g = {}
local d = {}
local m = 2
...
function r.DEBUG_SetOrderBoxPlayerPosition()
...
function r.DEBUG_GetSysVarsLog()
	local a = svars or {}
	local e = mvars or {}
	local e = {
		"missionName = "
			.. (
				tostring(e.mis_missionName)
				.. (
					", vars.missionCode = "
					.. (tostring(vars.missionCode) .. (", vars.locationCode = " .. tostring(vars.locationCode)))
				)
			),
		...
		"vars.mbLayoutCode = "
			.. (tostring(vars.mbLayoutCode) .. (", mvars.mis_nextLayoutCode = " .. tostring(e.mis_nextLayoutCode))),
		"vars.mbClusterId = "
			.. (tostring(vars.mbClusterId) .. (", mvars.mis_nextClusterId = " .. tostring(e.mis_nextClusterId))),
		"mvars.mis_isOutsideOfMissionArea = " .. tostring(e.mis_isOutsideOfMissionArea),
		"svars.mis_isDefiniteGameOver = "
			.. (
				tostring(a.mis_isDefiniteGameOver)
				.. (", svars.mis_isDefiniteMissionClear = " .. tostring(a.mis_isDefiniteMissionClear))
			),
...
function r.DEBUG_WarpHelicopter(i, o, s, a, n)
	...
	for s, r in pairs(soldierNameTable) do
		local r = t(r)
		e(r, { id = "SetEnabled", enabled = false })
		...
		if n then
			...
			e(r, { id = "SetAlertRoute", enabled = false, route = "", point = a })
		end
		e(r, { id = "SetEnabled", enabled = true })
	end
	local a = t(i)
	e(a, { id = "SetPosition", position = s, rotY = 0 })
end
function r.DEBUG_WarpVehicleAndSoldier(o, s, n, l, a, i)
	if not t(o) then
		o = { o }
	end
	local t = GameObject.GetGameObjectId
	...
	for o, r in pairs(o) do
		local r = t(r)
		e(r, { id = "SetEnabled", enabled = false })
		e(r, { id = "SetSneakRoute", route = n, point = a })
		e(r, { id = "SetCautionRoute", route = n, point = a })
		if i then
			e(r, { id = "SetAlertRoute", enabled = true, route = n, point = a })
		else
			e(r, { id = "SetAlertRoute", enabled = false, route = "", point = a })
		end
		e(r, { id = "SetEnabled", enabled = true })
	...
	local a = t(s)
	e(a, { id = "SetPosition", position = l, rotY = 0 })
end
r.DEBUG_SkipOnChangeSVarsLog = { timeLimitforSneaking = true, timeLimitforNonAbort = true }
function r.DEBUG_AddSkipLogSVarsName(e)
...
function r.DEBUG_FobGPU()
	local e = function(r)
		...
		do
			local e =
				{ CommonMetal = 100, MinorMetal = 100, PreciousMetal = 100, FuelResource = 100, BioticResource = 100 }
		...
		do
			...
			TppMotherBaseManagement.SetFobSvars({ fob = "Fob1", got = true, oceanAreaId = 70, topologyType = r, color = e[a] })
...
function r.QARELEASE_DEBUG_Init()
	local e
	do
		return
	end
...
function r.QAReleaseDebugUpdate()
	local e = svars
	local o = mvars
	local e = (nil).Print
	...
	if gvars.DEBUG_showSysVars then
		local r = r.DEBUG_GetSysVarsLog()
		e(a, { 0.5, 0.5, 1 }, "LuaSystem showSysVars")
		for o, r in ipairs(r) do
			e(a, r)
		end
		local r = {
		...
		e(a, "vars.fobSneakMode = " .. tostring(r[vars.fobSneakMode]))
		local r = TppScriptVars.GetVarValueInSlot(TppDefine.SAVE_SLOT.MB_MANAGEMENT, "vars", "mbmTppGmp", 0)
		e(a, "GMP(inSlot) = " .. tostring(r))
		local r = TppScriptVars.GetVarValueInSlot(TppDefine.SAVE_SLOT.MB_MANAGEMENT, "vars", "mbmTppHeroicPoint", 0)
		e(a, "HeroicPoint(inSlot) = " .. tostring(r))
	end
	if gvars.DEBUG_showGameStatus then
		...
		local r = TppGameStatus.IsSet("TppMain.lua", "S_IS_BLACK_LOADING")
		if r then
			e(a, " statusType = " .. (tostring("S_IS_BLACK_LOADING") .. (", IsSet = " .. tostring(r))))
		end
		e(a, "UIStatus")
		local r = {
			{ CallMenu = "INVALID" },
		...
		for o, r in pairs(r) do
			for o, r in pairs(r) do
				if TppUiStatusManager.CheckStatus(o, r) == true then
					e(a, string.format(" UI = %s, Status = %s", o, r))
				end
			end
		end
	end
	if o.qaDebug.showWeaponVars then
		local r = { "PRIMARY_HIP", "PRIMARY_BACK", "SECONDARY" }
		...
		for r = 0, 7 do
			e(
				...
				string.format(
					...
					r,
					vars.supportWeapons[r],
					vars.initSupportWeapons[r],
					gvars.ply_lastWeaponsUsingTemp[r + TppDefine.WEAPONSLOT.SUPPORT_0]
				)
			)
		end
		for r = 0, 7 do
			e(
				...
				string.format(
					...
					vars.items[r],
					vars.initItems[r],
					gvars.ply_lastItemsUsingTemp[r]
				)
			)
		end
	end
	if o.qaDebug.showPlayerPartsType then
		...
		e(
			...
			string.format(
				...
				gvars.ply_lastPlayerPartsTypeUsingTemp
			)
		)
		e(
			a,
			string.format(
				...
				vars.playerCamoType,
				gvars.ply_lastPlayerCamoTypeUsingTemp
			)
		)
		e(
			a,
			string.format(
				...
				vars.playerType,
				gvars.ply_lastPlayerTypeUsingTemp
			)
		)
	end
...
function r.Print2D(e)
	if e == nil then
		return
	end
	local n = e.showTime or (3 * 30)
	local t = e.xPos or 25
	local o = e.yPos or 425
	local s = e.size or 20
	local a = e.color or "white"
	local e = e.text or ""
	a = r._GetColor(a)
	GrxDebug.Print2D({ life = n, x = t, y = o, size = s, color = a, args = { e } })
end
function r.DEBUG_MakeUserSVarList(e)
	if not t(e) then
		return
	end
...
function r.DEBUG_Init()
	...
	(nil).AddDebugMenu("LuaMission", "DBG.showSVars", "bool", mvars.debug, "showSVars")
	mvars.debug.showMVars = false
	(nil).AddDebugMenu("LuaMission", "DBG.showMVars", "bool", mvars.debug, "showMVars")
	mvars.debug.showMissionArea = false
	(nil).AddDebugMenu("LuaMission", "MIS.missionArea", "bool", mvars.debug, "showMissionArea")
	mvars.debug.showClearState = false
	(nil).AddDebugMenu("LuaMission", "MIS.clearState", "bool", mvars.debug, "showClearState")
	mvars.debug.openEmergencyTimer = false
	(nil).AddDebugMenu("LuaMission", "MIS.openEmergencyTimer", "bool", mvars.debug, "openEmergencyTimer")
	mvars.debug.closeEmergencyTimer = false
	(nil).AddDebugMenu("LuaMission", "MIS.closeEmergencyTimer", "bool", mvars.debug, "closeEmergencyTimer")
	mvars.debug.showDebugPerfCheck = false
	(nil).AddDebugMenu("LuaSystem", "DBG.showPerf", "bool", mvars.debug, "showDebugPerfCheck")
	mvars.debug.showSysSVars = false
	(nil).AddDebugMenu("LuaSystem", "DBG.showSysSVars", "bool", mvars.debug, "showSysSVars")
	mvars.debug.showSysMVars = false
	(nil).AddDebugMenu("LuaSystem", "DBG.showSysMVars", "bool", mvars.debug, "showSysMVars")
	mvars.debug.AnimalBlock = false
	(nil).AddDebugMenu("LuaSystem", "DBG.AnimalBlock", "bool", mvars.debug, "AnimalBlock")
	mvars.debug.ply_rotCheck = 0
	(nil).AddDebugMenu("LuaSystem", "PLY.rotCheck", "int32", mvars.debug, "ply_rotCheck")
	mvars.debug.ply_intelTrap = false
	(nil).AddDebugMenu("LuaSystem", "PLY.intelTrap", "bool", mvars.debug, "ply_intelTrap")
	mvars.debug.enableAllMessageLog = false
	(nil).AddDebugMenu("LuaMessage", "enableAllMessageLog", "bool", mvars.debug, "enableAllMessageLog")
	mvars.debug.showSubscriptMessageTable = 0
	(nil).AddDebugMenu("LuaMessage", "subScripts", "int32", mvars.debug, "showSubscriptMessageTable")
	mvars.debug.showSequenceMessageTable = 0
	(nil).AddDebugMenu("LuaMessage", "sequence", "int32", mvars.debug, "showSequenceMessageTable")
	mvars.debug.showLocationMessageTable = 0
	(nil).AddDebugMenu("LuaMessage", "location", "int32", mvars.debug, "showLocationMessageTable")
	mvars.debug.showLibraryMessageTable = 0
	(nil).AddDebugMenu("LuaMessage", "library", "int32", mvars.debug, "showLibraryMessageTable")
	mvars.debug.showWeaponSelect = false
	(nil).AddDebugMenu("LuaWeapon", "showWeaponSelect", "bool", mvars.debug, "showWeaponSelect")
	mvars.debug.weaponCategory = 1
	(nil).AddDebugMenu("LuaWeapon", "category", "int32", mvars.debug, "weaponCategory")
	...
	mvars.debug.selectedWeaponId = 0
	(nil).AddDebugMenu("LuaWeapon", "weaponSelect", "int32", mvars.debug, "selectedWeaponId")
	mvars.debug.enableWeaponChange = false
	(nil).AddDebugMenu("LuaWeapon", "enableWeaponChange", "bool", mvars.debug, "enableWeaponChange")
end
function r.DEBUG_OnReload(a)
	l = {}
	g = {}
	d = {}
	i = 0
	s = 0
	r.PERF_CHECK_TYPE = Tpp.Enum(r.PERF_CHECK_TYPE)
	local e = {}
...
function r.DebugUpdate()
	...
	if not TppUiCommand.IsEndMissionTelop() then
		o(a, { 0.5, 0.5, 1 }, "Now showing result.")
	end
	if gvars.needWaitMissionInitialize then
		o(a, { 0.5, 0.5, 1 }, "Now neew wait mission initialize.")
	end
	...
	if t.showSVars then
		o(a, "")
		o(a, { 0.5, 0.5, 1 }, "LuaMission DBG.showSVars")
		for r, e in pairs(e.dbg_userSaveVarList) do
			o(a, string.format(" %s = %s", tostring(e), tostring(n[e])))
		end
	end
	if t.showMVars then
		o(a, { 0.5, 0.5, 1 }, "LuaMission DBG.showMVars")
		for r, e in pairs(e) do
			o(a, string.format(" %s = %s", tostring(r), tostring(e)))
		end
	end
	if t.showMissionArea then
		o(a, { 0.5, 0.5, 1 }, "LuaMission MIS.missionArea")
		local r
		...
		o(
			...
			"hotZone clear check : isNotAlert = "
				.. (
					...
					.. (
						", isPlayerStatusNormal = "
						.. (
							tostring(e.debug.notHotZone_isPlayerStatusNormal)
							.. (", isNotHelicopter = " .. tostring(e.debug.notHotZone_isNotHelicopter))
						)
					)
				)
		)
		o(a, "Mission clear timer: " .. tostring(v("Timer_OutsideOfHotZoneCount")))
		o(a, { 0.5, 1, 0.5 }, "Recent all target status")
	...
	if e.debug.showClearState then
		o(a, { 0.5, 0.5, 1 }, "LuaMission MIS.showClearState")
		o(a, "missionClearState = " .. tostring(TppDefine.MISSION_CLEAR_STATE_LIST[gvars.mis_missionClearState + 1]))
	end
	...
	if t.showSysSVars then
		o(a, "")
		o(a, { 0.5, 0.5, 1 }, "LuaSystem DBG.showSysSVars")
		for e, r in pairs(n.__as) do
			if r <= 1 then
				o(a, string.format(" %s = %s", tostring(e), tostring(n[e])))
			else
				o(a, string.format(" %s = %s", tostring(e), tostring(r)))
				for r = 0, (r - 1) do
					o(a, string.format("   %s[%d] = %s", tostring(e), r, tostring(n[e][r])))
				end
			end
		end
	end
	if t.showDebugPerfCheck then
		o(a, { 0.5, 0.5, 1 }, "LuaSystem DBG.showPerf")
		for t, e in pairs(d) do
			o(a, " perf[" .. (r.PERF_CHECK_TYPE[t] .. ("] = " .. e)))
		end
	end
	if e.debug.AnimalBlock then
		o(a, { 0.5, 0.5, 1 }, "LuaSystem DBG.AnimalBlock")
		local t, n = Tpp.GetCurrentStageSmallBlockIndex()
		o(a, string.format("current block position (x,y) = (%03d, %03d)", t, n))
		o(a, "Load animal block area = " .. tostring(e.animalBlockAreaName))
		...
		o(a, "animal block state : " .. tostring(n))
		if e.animalBlockScript then
			o(a, "animalBlockScript exist")
			local n = ""
			...
			o(a, "OnMessage " .. (tostring(n) .. (" OnReload " .. tostring(t))))
			r.ShowMessageTable(a, "MessageTable", e.animalBlockScript.messageExecTable)
		else
			if t == ScriptBlock.SCRIPT_BLOCK_STATE_INACTIVE or t == ScriptBlock.SCRIPT_BLOCK_STATE_ACTIVE then
				o(a, { 1, 0, 0 }, "this data is invalid!!!! please check data!!!")
			else
				o(a, "animalBlockScript   not")
			end
		end
	end
	if e.debug.ply_intelTrap then
		o(a, { 0.5, 0.5, 1 }, "LuaSystem PLY.intelTrap")
		for e, r in pairs(e.ply_intelTrapInfo) do
			if Tpp.IsTypeString(e) then
				o(a, { 0.5, 1, 0.5 }, "intelName = " .. tostring(e))
				for r, e in pairs(r) do
					o(a, tostring(r) .. (" = " .. tostring(e)))
				end
			end
		end
	end
	if t.showSubscriptMessageTable > 0 then
		o(a, { 0.5, 0.5, 1 }, "LuaMessage subScripts")
		...
		if o then
			...
			if e.rad_subScripts[o] then
				...
				r.ShowMessageTable(a, t, e)
			end
		end
	end
	if t.showSequenceMessageTable > 0 then
		o(a, { 0.5, 0.5, 1 }, "LuaMessage sequence")
	...
	if e.debug.showWeaponSelect then
		...
		o(a, { 0.5, 1, 0.5 }, "Current weapon category : " .. n[1])
		local i, p
		local r, t, s = 0, 1, 5
			...
			if l then
				...
				if (t - r) <= s then
					if r == t then
						i = d
						p = e
						o(a, { 0.5, 1, 0.5 }, "> EquipId = TppEquip." .. e)
					else
						o(a, "  EquipId = TppEquip." .. e)
					end
				end
				if r == (t + s) then
					break
		...
		if e.debug.enableWeaponChange then
			GameObject.SendCommand(
				{ type = "TppPlayer2", index = PlayerInfo.GetLocalPlayerIndex() },
				{ id = "DEBUG_ChangeEquip", equipId = { i } }
			)
			e.debug.enableWeaponChange = false
		end
	end
end
function r.ShowMessageTable(e, o, r)
	local a = (nil).Print
	a(e, { 0.5, 1, 0.5 }, o)
	if r == nil then
		return
	...
	for o, r in pairs(r) do
		...
		if r then
			for o, r in pairs(r) do
				local o = p(o)
				if r.func then
					a(e, { 1, 1, 1 }, t .. (" : " .. (o .. (" : " .. tostring(r.func)))))
				end
				local r = r.sender
				if r then
					for n, r in pairs(r) do
						a(e, { 1, 1, 1 }, t .. (" : " .. (o .. (" : Sender = " .. (p(n) .. (" : " .. tostring(r)))))))
					end
				end
			end
		end
...
function r.PerfCheckStart(e)
	...
	if e == a.PERF_CHECK_TYPE.OnUpdate then
		if l[a.PERF_CHECK_TYPE.OnUpdate] ~= nil then
			i = i + (os.clock() - l[a.PERF_CHECK_TYPE.OnUpdate])
		end
	end
	l[e] = os.clock()
end
function r.PerfCheckEnd(e, a)
	local t = mvars
	local o = r
	...
	d[e] = string.format("%4.2f", r * 1e3) .. ("ms." .. n)
