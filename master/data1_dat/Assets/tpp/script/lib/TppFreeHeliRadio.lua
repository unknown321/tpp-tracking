...
e.DAY_TIME = 3
e.NIGHT_TIME = 19
e.DAY_TO_NIGHT = string.format("%d:00:00", e.NIGHT_TIME)
e.NIGHT_TO_DAY = string.format("%d:00:00", e.DAY_TIME)
e.isPlayBreefing = false
e.ON_ENTER_RESULT = { START_PANDEMIC_TUTORIAL = 1 }
e.PANDEMIC_RADIO = {
...
function e.Messages()
	return n({
		Player = {
			{
				...
				func = function(n, i, n, n, n)
					if Tpp.IsSoldier(i) then
						local n = "f2000_rtrg0040"
						if
							(not TppRadio.IsPlayed(n) and TppRadio.IsPlayed("f2000_oprg0210"))
							and (not TppStory.IsMissionCleard(10040))
						then
							e._PlayRadio(n)
						end
						if not TppStory.IsMissionCleard(10070) then
							local n = "f2000_rtrg0060"
							local i = a(i, { id = "GetStateFlag" })
							if o(i, StateFlag.DYING_LIFE) ~= 0 then
		...
		GameObject = {
			{
				...
				func = function(n)
					if Tpp.IsHostage(n) then
						e._UnregisterOptionRadio("f2000_oprg0105")
					end
				end,
			},
			{
				...
				func = function(n)
					if Tpp.IsHostage(n) then
						e._RegisterOptionRadio("f2000_oprg0105")
					end
				end,
			},
			{
				...
				func = function(i)
					...
					if
						(Tpp.IsSoldier(i) and not mvars.FreeHeliRadio_addOptionRadioCount[n])
						and (not TppStory.IsMissionCleard(10040))
					then
						e._RegisterOptionRadio(n)
					end
				end,
			},
			{
				msg = "FultonFailed",
				func = function(i, t, t, n)
					if Tpp.IsSoldier(i) and n == TppGameObject.FULTON_FAILED_TYPE_ON_FINISHED_RISE then
						...
						if (vars.weather ~= TppDefine.WEATHER.SUNNY) and (not TppRadio.IsPlayed(n)) then
							...
							local n = a(i, { id = "GetStateFlag" })
							if (o(n, StateFlag.DYING_LIFE) ~= 0) and (math.random(1, 2) < 2) then
								e._PlayRadio("f2000_rtrg0070")
							else
								e._PlayRadio("f2000_rtrg0050")
							end
						end
					end
				end,
		...
		MotherBaseManagement = { { msg = "UpdatedPandemic", func = e.UpdatePandemicEvent } },
...
function e.OnEnter()
	...
	TppMission.RegisterMissionSystemCallback({ OnAddStaffsFromTempBuffer = e.OnAddStaffsFromTempBuffer })
	TppCassette.OnEnterFreeHeliPlay()
	local n = {}
	if TppMotherBaseManagement.GetStaffCount() <= 10 then
		e._RegisterOptionRadio("f2000_oprg0020")
	end
	if TppMotherBaseManagement.GetDevelopedEquipCount() <= 5 then
		e._RegisterOptionRadio("f2000_oprg0030")
	end
	if not TppStory.IsMissionCleard(10040) then
		e._RegisterOptionRadio("f2000_oprg0220")
	end
	if not e._IsTimeOfDay() then
		e.RegistNvgOptionRadio()
	end
	if vars.weather == TppDefine.WEATHER.SANDSTORM then
		e._RegisterOptionRadio("f2000_oprg0185")
	end
	local n = e.TryPandemicStart()
	...
	local i = TppStory.GetClearedMissionCount({
		10010,
		10020,
		10030,
		10036,
...
function e.RegistNvgOptionRadio()
	mvars.FreeHeliRadio_nvgRadioGroup = nil
	if not TppMotherBaseManagement.IsEquipDeveloped({ equipID = TppEquip.EQP_IT_Nvg }) then
		...
		e._RegisterOptionRadio(mvars.FreeHeliRadio_nvgRadioGroup)
	end
end
function e.UnregistNvgOptionRadio()
	if mvars.FreeHeliRadio_nvgRadioGroup then
		e._UnregisterOptionRadio(mvars.FreeHeliRadio_nvgRadioGroup)
	end
end
function e.RegistAnimalOptionalRadio(n)
	...
	if mvars.FreeHeliRadio_animalRadioGroup then
		e._RegisterOptionRadio(mvars.FreeHeliRadio_animalRadioGroup)
	end
end
function e.UnregistAnimalOptionalRadio()
	if not mvars.FreeHeliRadio_addOptionRadioCount then
		return
	end
	if mvars.FreeHeliRadio_animalRadioGroup then
		e._UnregisterOptionRadio(mvars.FreeHeliRadio_animalRadioGroup)
		mvars.FreeHeliRadio_animalRadioGroup = nil
	end
end
function e.OnEnterCpIntelTrap(n)
	if not mvars.FreeHeliRadio_addOptionRadioCount then
		return
	end
	...
	if TppClock.GetTimeOfDay() == "night" then
		...
		e._RegisterOptionRadio("f2000_oprg0155")
	end
end
function e.OnExitCpIntelTrap(n)
	if not mvars.FreeHeliRadio_addOptionRadioCount then
		return
	end
...
function e.TryPandemicStart()
	...
	if TppMotherBaseManagement.IsPandemicEventMode() then
		if not TppRadio.IsPlayed("f2000_rtrg9010") then
			if i then
				e._PlayRadio(e.PANDEMIC_RADIO.START_CONTINED_IN_HELI)
				n = true
			end
		end
	end
...
local i = 0.85
local a = 0.2
function e.UpdatePandemicEvent(o, n)
	...
	if n > i then
		if not TppRadio.IsPlayed(e.PANDEMIC_RADIO.FINISH) then
			e._PlayRadio(e.PANDEMIC_RADIO.FINISH)
		end
		return
	end
	if n > a then
		if not TppRadio.IsPlayed(e.PANDEMIC_RADIO.ISOLATE_SUCCEED_MANY) then
			e._PlayRadio(e.PANDEMIC_RADIO.ISOLATE_SUCCEED_MANY)
		end
		return
	end
	if o > 0 then
		if not gvars.trm_doneIsolateByManual then
			e._PlayRadio(e.PANDEMIC_RADIO.NO_ISOLATED_YET)
			return
		end
	end
end
function e._RegistClock()
	TppClock.RegisterClockMessage("ChangeDayToNight", e.DAY_TO_NIGHT)
	TppClock.RegisterClockMessage("ChangeNightToDay", e.NIGHT_TO_DAY)
end
function e._PlayRadio(e)
	TppRadio.Play(e, { isEnqueue = true, delayTime = "long" })
end
function e._RegisterOptionRadio(e)
...
function e._UnregisterOptionRadio(e, i)
	local n = mvars.FreeHeliRadio_addOptionRadioCount[e]
	if n then
		if n > 1 then
			mvars.FreeHeliRadio_addOptionRadioCount[e] = n - 1
...
function e._IsRegistOptionRadio(e)
	return mvars.FreeHeliRadio_addOptionRadioCount[e]
end
function e._IsTimeOfDay()
	local n = TppClock.GetTime("time")
	return (n >= e.DAY_TIME) and (n <= e.NIGHT_TIME)
end
return e
