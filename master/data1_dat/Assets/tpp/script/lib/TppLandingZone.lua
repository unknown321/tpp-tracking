...
function a.OnMissionCanStart(_, o)
	for _, e in pairs(a.ConnectLandingZoneTable) do
		if not n.IsBrokenGimmick(_) then
			a.DisableLandingZone(a.ConnectLandingZoneTable[_], o)
		end
		n.RegisterAssaultDropLandingZone(e.drpLandingZoneName)
	end
end
function a.DisableLandingZone(e, o)
	local _
	local n
	if o == "heli" then
		_ = "drpLandingZoneName"
		n = TppUiCommand.AddDisabledLandPoint
	else
		_ = "aprLandingZoneName"
		n = a.GroundDisableLandingZone
	end
	local a = e[_]
	if a then
...
function a.GroundDisableLandingZone(n)
	if TppHelicopter.GetLandingZoneExists({ landingZoneName = n }) then
		TppHelicopter.SetDisableLandingZone({ landingZoneName = n })
	end
end
local _ = {}
_.ConnectLandingZoneTable = {
	banana_aacr001 = {
...
function _.OnMissionCanStart(a, e)
	for a, o in pairs(_.ConnectLandingZoneTable) do
		if not n.IsBrokenGimmick(a) then
			_.DisableLandingZone(_.ConnectLandingZoneTable[a], e)
		end
		n.RegisterAssaultDropLandingZone(o.drpLandingZoneName)
	end
end
function _.DisableLandingZone(e, o)
	local n
	local a
	if o == "heli" then
		n = "drpLandingZoneName"
		a = TppUiCommand.AddDisabledLandPoint
	else
		n = "aprLandingZoneName"
		a = _.GroundDisableLandingZone
	end
	local n = e[n]
	if n then
...
function _.GroundDisableLandingZone(n)
	if TppHelicopter.GetLandingZoneExists({ landingZoneName = n }) then
		TppHelicopter.SetDisableLandingZone({ landingZoneName = n })
	end
end
function n.OnInitialize()
	local n = {}
	Tpp.MergeTable(n, a.ConnectLandingZoneTable)
	Tpp.MergeTable(n, _.ConnectLandingZoneTable)
	TppGimmick.SetUpConnectLandingZoneTable(n)
	mvars.ldz_assaultDropLandingZoneTable = {}
	local _ = {}
	local a = {}
	for e, n in pairs(n) do
		local e = n.drpLandingZoneName
		local n = n.aprLandingZoneName
		table.insert(_, e[1])
		table.insert(a, n[1])
	end
	if TppUiCommand.RegisterDangerLandPointTable ~= nil then
		TppUiCommand.RegisterDangerLandPointTable(_)
	end
	if TppUiCommand.RegisterDangerLandingZoneTable ~= nil then
		TppUiCommand.RegisterDangerLandingZoneTable(a)
	end
end
function n.OnMissionCanStart()
	if (TppLocation.IsAfghan() or TppLocation.IsMiddleAfrica()) or TppLocation.IsMotherBase() then
		local o, e = TppMission.ParseMissionName(TppMission.GetMissionName())
		if e == "heli" or e == "free" then
			TppUiCommand.ClearAllDisabledLandPoints()
			n.DisableLandingZoneForMission(a.MissionLandingZoneTable, e)
			n.DisableLandingZoneForMission(_.MissionLandingZoneTable, e)
		end
		a.OnMissionCanStart(o, e)
		_.OnMissionCanStart(o, e)
		if TppQuest.IsActive("waterway_q99010") then
			a.DisableLandingZone(a.ConnectLandingZoneTable.sovietBase_aacr001, "heli")
			a.DisableLandingZone(a.ConnectLandingZoneTable.powerPlant_aacr001, "heli")
		end
	end
end
function n.DisableLandingZoneForMission(a, n)
	local e
	local o
	if n == "heli" then
		e = "drpLandingZoneName"
		o = TppUiCommand.AddDisabledLandPoint
	else
		e = "aprLandingZoneName"
		o = _.GroundDisableLandingZone
	end
	for a, n in ipairs(a) do
		local a = true
		if not n.missionList then
			a = true
		else
			for _, n in ipairs(n.missionList) do
				if TppStory.IsMissionOpen(n) then
					a = false
				end
			end
		end
...
function n.DisableUnlockLandingZoneOnMission(n)
	mvars.ldz_isDisableUnlockLandingZone = n
end
function n.IsDisableUnlockLandingZoneOnMission()
	return mvars.ldz_isDisableUnlockLandingZone
end
function n.RegisterAssaultDropLandingZone(n)
	for a, n in ipairs(n) do
		local n = Fox.StrCode32(n)
		mvars.ldz_assaultDropLandingZoneTable[n] = true
	end
end
function n.IsAssaultDropLandingZone(n)
	if not mvars.ldz_assaultDropLandingZoneTable then
		return
	end
	local n = mvars.ldz_assaultDropLandingZoneTable[n]
	return n
end
function n.IsBrokenGimmick(n)
	local n = i[n]
	if n == nil then
		return
	end
	return Gimmick.IsBrokenGimmick(n.type, n.locatorName, n.dataSetName)
end
function n.OverwriteBuddyVehiclePosForALZ()
	local n = {}
	n = {
		cliffTown = {
	...
	for a, n in pairs(n) do
		TppEnemy.NPCEntryPointSetting(n)
	end
end
return n
