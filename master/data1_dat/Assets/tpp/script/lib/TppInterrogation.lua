...
function e.Messages()
	return Tpp.StrCode32Table({
		GameObject = {
			{ msg = "Interrogate", func = e._OnInterrogation },
			{ msg = "InterrogateEnd", func = e._OnInterrogationEnd },
			{ msg = "MapUpdate", func = e._OnMapUpdate },
		},
	})
end
...
function e.OnMessage(i, r, n, a, t, o, l)
	Tpp.DoMessage(e.messageExecTable, TppMission.CheckMessageOption, i, r, n, a, t, o, l)
end
function e.MakeFlagHigh(e)
	if e == n then
		return
	end
...
function e.UniqueInterrogationWithVoice(a, i)
	if a == n then
		return
	end
	local e = 0
...
function e.UniqueInterrogation(i, a)
	if i == n then
		return
	end
	local e = 0
...
function e.QuestInterrogation(a, i)
	if a == n then
		return
	end
	local e = 0
...
function e.SetQuestHighIntTable(e, a)
	if e == n then
		return
	end
...
function e.RemoveQuestHighIntTable(e, a)
	if e == n then
		return
	end
	if i(mvars.interTable[e].layer.high) == false then
...
function e.AddHighInterrogation(e, a)
	if e == n then
		return
	end
	if i(mvars.interTable[e].layer.high) == false then
...
function e.RemoveHighInterrogation(e, a)
	if e == n then
		return
	end
	if i(mvars.interTable[e].layer.high) == false then
...
function e.InitUniqueInterrogation(e)
...
function e.InitInterrogation(e)
	mvars.interTable = {}
	local a = 0
...
function e._OnMapUpdate()
	TppUI.ShowAnnounceLog("updateMap")
end
function e._OnInterrogation(a, n, r)
	if r > 0 then
	end
	if mvars.questTable ~= nil then
	...
	local a = e._SelectInterrogation(n, r)
	if a == nil then
		return
	end
	e._AssignInterrogation(n, a.name, a.index)
end
function e._OnInterrogationEnd(r, n, i, e)
	if e == 0 then
		return
	end
	local e = e
	...
	if e > 32 then
		...
		local n = svars.InterrogationHigh[a]
		svars.InterrogationHigh[a] = bit.band(n, bit.bnot(bit.lshift(1, e - 1)))
	else
		local i = false
...
function e._AssignInterrogation(e, n, a)
	r(e, { id = "AssignInterrogation", messageId = n, index = a })
end
function e._AssignInterrogationCollection(e)
	r(e, { id = "AssignInterrogationCollection" })
end
function e._SelectInterrogation(n, a)
	if mvars.interTable == nil then
		if a > 0 then
			e._AssignInterrogationCollection(n)
			return nil
		end
