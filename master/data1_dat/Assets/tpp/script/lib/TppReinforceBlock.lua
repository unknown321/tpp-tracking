...
e.REINFORCE_TYPE_NAME =
	{ "NONE", "EAST_WAV", "EAST_WAV_ROCKET", "WEST_WAV", "WEST_WAV_CANNON", "EAST_TANK", "WEST_TANK", "HELI" }
...
e.REINFORCE_SOLDIER_NAMES =
	{ "reinforce_soldier_0000", "reinforce_soldier_0001", "reinforce_soldier_0002", "reinforce_soldier_0003" }
...
function e.LoadReinforceBlock(i, o, t)
	...
	local c = TppEnemy.GetCpSubType(mvars.reinforce_cpId)
	local c = e.GetFpk(i, c, t)
	if c == nil then
	...
	if i ~= e.REINFORCE_TYPE.NONE then
		...
		n(
			...
			{ id = "SetNominateList", driver = i, vehicle = c, sol01 = r[1], sol02 = r[2], sol03 = r[3], sol04 = r[4] }
		)
	else
		mvars.reinforce_reinforceCpId = r
	end
end
function e.UnloadReinforceBlock(n)
	if not mvars.reinforce_hasReinforceBlock then
		return
	end
...
function e.ReinforceBlockOnUpdate()
	local n = e.GetReinforceBlockState()
	if n == nil then
		return
	end
	local c = ScriptBlock
...
function e._HasSoldier()
	if
		(
			(
				mvars.reinforce_reinforceType == e.REINFORCE_TYPE.HELI
				or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.EAST_WAV_ROCKET
			) or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.EAST_TANK
		) or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.WEST_TANK
	then
		return false
	end
	return true
end
function e._HasVehicle()
	if
		(
			(
				(
					(
						mvars.reinforce_reinforceType == e.REINFORCE_TYPE.EAST_WAV
						or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.EAST_WAV_ROCKET
					) or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.WEST_WAV
				) or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.WEST_WAV_CANNON
			) or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.EAST_TANK
		) or mvars.reinforce_reinforceType == e.REINFORCE_TYPE.WEST_TANK
	then
		return true
...
function e._GetHeliRoute(e)
	return "reinforce_heli_route_0000"
end
function e._SetEnabledSoldier(e, i)
...
function e._SetEnabledVehicle(c, o)
	...
	if o then
		...
		local e = { id = "Respawn", name = c, type = 9, subType = r, paintType = o, class = e }
		n(i, e)
	else
...
function e._OnRequestLoadReinforce(i)
