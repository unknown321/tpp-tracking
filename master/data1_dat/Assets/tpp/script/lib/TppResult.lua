...
e.NO_SPECIAL_BONUS = { [10030] = true, [10115] = true, [10240] = true }
function e.AcquireSpecialBonus(t)
	...
	if t.first then
		if mvars.res_isExistFirstSpecialBonus then
			e._AcquireSpecialBonus(
				t.first,
				"bestScoreBounus",
				"bestScoreBounusScore",
				mvars.res_firstSpecialBonusMaxCount,
				e.COMMON_SCORE_PARAM.firstSpecialBonus,
				"isCompleteFirstBonus",
				mvars.res_firstBonusMissionTask,
				mvars.res_firstSpecialBonusPointList,
	...
	if t.second then
		if mvars.res_isExistSecondSpecialBonus then
			e._AcquireSpecialBonus(
				...
				mvars.res_secondSpecialBonusMaxCount,
				e.COMMON_SCORE_PARAM.secondSpecialBonus,
				"isCompleteSecondBonus",
				mvars.res_secondBonusMissionTask,
				mvars.res_secondSpecialBonusPointList,
				"isAcquiredSecondBonusInPointList"
			)
		end
	end
end
function e._AcquireSpecialBonus(t, a, o, s, c, u, i, r, l)
	local n = t.isComplete
	...
	if t.pointIndex then
		...
		svars[l][t] = true
		local e, t = e.CalcPoinListBonusScore(r, l)
		svars[a] = e
		svars[o] = t
		if svars[a] == #r then
			n = true
...
function e.SetMissionFinalScore()
	...
	a = e.UpdateGmpOnMissionClear(vars.missionCode, t, r)
	...
	if mvars.res_enablePlayStyle then
		e.SaveMissionClearPlayStyleParameter()
		svars.playStyle = e.DecidePlayStyle()
...
function e.DeclareSVars()
	return {
		...
		{
			name = "isAcquiredFirstBonusInPointList",
		...
		{
			name = "isAcquiredSecondBonusInPointList",
			type = TppScriptVars.TYPE_BOOL,
			value = false,
			save = true,
			arraySize = 16,
		...
		{
			name = "bestScoreRediscoveryCount",
			type = TppScriptVars.TYPE_UINT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreTacticalActionPoint",
			type = TppScriptVars.TYPE_UINT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreRediscoveryCountScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreTakeHitCountScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreTacticalActionPointScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScorePerfectStealthNoKillBonusScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreNoTraceBonusScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreInterrogateScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreHeadShotBonusScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreNeutralizeBonusScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
		...
		{
			name = "bestScoreHitRatioBonusScore",
			type = TppScriptVars.TYPE_INT32,
			value = 0,
			save = true,
			sync = false,
...
function e.Init(t)
	...
	if t.sequence then
		...
		mvars.res_hitRatioBonusParam = {
			...
			exponetHitRatio = 6,
			limitHitRatioBonus = 1e3,
			perfectBonusBase = 3e4,
		}
	...
	if t.score and t.score.missionScoreTable then
		...
		e.SetMissionScoreTable({
			baseTime = { S = 300, A = 600, B = 1800, C = 5580, D = 6480, E = 8280 },
			tacticalTakeDownPoint = { countLimit = 40 },
			missionUniqueBonus = { 5e3, 5e3 },
		})
	end
...
function e.OnMessage(t, n, o, r, i, s, a)
	Tpp.DoMessage(e.messageExecTable, TppMission.CheckMessageOption, t, n, o, r, i, s, a)
end
...
function e.CalcBaseScore()
	...
	for e = 1, n do
		...
		local t = mvars.res_missionScoreTable.baseTime[s] * i
	...
	if t == "s10040" then
		a = e.RANK_BASE_SCORE_10040[s]
	elseif t == "s10054" or t == "s11054" then
		a = e.RANK_BASE_SCORE_10054[s]
	elseif t == "s10130" or t == "s11130" then
		a = e.RANK_BASE_SCORE_10130[s]
	elseif t == "s10140" or t == "s11140" then
		a = e.RANK_BASE_SCORE_10140[s]
	else
		a = e.RANK_BASE_SCORE[s]
	end
...
local o = (t * 60) * 4
local n = (t * 60) * 0.5
function e.CalcTimeScore(r, s)
	if not mvars.res_missionScoreTable then
		return
	end
	local i = svars
	local a = TppDefine.MISSION_CLEAR_RANK_LIST[s]
	local a = mvars.res_missionScoreTable.baseTime[a]
	local a = a - (i.bestScoreTime * p)
	if a < 0 then
		a = 0
	end
	local t = a * t
...
function e.CalcEachScore()
	...
	for r, a in pairs(e.calcScoreTable) do
		...
		t[r] = e.CalcScore(s, mvars.res_scoreTable[a[1]], mvars.res_missionScoreTable[a[3]])
	end
	if not e.IsUsedChickCap() then
		...
		t.bestScoreHitRatioBonusScore = e.CalcHitRatioBonusScore(
			vars.shootHitCountInMission,
			vars.playerShootCountInMission,
			vars.shootHitCountEliminatedInMission,
			t.shootNeutralizeCount,
			mvars.res_hitRatioBonusParam.hitRatioBaseScoreUnit,
			mvars.res_hitRatioBonusParam.numOfBulletsPerNeutralizeCount,
			mvars.res_hitRatioBonusParam.exponetHitRatio,
			mvars.res_hitRatioBonusParam.limitHitRatioBonus,
			mvars.res_hitRatioBonusParam.perfectBonusBase
		)
...
function e.CalcHitRatioBonusScore(l, a, r, t, o, c, u, s, n)
	local a = a - r
	if a <= 0 then
		return 0
	end
...
function e.UpdateGmpOnMissionClear(t, a, r)
	...
	local s = e.GetMissionGuaranteeGMP(t)
	svars.gmpClear = TppTerminal.CorrectGMP({ gmp = s })
	if a ~= TppDefine.MISSION_CLEAR_RANK.NOT_DEFINED then
		local e = e.GetMissionClearRankGMP(a, t)
		e = e + r
...
function e.SetBestRank(t, e)
	local t = TppDefine.MISSION_ENUM[tostring(t)]
	if not t then
		return
	end
...
function e.GetMissionClearRankGMP(s, t)
	local a = e.GetBestRank(t)
	if not a then
		return 0
	end
	...
	local n = #TppDefine.MISSION_CLEAR_RANK_LIST
	for s = n, s, -1 do
		local n = TppDefine.MISSION_CLEAR_RANK_LIST[s]
...
function e.GetMbMissionListParameterTable()
	...
	for t, a in pairs(TppDefine.MISSION_ENUM) do
		...
		if e.MISSION_GUARANTEE_GMP[t] then
			a.baseGmp = e.MISSION_GUARANTEE_GMP[t]
			a.currentGmp = e.GetMissionGuaranteeGMP(t)
		end
		if e.MISSION_TASK_LIST[t] then
			a.completedTaskNum = TppUI.GetTaskCompletedNumber(t)
			a.maxTaskNum = #e.MISSION_TASK_LIST[t]
			a.taskList = e.MISSION_TASK_LIST[t]
		end
		table.insert(s, a)
	end
	return s
...
local r = 0
function e.AddMissionClearHistory(t)
...
function e.GetMissionClearCountFromHistory(a)
	local e = 0
...
function e._SetSpecialBonusResultScore(t, i, o, n, r, s, a)
	if not a.taskNo then
		TppUiCommand.SetResultScore("invalid", "bonus", t)
		return
	end
	local r = e.MakeMissionTaskLangId(a.taskNo)
	local a = svars[i]
	...
	if e == -1 then
		TppUiCommand.SetResultScore(r, "bonus", t, e, s)
	else
		TppUiCommand.SetResultScore(r, "bonus_rate", t, e, n, s)
	end
end
function e.MakeMissionTaskLangId(t)
	...
	return "task_mission_"
		.. (string.format("%02d", vars.locationCode) .. ("_" .. (tostring(e) .. ("_" .. string.format("%02d", t)))))
end
...
function e.DebugUpdate()
	...
	if a.debug.showHitRatio then
		...
		(nil).Print(t, { 0.5, 0.5, 1 }, "LuaMission RES.hitRatio");
	...
	if a.debug.showMissionClearHistory then
		(nil).Print(t, { 0.5, 0.5, 1 }, "LuaMission RES.clearHistory");
		...
		for t = 0, n do
			e = math.floor(s / r) + 1
			a[e] = a[e] or "   "
			a[e] = a[e] .. (tostring(gvars.res_missionClearHistory[t]) .. ", ")
			s = s + 1
		end
		for e = 1, e do
			(nil).Print(t, a[e])
	...
	if a.debug.showMissionScoreTable and a.res_missionScoreTable then
		(nil).Print(t, { 0.5, 0.5, 1 }, "LuaMission RES.scoreTable");
		...
		if a.res_missionScoreTable.tacticalTakeDownPoint then
			(nil).Print(
				t,
				"tacticalTakeDownPoint : countLimit = "
					.. tostring(a.res_missionScoreTable.tacticalTakeDownPoint.countLimit)
			)
		else
			(nil).Print(t, "cannot find tacticalTakeDown param")
		end
	end
	if a.debug.showPlayData then
		(nil).Print(t, { 0.5, 0.5, 1 }, "LuaMission RES.showPlayData");
		(nil).Print(t, "gvars.totalMissionClearCount = " .. tostring(gvars.totalMissionClearCount));
		(nil).Print(
			t,
			"gvars.totalPerfectStealthMissionClearCount = " .. tostring(gvars.totalPerfectStealthMissionClearCount)
		);
	...
	if a.debug.showPlayStyleHistory then
		(nil).Print(t, { 0.5, 0.5, 1 }, "LuaMission RES.showPlayStyleHistory");
		(nil).Print(
			t,
			{ 0.5, 1, 0.5 },
			"gvars.res_neutralizeHistorySize = " .. tostring(gvars.res_neutralizeHistorySize)
		);
		(nil).Print(t, { 0.5, 1, 0.5 }, " history = 0         | history = 1          | history = 2        ");
		(nil).Print(t, { 0.5, 1, 0.5 }, "isPerfectStealth");
		(nil).Print(
			t,
			"( "
				.. (
					tostring(gvars.res_isPerfectStealth[0])
					.. (
						" ) | ( "
						.. (
							tostring(gvars.res_isPerfectStealth[1])
							.. (" ) | ( " .. (tostring(gvars.res_isPerfectStealth[2]) .. " )"))
						)
					)
				)
		);
		(nil).Print(t, { 0.5, 1, 0.5 }, "isStealth");
		(nil).Print(
			t,
			"( "
				.. (
					tostring(gvars.res_isStealth[0])
					.. (
						" ) | ( "
						.. (
							tostring(gvars.res_isStealth[1]) .. (
								" ) | ( " .. (tostring(gvars.res_isStealth[2]) .. " )")
							)
						)
					)
				)
		);
		(nil).Print(t, { 0.5, 1, 0.5 }, "Head shot count");
		(nil).Print(
			t,
			string.format(
				"( %07d ) | ( %07d ) | ( %07d )",
				gvars.res_headShotCount[0],
				gvars.res_headShotCount[1],
				gvars.res_headShotCount[2]
			)
		);
		(nil).Print(t, { 0.5, 1, 0.5 }, "( historyIndex, neutralizeType, count )")
		for s = 0, TppDefine.PLAYSTYLE_SAVE_INDEX_MAX - 1 do
			local a = ""
			local r = e.DEBUG_NEUTRALIZE_TYPE_TEXT[s + 1]
			for e = 0, TppDefine.PLAYSTYLE_HISTORY_MAX do
				local e = string.format(
					"( %02d, %s, %03d ) | ",
					e,
					r,
					gvars.res_neutralizeCount[e * TppDefine.PLAYSTYLE_SAVE_INDEX_MAX + s]
				)
				a = a .. e
			end
			(nil).Print(t, a)
	...
	if a.debug.showPlayDataNeutralizeCount then
		(nil).Print(t, { 0.5, 0.5, 1 }, "LuaMission RES.showPlayDataNeutralizeCount");
		(nil).Print(t, { 0.5, 1, 0.5 }, "( neutralizeType, count )")
...
function e.Messages()
	return Tpp.StrCode32Table({
		Player = { { msg = "PlayerDamaged", func = e.IncrementTakeHitCount } },
		GameObject = {
			{
				msg = "Dead",
				func = function(e, t, a, a)
			...
			{ msg = "TapHeadShotFar", func = e.OnTacticalActionPoint },
			{ msg = "TapRocketArm", func = e.OnTacticalActionPoint },
			{ msg = "TapHoldup", func = e.OnTacticalActionPoint },
			{ msg = "TapCqc", func = e.OnTacticalActionPoint },
...
function e.OnTacticalActionPoint(t)
	if S(t, { id = "IsDoneTacticalTakedown" }) then
	else
		S(t, { id = "SetTacticalTakedown" })
		e.AddTacticalActionPoint()
	end
end
function e.AddTacticalActionPoint()
	...
	e.CallCountAnnounce("result_tactical_takedown", svars.tacticalActionPoint, a)
...
function e.GetPlayStyleSaveIndex(r, n, a, t)
	if a == NeutralizeType.INVALID then
		return
	end
	local r = e.NPC_CAUSE_TO_SAVE_INDEX[t]
	if r then
		return r
	end
	...
	if Tpp.IsPlayer(n) then
		local a = {
			[NeutralizeCause.NO_KILL_BULLET] = true,
			[NeutralizeCause.HANDGUN] = true,
			[NeutralizeCause.SUBMACHINE_GUN] = true,
			[NeutralizeCause.SHOTGUN] = true,
			[NeutralizeCause.ASSAULT_RIFLE] = true,
			[NeutralizeCause.MACHINE_GUN] = true,
			[NeutralizeCause.SNIPER_RIFLE] = true,
			[NeutralizeCause.MISSILE] = true,
		}
		if a[t] then
...
function e.OnNeutralize(n, r, a, t)
	local t = e.GetPlayStyleSaveIndex(n, r, a, t)
	if not t then
		return
	end
...
function e.AddNewPlayStyleHistory()
	...
	for t = (TppDefine.PLAYSTYLE_HISTORY_MAX - 1), 0, -1 do
		for e = 0, TppDefine.PLAYSTYLE_SAVE_INDEX_MAX - 1 do
			gvars.res_neutralizeCount[(t + 1) * TppDefine.PLAYSTYLE_SAVE_INDEX_MAX + e] =
				gvars.res_neutralizeCount[t * TppDefine.PLAYSTYLE_SAVE_INDEX_MAX + e]
		end
...
function e.GetNeutralizeCountBySaveIndex(a)
	local e = 0
	for t = 0, TppDefine.PLAYSTYLE_HISTORY_MAX do
...
function e.DecideNeutralizePlayStyle()
	...
	local t = -1
	for a = 0, TppDefine.PLAYSTYLE_SAVE_INDEX_MAX - 1 do
		local n = e.GetNeutralizeCountBySaveIndex(a)
		if t < n then
			s = false
			r = a
			t = n
	...
	if s then
		...
		local e = e.NEUTRALIZE_PLAY_STYLE_ID[r + 1]
		if e then
			return e
		else
			return
