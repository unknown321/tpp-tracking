...
for i, packagePath in ipairs(TppDefine.MISSION_COMMON_PACK.AVATAR_ASSET_LIST) do
	table.insert(
		missionPackTable[CYPRUS_SMALL_MISSION_BLOCK_LIST[3]][cypr_mission_block.GetSmallMissionPackageLabel(3, 13)],
		packagePath
	)
end
	...
	for blockName, packageTable in pairs(missionPackTable) do
		...
		if DEBUG then
			DebugText.Print(
				...
				{ 0.5, 0.5, 1.0 },
				"cypr_mission_block.IsCompleteLoading(): state:" .. tostring(state)
			)
		end
...
cypr_mission_block.SetLoadStep = function(step)
	...
	cypr_mission_block.LoadSmallMissionBlock(step)
