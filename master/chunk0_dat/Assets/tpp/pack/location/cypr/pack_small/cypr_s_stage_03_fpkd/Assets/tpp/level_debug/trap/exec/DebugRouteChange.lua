	...
	Exec = function(info)
		if info.trapFlagString == "GEO_TRAP_S_ENTER" then
			...
			if not Entity.IsNull(charaLocator) then
				TppEnemyMoveUtility.ChangeSneakRoute(
					charaLocator,
					info.conditionHandle.nextRoute,
					info.conditionHandle.nextRoutePoint
				)
				TppEnemyMoveUtility.ChangeCautionRoute(
					charaLocator,
					info.conditionHandle.nextCautionRoute,
					info.conditionHandle.nextCautionRoutePoint
				)
			end
