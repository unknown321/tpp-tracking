...
local HELI_SPACE_REFLECTION_PATH =
	"/Assets/tpp/common_source/cubemap/environ/heliSpace/cm_heli_cb_indoor001/sourceimages/cm_heli_cb_indoor001.ftex"
local SORTIE_REFLECTION_PATH =
	"/Assets/tpp/common_source/cubemap/environ/sortie_space/cm_sortie_cb_001/sourceimages/cm_sortie_cb_001.ftex"
...
function this.MissionPrepare()
	...
	if TppPackList.IsMissionPackLabel("default") then
		...
		local maxSightBlockSize = 105 * 1024
		local platform = Fox.GetPlatformName()
		if not ((platform == "Xbox360") or (platform == "PS3")) then
			maxReceiverBlockSize = 330 * 1024
			maxSightBlockSize = 106 * 1024
		end
...
function this.CommonBuddyHeliSpaceSetting(buddyType, locatorName)
	...
	if translation then
		Fox.Log(
			string.format(
				"HeliSpaceSetting. locatorName = %s, pos = (%f, %f, %f)",
				locatorName,
				translation:GetX(),
				translation:GetY(),
				translation:GetZ()
...
function this.Messages()
	return StrCode32Table({
		UI = {
			{
				msg = "EndFadeOut",
				sender = "OnEstablishMissionClearFadeOut",
				func = function()
...
function this.StartRain()
	Gimmick.ResetGimmickData(
		"uth0_rain1_def_gim_i0000|TppPermanentGimmick_uth0_rain1_def",
		"/Assets/tpp/level/mission2/heli/common/heli_common_asset.fox2"
	)
	Gimmick.ChangeSequentialEventAnimation2({
		name = "uth0_rain1_def_gim_n0000|srt_uth0_rain1_def",
		setPath = "/Assets/tpp/level/mission2/heli/common/heli_common_asset.fox2",
		index = 0,
	})
end

function this.StopRain()
	Gimmick.ContinueSquentialEventAnimation({
		name = "uth0_rain1_def_gim_n0000|srt_uth0_rain1_def",
		setPath = "/Assets/tpp/level/mission2/heli/common/heli_common_asset.fox2",
	})
end
...
function this.AddCommonHeliSpaceMessage(messageTable)
	...
	table.insert(
		messageTable.UI,
		{ msg = "EndFadeIn", sender = "FadeInOnStartMissionGame", func = TppTerminal.GetFobStatus }
	)
...
local function HeliSpaceGoToMgoCoroutine()
	...
	if Chunk.GetChunkState(mvars.heliSpaceMgoChunkIndex) ~= Chunk.STATE_INSTALLED then
		...
		while
			(Chunk.GetChunkState(mvars.heliSpaceMgoChunkIndex) ~= Chunk.STATE_INSTALLED)
			and TppUiCommand.IsShowPopup(TppDefine.ERROR_ID.NOW_INSTALLING)
		do
			Tpp.ShowChunkInstallingPopup(mvars.heliSpaceMgoChunkIndex, true)
			coroutine.yield()
...
function this.OnUpdateChunkInstalling()
	...
	if (coroutine.status(mvars.heliSpace_goToMgoCoroutine) == "dead") or not status then
...
function this.OnEndFadeOutSelectLandingPoint()
	TppMission.SelectNextMissionHeliStartRoute(
		mvars.heliSequence_nextMissionCode,
		mvars.heliSequence_heliRoute,
		mvars.heliSequence_startFobSneaking
	)
...
sequences.Seq_Game_MainGame = {
	...
	Messages = function(self)
		local messageTable = {
			UI = {
				{
					msg = "EndFadeOut",
					sender = "OnSelectLandingPoint",
					func = this.OnEndFadeOutSelectLandingPoint,
				},
				{
					msg = "EndFadeOut",
					sender = "OnSelectWeaponCustomize",
					func = function()
			...
			Terminal = {
				...
				{
					msg = "MbDvcActSelectLandPoint",
					func = this.OnSelectLandPoint,
				},
				{
	...
	OnLeave = function(self, nextSequenceName)
		...
		if
			(nextSequenceName == "Seq_Game_MainGameToMissionPreparationTop")
			or (nextSequenceName == "Seq_Game_WeaponCustomize")
		then
	...
	Messages = function(self)
		return StrCode32Table({
			Timer = {
				{
					msg = "Finish",
					sender = "Timer_GoToMissionPreparationTop",
					func = function()
...
sequences.Seq_Game_MissionPreparationTop = {
	...
	Messages = function(self)
		return StrCode32Table({
			UI = {
				{
					msg = "EndFadeIn",
					sender = "OnMissionPreparationStart",
					func = function() end,
				},
				{
					msg = "EndFadeOut",
					sender = "OnMissionPreparationAbort",
					func = function()
	...
	OnEnter = function(self)
		...
		if (nextMissionCode == 10050) or (nextMissionCode == 10260) or (nextMissionCode == 11050) then
...
local SelectCameraParameter = {
	...
	[Fox.StrCode32("MissionPrep_FocusTarget_None")] = { "StageCenter", 3.5, rotX = -5, rotY = 170, interpTime = 0.3 },

	[Fox.StrCode32("Customize_Target_Weapon")] = {
		"CustomizeWeaponPosition",
		1.7,
		rotX = -5,
		rotY = 170,
		interpTime = 0.3,
	...
	[Fox.StrCode32("Customize_Target_BuddyHorse")] = {
		"CustomizeBuddyPosition_HorsePos",
		4.5,
		rotX = -5,
		rotY = 170,
		interpTime = 0.3,
	...
	[Fox.StrCode32("Customize_Target_Heli_Body")] = {
		"CustomizeHelicopterCameraPosition",
		16,
		rotX = -5,
		rotY = 170,
		interpTime = 0.3,
	...
	[Fox.StrCode32("Customize_Target_Heli_Main")] = {
		"CustomizeHelicopterCameraPosition_MainWeapon",
		10,
		rotX = 25,
		rotY = 150,
		interpTime = 0.3,
	...
	[Fox.StrCode32("Customize_Target_Vehicle")] = {
		"CustomizeVehicleCameraPosition",
		12,
		rotX = 15,
		rotY = 150,
		interpTime = 0.2,
...
function this.UpdateCameraParameter(focusTarget, immediately)
	...
	local locatorName, distance, rotX, rotY, interpTime =
		cameraParameter[1], cameraParameter[2], cameraParameter.rotX, cameraParameter.rotY, cameraParameter.interpTime
	Fox.Log(
		"MissionPrepare.UpdateCameraParameter: locatorName = "
	...
	if
		focusTarget == Fox.StrCode32("Customize_Target_Buddy")
		...
		or focusTarget == Fox.StrCode32("MissionPrep_FocusTarget_BuddyBattleGear")
	then
		ignoreCollision = GameObject.CreateGameObjectId("TppWalkerGear2", 0)
	elseif
		focusTarget == Fox.StrCode32("Customize_Target_Vehicle")
		or focusTarget == Fox.StrCode32("MissionPrep_FocusTarget_Vehicle")
	then
	...
	if PlayerMissionPrepareAction then
		GameObject.SendCommand(
			{ type = "TppPlayer2", index = PlayerInfo.GetLocalPlayerIndex() },
			{
				...
				enabled = true,
				type = "MissionPrepare",
				sequence = PlayerMissionPrepareAction.READY,
			}
		)
	end
	...
	Messages = function(self)
		return StrCode32Table({
			UI = {
				...
				{
					msg = "EndFadeOut",
					sender = "OnMissionPreparationEnd",
	...
	Messages = function(self)
		return StrCode32Table({
			Timer = {
				{
					msg = "Finish",
					sender = "Timer_NextMission",
					func = function()
						this.RestoreClock()
						TppMission.OnEndMissionPreparation(
							mvars.heliSpace_selectedDeployTime,
							mvars.heliSequence_clusterCategory
						)
					end,
				},
			},
...
sequences.Seq_Game_WeaponCustomize = {
	...
	Messages = function(self)
		return StrCode32Table({
			UI = {
				{
					msg = "EndFadeIn",
					sender = "OnWeaponCustomizeStart",
					func = function() end,
				},
				{
					msg = "EndFadeOut",
					sender = "OnWeaponCustomizeAbort",
					func = function()
	...
	OnEnter = function(self)
		...
		elseif
			mvars.startCustomizeTarget == Fox.StrCode32("Customize_Target_Helicopter")
			or mvars.startCustomizeTarget == Fox.StrCode32("Customize_Target_Vehicle")
		then
...
sequences.Seq_Game_PandemicTutorial = {
	...
	OnEnter = function(self)
		...
		this.SetPandemicTutorialMbMenuActive(PANDEMIC_TUTORIAL_DISPLAY.STAFF_ENABLED)
		TppUiCommand.RequestMbDvcOpenCondition({
			imagePath = "/Assets/tpp/ui/texture/Photo/tpp/demo/mission_demo_s10211_09.ftex",
		})
	end,
