...
function OnlineChallengeTask.InitializeAscendingOrderTaskValue(detectType, taskNo)
	...
	if currentValue == 0 then
		Fox.Log(
			"OnlineChallengeTask.InitializeAscendingOrderTaskValue : detectType = "
				.. tostring(detectType)
				.. ", initialValue = "
...
OnlineChallengeTask.OnMessage = function(sender, messageId, arg0, arg1, arg2, arg3, strLogText)
	Tpp.DoMessage(
		OnlineChallengeTask.messageExecTable,
		...
		GameObject = {
			{
				...
				msg = "NeutralizeFob",
				func = OnlineChallengeTask.OnNeutralize,
			},
			{
			...
			},
			{
				...
				func = function(gameObjectId, state)
					if (state == STRCODE_START) and Tpp.IsEnemyWalkerGear(gameObjectId) then
...
function OnlineChallengeTask.OnNeutralize(
