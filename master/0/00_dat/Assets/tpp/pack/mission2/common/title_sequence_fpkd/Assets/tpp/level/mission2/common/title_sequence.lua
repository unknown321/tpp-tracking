...
function this.IsBrokenSaveData(missionCode)
	if
		not TppScriptVars.GetVarValueInSlot(TppDefine.SAVE_SLOT.CHECK_POINT, "svars", "mis_isDefiniteMissionClear", 0)
	then
		if
			(missionCode == 10150)
			and (TppScriptVars.GetVarValueInSlot(TppDefine.SAVE_SLOT.CHECK_POINT, "svars", "seq_sequence", 0) == 2)
		then
			return true
		end
	...
	local missionSequenceIndex =
		TppScriptVars.GetVarValueInSlot(TppDefine.SAVE_SLOT.CHECK_POINT, "svars", "seq_sequence", 0)
	...
	if (missionSequenceIndex >= clearedSequence[1]) and (missionSequenceIndex <= clearedSequence[2]) then
		return false
	else
		return true
	end
...
function this.RecoverSaveDataIfPandemicTutorial()
	...
	if
		((vars.missionCode == 10085) or (vars.missionCode == 10200))
		and (not TppTerminal.IsPandemicTutorialFinished())
	then
		this.ReverseMissionClearedFlag()
		this.ReverseStorySequence()
...
sequences.Seq_Game_TitleMenu = {
	...
	OnSelectContinue = function()
		if
			TppMission.IsHelicopterSpace(gvars.title_nextMissionCode)
			or (gvars.title_nextMissionCode == 10115)
			or (gvars.title_nextMissionCode == 50050)
		then
			this.SetOnSelectFlag(SELECT_FLAG.SELECT_RESTART_HELI)
		else
...
sequences.Seq_Game_ChunkInstalled = {
	...
	OnEndFadeOutSelectContinue = function()
		...
		if not this.IsBrokenSaveData(vars.missionCode) then
			...
			if
				...
				and (vars.locationCode == TppDefine.LOCATION_ID.MBQF)
			then
				vars.locationCode = TppDefine.LOCATION_ID.MTBS
				TppScriptVars.SetVarValueInSlot(
					TppDefine.SAVE_SLOT.CHECK_POINT,
					"vars",
					"locationCode",
					TppDefine.LOCATION_ID.MTBS
				)
			end
	...
	OnUpdateSelectMGO = function()
		if mvars.title_patchDlcCheckCoroutine then
			...
			if (coroutine.status(mvars.title_patchDlcCheckCoroutine) == "dead") or not status then
...
this.RestoreSaveDetaForBtk44624 = function()
	...
	local isUseMissionStartPos =
		not TppScriptVars.GetVarValueInSlot(TppDefine.SAVE_SLOT.CHECK_POINT, "gvars", "sav_varRestoreForContinue")
