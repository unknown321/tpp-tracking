...
this.missionObjectiveDefine = {
	...
	marker_area_vip = {
		gameObjectName = "marker_area_vip",
		visibleArea = 1,
		randomRange = 0,
		viewType = "all",
		setNew = true,
	...
	on_itm_s10036_material = {
		gameObjectName = "TppMarker2Locator_field_material",
		goalType = "none",
	...
	on_itm_s10036_herb = {
		gameObjectName = "TppMarker2Locator_field_herb",
		visibleArea = 1,
		randomRange = 0,
		goalType = "none",
		viewType = "map_only_icon",
...
function this.Messages()
	return StrCode32Table({
		...
		Trap = {
			...
			{
				msg = "Enter",
				sender = "trap_CHK_field_E01",
				func = function(trap, player) end,
			},
		},
		Timer = {
			{
				msg = "Finish",
				sender = "Timer_PlayFullutonSE",
				func = function()
...
sequences.Seq_Game_MainGame = {
				...
				},
				{
					...
					func = function(characterId, rideVehicleID)
						...
						if Tpp.IsHelicopter(rideVehicleID) and TppEnemy.CheckAllTargetClear() then
							...
							TppMission.UpdateObjective({ objectives = { "announce_rescue_target", "clear_photo" } })
							TppMission.UpdateObjective({
								objectives = { "clear_missionTask_00", "clear_missionTask_02" },
							})
	...
	TargetUnconscious = function(gameObjectId, attakerId, type, cause)
		...
		Fox.Log(
			"check neutralize state. attaker:"
				.. tostring(attakerId)
				.. " type"
				.. tostring(type)
				.. " cause:"
		...
		if Tpp.IsPlayer(attakerId) then
			if
				type == NeutralizeType.DEAD
				or type == NeutralizeType.FAINT
				or type == NeutralizeType.SLEEP
				or type == NeutralizeType.DYING
				or type == NeutralizeType.FULTON
			then
		...
		if status == TppGameObject.NPC_LIFE_STATE_SLEEP or status == TppGameObject.NPC_LIFE_STATE_FAINT then
