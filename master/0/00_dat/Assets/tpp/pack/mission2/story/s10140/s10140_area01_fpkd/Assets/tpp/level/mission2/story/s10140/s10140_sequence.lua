...
this.VARIABLE_TRAP_SETTING = {
	...
	{
		name = "trig_hotZone",
		type = TppDefine.TRAP_TYPE.TRIGGER,
...
function this.OnSetMissionFinalScore() end
...
function this.OnEndMissionCredit()
	...
	TppScriptBlock.LoadDemoBlock("Demo_Inside_Heli", false)
...
function this.OnEndMissionReward()
	...
	TppScriptBlock.LoadDemoBlock("Demo_PandemicConverge", false)
...
this.SetBuddyQuietPositionOnDemo = function()
	...
	if doesExist then
		Fox.Log(
			"#### s10140_sequence.SetBuddyQuietPositionOnDemo #### Buddy Quiet Exist! warpPos = "
				.. tostring(warpPos)
				.. ", warpRot = "
				.. tostring(warpRot)
		)
...
this.CheckFultonNPC = function(characterId)
	...
	if
		characterId == GameObject.GetGameObjectId("TppParasite2", "wmu_s10140_0000")
		or characterId == GameObject.GetGameObjectId("TppParasite2", "wmu_s10140_0001")
		or characterId == GameObject.GetGameObjectId("TppParasite2", "wmu_s10140_0002")
		or characterId == GameObject.GetGameObjectId("TppParasite2", "wmu_s10140_0003")
	then
...
this.CheckMissionTask_02 = function()
	Fox.Log(
		"#### s10140_sequence.CheckMissionTask_02 #### svars.isBittenByZombies = " .. tostring(svars.isBittenByZombies)
	)
...
this.CheckMissionTask_04 = function()
	Fox.Log(
		"#### s10140_sequence.CheckMissionTask_04 #### svars.recoveryCountParasites = "
			.. tostring(svars.recoveryCountParasites)
	)
...
function this.CheckMissionClear()
	Fox.Log(
		"#### s10140_sequence.CheckMissionClear #### isNearCodeTalke = "
			.. tostring(svars.isNearCodeTalker)
			.. ", isArrivedHeli = "
			.. tostring(svars.isArrivedHeli)
			.. ", isStayClearTrap = "
			.. tostring(svars.isStayClearTrap)
	)
...
this.StartTimer = function(timerName, timerTime)
	Fox.Log(
		"#### s10140_sequence.StartTimer #### timerName = "
			.. tostring(timerName)
			.. ", timerTime = "
			.. tostring(timerTime)
	)
...
this.SetMeshForRustyTank = function() end
...
this.CheckNgAction = function(actor, target, type)
	Fox.Log(
		"#### s10050_sequence.CheckNgAction #### actor = [ "
			.. actor
			.. " ], target = [ "
			.. target
			.. " ], type = [ "
	...
	elseif
		target == target_parasite[1]
		or target == target_parasite[2]
		or target == target_parasite[3]
		or target == target_parasite[4]
	then
		if type == "fulton" then
	...
	Messages = function(self)
		return StrCode32Table({
			GameObject = {
				...
				{
					msg = "StartedSmokeAction",
					func = function()
						s10140_radio.BeCarefulRustyGus()
					end,
				},
			},
			Player = {

				{
					msg = "ZombBiteConnect",
					func = function()
						svars.isBittenByZombies = true
					end,
				},
				{
					msg = "PressedCarryNgIcon",
					func = function(arg1, arg2)
						this.CheckNgAction(arg1, arg2, "carry")
					end,
				},
				{
					msg = "PressedFultonNgIcon",
					func = function(arg1, arg2)
						this.CheckNgAction(arg1, arg2, "fulton")
					end,
				},
			},
			Radio = {
				{
					msg = "Finish",
					sender = "s0140_rtrg1010",
					func = function()
						svars.isFinishFirstRadio = true
					end,
				},
			},
			Trap = {
				{
					msg = "Enter",
					sender = "trap_codeTalker",
					func = function()
						s10140_radio.TakeAwayFromCodeTalker("trap")
					end,
				},
				{
					msg = "Exit",
			...
			Terminal = {
				{
					...
					func = function(menuId)
						...
						if
							menuId == StrCode32(TppTerminal.MBDVCMENU.MSN_HELI)
							or menuId == StrCode32(TppTerminal.MSN_HELI_RENDEZVOUS)
						then
							s10140_radio.UseWarningFlare()
						end
					end,
...
sequences.Seq_Game_Escape = {
	Messages = function(self)
		return StrCode32Table({
			...
			GameObject = {
				...
				},
				{
					...
					func = function(gameObjectId, state, gameObject)
						Fox.Log(
							"#### s10140_sequence.Seq_Game_Escape.Messages #### LostControl : SUPPORT_HELI, state = "
								.. tostring(state)
						)
						...
						elseif state == StrCode32("End") then
							TppMission.ReserveGameOver(
								...
								TppDefine.GAME_OVER_RADIO.PLAYER_DESTROY_HELI
							)
						end
					end,
				},
				{
					...
					func = function()
						Fox.Log(
							"#### s10140_sequence.Seq_Game_Escape.Messages #### ArrivedAtLandingZoneWaitPoint : SUPPORT_HELI"
						)
			...
			Radio = {
				{ msg = "Finish", sender = "s0140_rtrg2020", func = function() end },
			},
		})
	end,
...
sequences.Seq_Demo_PandemicConverge = {
	OnEnter = function(self)
		...
		if vars.buddyType == BuddyType.DOG or vars.buddyType == BuddyType.QUIET then
			if
				(TppBuddyService.IsDeadBuddyType(BuddyType.DOG))
				or (
					(vars.buddyType == BuddyType.QUIET)
					and (
						TppBuddyService.CheckBuddyCommonFlag(BuddyCommonFlag.BUDDY_QUIET_LOST)
						or TppBuddyService.CheckBuddyCommonFlag(BuddyCommonFlag.BUDDY_QUIET_HOSPITALIZE)
					)
				)
			then
				...
				Fox.Log(
					"#### s10140_sequence.Seq_Demo_PandemicConverge #### Controled Buddy Exist. [ buddyType = "
						.. vars.buddyType
						.. " ]"
				)
	...
	OnUpdate = function(self)
		if TppBuddy2BlockController.IsBlockActive() then
			if not svars.isPrintLog then
				Fox.Log(
					"#### s10140_sequence.Seq_Demo_PandemicConverge::OnUpdate #### TppBuddy2BlockController.IsBlockActive()"
				)
