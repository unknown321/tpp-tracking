...
local PLAYER_DISABLE = {
	ARRIVE_AT_DD = PlayerDisableAction.FULTON
		+ PlayerDisableAction.RUN
		+ PlayerDisableAction.CQC_KNIFE_KILL
		+ PlayerDisableAction.CQC_INTERROGATE
		+ PlayerDisableAction.KILLING_WEAPON
		+ PlayerDisableAction.TIME_CIGARETTE,
	FIRST_TALK = PlayerDisableAction.FULTON
		+ PlayerDisableAction.CQC_KNIFE_KILL
		+ PlayerDisableAction.CQC_INTERROGATE
		+ PlayerDisableAction.KILLING_WEAPON
		+ PlayerDisableAction.TIME_CIGARETTE,
	GIVEN_FULTON = PlayerDisableAction.CQC_KNIFE_KILL
		+ PlayerDisableAction.CQC_INTERROGATE
		+ PlayerDisableAction.KILLING_WEAPON
		+ PlayerDisableAction.TIME_CIGARETTE,

	CLEAR_DEVELOP_TUTORIAL = PlayerDisableAction.CQC_KNIFE_KILL
		+ PlayerDisableAction.KILLING_WEAPON
		+ PlayerDisableAction.CQC_INTERROGATE
		+ PlayerDisableAction.TIME_CIGARETTE,
	CLEAR_HANG_TUTORIAL = PlayerDisableAction.CQC_KNIFE_KILL
		+ PlayerDisableAction.KILLING_WEAPON
		+ PlayerDisableAction.TIME_CIGARETTE,
}
...
function this.OnRestoreSVars()
	...
	if svars.restartSeq == RESTART_SEQUENCE.ARRIVE_AT_MB then
		Fox.Log(
			"*** RESTART FROM ::: RESTART_SEQUENCE.ARRIVE_AT_MB *** svars.restartSeq = " .. svars.restartSeq .. "!!"
		)
	...
	elseif svars.restartSeq == RESTART_SEQUENCE.GIVEN_FULTON then
		Fox.Log(
			"*** RESTART FROM ::: RESTART_SEQUENCE.GIVEN_FULTON *** svars.restartSeq = " .. svars.restartSeq .. "!!"
		)
	...
	elseif svars.restartSeq == RESTART_SEQUENCE.CLEAR_DEVELOP_TUTORIAL then
		Fox.Log(
			"*** RESTART FROM ::: RESTART_SEQUENCE.CLEAR_DEVELOP_TUTORIAL *** svars.restartSeq = "
				.. svars.restartSeq
				.. "!!"
		)
...
function this.SetLandingZnoeDoor()
	...
	GameObject.SendCommand(
		gameObjectId,
		{ id = "SetLandingZnoeDoorFlag", name = "lz_commfacility_0000", leftDoor = "Open", rightDoor = "Close" }
	)
	GameObject.SendCommand(
		gameObjectId,
		{ id = "SetLandingZnoeDoorFlag", name = "lz_commfacility_0002", leftDoor = "Open", rightDoor = "Close" }
	)
end
...
this.EnterHeliportAfterCallHeli = function()
	...
	if mvars.isCallHeliForMissionClear == true and mvars.isHeliLanding == true then
...

			{
				...
				func = function(gameObjectId)
					...
					TppMission.ReserveGameOver(
						TppDefine.GAME_OVER_TYPE.PLAYER_KILL_TARGET,
						TppDefine.GAME_OVER_RADIO.S10030_TARGET_DEAD
					)
				end,
			},
...
function this.ResetInteruptTalk(speechLabel, isSuccess)
	if
		(speechLabel == StrCode32("MBTS_010") and isSuccess ~= 0)
		...
		or (speechLabel == StrCode32("MBTS_077") and isSuccess ~= 0)
	then
	elseif
		(speechLabel == StrCode32("MBTS_080") and isSuccess ~= 0)
		...
		or (speechLabel == StrCode32("MBTS_089") and isSuccess ~= 0)
	then
		mvars.isInterruptTalk = false
	elseif
		(speechLabel == StrCode32("MBTS_100") and isSuccess ~= 0)
		or (speechLabel == StrCode32("MBTS_101") and isSuccess ~= 0)
	then
	elseif
		(speechLabel == StrCode32("MBTS_110") and isSuccess ~= 0)
		or (speechLabel == StrCode32("MBTS_111") and isSuccess ~= 0)
	then
		mvars.isInterruptTalk = false
	elseif
		(speechLabel == StrCode32("MBTS_112") and isSuccess ~= 0)
		...
		or (speechLabel == StrCode32("MBTS_117") and isSuccess ~= 0)
	then
		mvars.isInterruptTalk = false
		mvars.isOcelotDamage = false
	elseif
		(speechLabel == StrCode32("MBTS_120") and isSuccess ~= 0)
		...
		or (speechLabel == StrCode32("MBTS_411") and isSuccess ~= 0)
	then
		mvars.isInterruptTalk = false
	elseif
		(speechLabel == StrCode32("MBTS_420") and isSuccess ~= 0)
		...
		or (speechLabel == StrCode32("MBTS_424") and isSuccess ~= 0)
	then
		mvars.isSleepTalk = false
		mvars.isInterruptTalk = false
	elseif
		(speechLabel == StrCode32("MBTS_430") and isSuccess ~= 0)
		...
		or (speechLabel == StrCode32("MBTS_510") and isSuccess ~= 0)
	then
		mvars.isInterruptTalk = false
	end
end
...
sequences.Seq_Demo_ArriveAtDD = {
	Messages = function(self)
		return StrCode32Table({
			Demo = {
				...
				{
					msg = "visibleOnNPC",
					sender = s10030_demo.demoList.ArriveAtDD,
					func = function()
	...
	OnEnter = function()
		...
		GameObject.SendCommand(
			gameObjectId,
			{ id = "SetDemoToAfterDropEnabled", enabled = true, route = "rt_heli_takeoff_opening", isTakeOff = true }
		)
		...
		return StrCode32Table({
			...
			GameObject = {
						...
						if TppEnemy.GetLifeStatus(GetGameObjectId) == TppEnemy.LIFE_STATUS.SLEEP then
							if mvars.isCarried == false then
								if
									...
									and mvars.isStopFultonTalk == false
									and svars.isFultonFirstSoldier == false
								then
...

				{
					...
					func = function(gameObjectId, switch, releaseAction)
						if switch == 0 then
							if mvars.isStopFultonTalk == false and svars.isFultonFirstSoldier == false then
...

				{
					...
					func = function(gameObjectId, speechLabel, isSuccess)
						...
						elseif
							(speechLabel == StrCode32("MBTS_042") and isSuccess ~= 0)
							or (speechLabel == StrCode32("MBTS_280") and isSuccess ~= 0)
						then
						...
						elseif
							(speechLabel == StrCode32("MBTS_030") and isSuccess ~= 0)
							or (speechLabel == StrCode32("MBTS_045") and isSuccess ~= 0)
							or (speechLabel == StrCode32("MBTS_160") and isSuccess ~= 0)
						then
							mvars.isStopFultonTalk = false
					...
					func = function(gameObjectId, speechLabel, isSuccess)
						...
						if speechLabel == StrCode32("MBTS_060") and isSuccess ~= 0 then
							...
							if developSectionLv < DEVELOP_LV_BOX then
								...
								Fox.Log(
									"#### can develop box BLOCK_CANCEL_DEVELOP_DIALOG #### Develop Lv "
										.. developSectionLv
								)
		...
		return StrCode32Table({
			...
			Timer = {
				{
					...
					func = function()
						if
							mvars.ocerotTalkProcess == this.OCELOT_TALK_PROCESS.TALKING_1
							or mvars.ocerotTalkProcess == this.OCELOT_TALK_PROCESS.WAITING_5
						then
							s10030_radio.WaitingApproach()
						else
							s10030_radio.TalkingToApproach()
...

				{
					...
					func = function()
						if svars.isFultonSecondSoldier == false then
							if
								mvars.ocerotTalkProcess >= this.OCELOT_TALK_PROCESS.WAITING_3
								and mvars.ocerotTalkProcess <= this.OCELOT_TALK_PROCESS.WAITING_4
							then
			...
			GameObject = {
				...
				},
				{
					...
					sender = s10030_enemy.ALL_SOLDIER_GROUP,
					func = function(gameObjectId, attackId)
						mvars.actionOnCQC10 = this.IsWrongAction(
							attackId,
							{ TppDamage.ATK_CqcThrow, TppDamage.ATK_CqcThrowWall, TppDamage.ATK_CqcThrowBehind }
						)
		...
		if svars.FulltonCount < 3 then
			...
			if s10030_enemy.SetTutorialRoute(ddSoldiersTable.TUTORIAL_CQC_10.ROUTE_FIGHT) == false then
				TppEnemy.SetSneakRoute(
					ddSoldiersTable.TUTORIAL_CQC_10.NAME,
					ddSoldiersTable.TUTORIAL_CQC_10.ROUTE_FIGHT
				)
			end
			s10030_enemy.SetSaluteCQCAll()
		...
		if mvars.isSpeechedOcelot_CQC_THOROW == false then
			if mvars.ocerotTalkProcess >= this.OCELOT_TALK_PROCESS.WAITING_3 then
				for index, enemyName in pairs(s10030_enemy.TUTORIAL_SOLDIER_GROUP) do
					if
						(this.GetDistanceFromPlayer(enemyName, "TppSoldier2") < DISTANCE_APPROACH_DD_SOLDIER)
						and (TppEnemy.IsNeutralized(enemyName) ~= true)
					then
...
sequences.Seq_Game_TutorialCQC15 = {
	...
	OnUpdate = function()
		if mvars.isSpeechedOcelot_CQC_THOROW15 == true then
			...
			for index, enemyName in pairs(s10030_enemy.TUTORIAL_SOLDIER_GROUP) do
				if
					(this.GetDistanceFromPlayer(enemyName, "TppSoldier2") < DISTANCE_APPROACH_DD_SOLDIER_LONG)
					and (TppEnemy.IsNeutralized(enemyName) ~= true)
				then
					i = i + 1
				end
		...
		return StrCode32Table({
			...
			GameObject = {
				...
				},
				{
					...
					func = function(gameObjectId, attackId)
						mvars.actionOnCQC20 =
							this.IsWrongAction(attackId, { TppDamage.ATK_CqcChoke, TppDamage.ATK_CqcHang })
						if attackId == TppDamage.ATK_SupplyCBoxHit then
							Fox.Log("## dd soldier box hit reaction ##")
							s10030_radio.SoldierBoxHit()
						end
			...
			Timer = {
				{
					...
					func = function()
						if
							mvars.ocerotTalkProcess == this.OCELOT_TALK_PROCESS.TALKING_1
							or mvars.ocerotTalkProcess == this.OCELOT_TALK_PROCESS.WAITING_7
						then
							s10030_radio.WaitingApproach()
						else
							s10030_radio.TalkingToApproach()
				...
				},
				{
					...
					func = function(gameObjectId, attackId)
						...
						elseif
							...
							or (attackId == TppDamage.ATK_DownKick)
						then
						...
						elseif
							(attackId == TppDamage.ATK_CqcThrow)
							...
							or (attackId == TppDamage.ATK_CqcDashPunch)
						then
							Fox.Log("## wrong faint ##")
...
function this.DirectAddStaff(SoldierID)
	...
	for index, enemyName in pairs(SOLDIER_GROUP) do
		if
			GameObject.GetGameObjectId("TppSoldier2", SoldierID)
		...
		then
			...
			local DDStaffId = TppMotherBaseManagement.GenerateStaffParameter({
				staffType = "Unique",
				uniqueTypeId = SOLDIER_STAFF_ID_GROUP[index],
			})
...
function this.DirectAddStaffContinue()
	...
	local defaultDDStaffId050 =
		TppMotherBaseManagement.GenerateStaffParameter({ staffType = "Unique", uniqueTypeId = 0 })
	TppMotherBaseManagement.DirectAddStaff({ staffId = defaultDDStaffId050, section = "Support" })

	local defaultDDStaffId060 =
		TppMotherBaseManagement.GenerateStaffParameter({ staffType = "Unique", uniqueTypeId = 1 })
	TppMotherBaseManagement.DirectAddStaff({ staffId = defaultDDStaffId060, section = "Support" })

	local defaultDDStaffId070 =
		TppMotherBaseManagement.GenerateStaffParameter({ staffType = "Unique", uniqueTypeId = 2 })
			...
			Timer = {

				{
					...
					func = function()
						if
							mvars.ocerotTalkProcess == this.OCELOT_TALK_PROCESS.TALKING_1
							or mvars.ocerotTalkProcess == this.OCELOT_TALK_PROCESS.WAITING_3
						then
							s10030_radio.WaitingApproach()
						elseif mvars.ocerotTalkProcess == this.OCELOT_TALK_PROCESS.WAITING_2 then
							s10030_radio.TalkingToApproach()
			...
			GameObject = {
...

				{
					...
					func = function(gameObjectId, attackId)
						...
						elseif (attackId == TppDamage.ATK_CqcChoke) or (attackId == TppDamage.ATK_CqcDashPunch) then
							Fox.Log("## wrong fauin ##")
						...
						elseif
							(attackId == TppDamage.ATK_CqcContinuous2nd)
							or (attackId == TppDamage.ATK_CqcContinuousOver3Times)
						then
			...
			GameObject = {
				{
					...
					func = function(gameObjectId, attackId)
						...
						elseif attackId == TppDamage.ATK_SupplyCBoxHit then
							Fox.Log("## dd soldier box hit reaction ##")
							s10030_radio.SoldierBoxHit()
						elseif (attackId == TppDamage.ATK_Push) or (attackId == TppDamage.ATK_PushSlide) then
						else
						end
					end,
				},
