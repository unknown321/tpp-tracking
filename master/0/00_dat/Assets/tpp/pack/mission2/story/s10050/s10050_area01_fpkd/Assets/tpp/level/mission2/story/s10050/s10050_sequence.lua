...
this.missionObjectiveDefine = {
	...
	default_subGoal_missionStart = {
		subGoalId = 0,
		gameObjectName = MARKER_NAME.ESCAPE_GOAL,
		goalType = "moving",
		viewType = "map",
		visibleArea = 1,
		randomRange = 0,
		...
		if svars.restoreState == StrCode32("Dying") or svars.restoreState == StrCode32("Dead") then
			Fox.Log(
				"*** "
					...
					.. " OnRestoreSVars : Execute Restore Quiet State! [ State = "
					.. tostring(svars.restoreState)
					.. " , sequence = "
					.. tostring(restoreSeq)
					.. " ] ***"
			)
			...
			Fox.Log(
				"*** "
					.. tostring(missionName)
					.. " OnRestoreSVars : Not Execute Restore Quiet State! [ State = "
					.. tostring(svars.restoreState)
					.. " , sequence = "
					.. tostring(restoreSeq)
					.. " ] ***"
...
function this.OnOutOfHotZoneMissionClear()
	Fox.Log(
		"##### s10050_sequence.OnOutOfHotZoneMissionClear ##### svars.isQuietCarried = "
			.. tostring(svars.isQuietCarried)
	)

	if svars.isQuietCarried then
		svars.isQuietRideHeli = true
		Fox.Log(
			"##### s10050_sequence.OnOutOfHotZoneMissionClear ##### Execute Flag Control! [ isQuietRideHeli = "
				.. tostring(svars.isQuietRideHeli)
				.. " ]"
		)
	else
...
function this.OnOutOfMissionArea()
	Fox.Log(
		"##### s10050_sequence.OnOutOfMissionArea ##### isQuietDown = "
			.. tostring(svars.isQuietDown)
			.. ", isKillMode = "
			.. tostring(svars.isKillMode)
	)
	...
	if TppPackList.IsMissionPackLabel("MotherBaseDemo") then
		...
		return StrCode32Table({
			...
			Demo = {
				...
				{
					msg = "SetQuietKillGame",
					func = this.SetQuietKillGameOnPlayingDemo,
			...
			Timer = {
				{
					msg = "Finish",
					sender = "timer_execDemoPrepareFunction",
					func = function()
						this.FadeOutForDemoPlay()
					end,
				},
				{
					msg = "Finish",
					sender = "timer_startDemo",
					func = function()
			...
			Subtitles = {
				{
					...
					func = function(speechLabel, status)
						...
						if speechLabel == checkSubtitles[1] then
							Fox.Log(
								"#### s10050_sequence.Messages #### SubtitlesEndEventMessage::" .. tostring(speechLabel)
							)
...
this.EnablePadMaskKillQuietGame = function(enabled)
	if enabled then
		...
		Player.SetPadMask({
			...
			buttons = PlayerPad.FIRE + PlayerPad.STOCK,
			sticks = PlayerPad.STICK_R + PlayerPad.TRIGGER_R,
		})
	else
...
this.SetQuietKillGameOnPlayingDemo = function()
	...
	GameObject.SendCommand(
		...
		{ id = "SetSpecialAttackMode", enabled = true, type = "KillQuiet" }
	)
...
this.QuietRideHeli = function(gameObjectId, vehicleId)
	if vehicleId == GameObject.GetGameObjectId("TppHeli2", SUPPORT_HELI) then
		...
		Fox.Log(
			"#### s10050_sequence.QuietRideHeli ### Not Quiet or Not Heli!! gameObjectId = "
				.. gameObjectId
				.. ", vehicleId = "
				.. vehicleId
		)
...
this.QuietCarried = function(gameObjectId, carriedState)
	Fox.Log(
		"#### s10050_sequence.QuietCarried ### gameObjectId = "
			.. tostring(gameObjectId)
			.. ", carriedState = "
			.. tostring(carriedState)
	)
...
this.QuietDead = function(deadId, attackerId)
	Fox.Log(
		"#### s10050_sequence.QuietDead ### deadId = " .. tostring(deadId) .. ", attackerId = " .. tostring(attackerId)
	)
	...
	local func = function()
		GameObject.SendCommand(
			{ type = "TppPlayer2", index = PlayerInfo.GetLocalPlayerIndex() },
			{ id = "SetSpecialAttackMode", enabled = false, type = "KillQuiet" }
		)
...
this.PlayerRideHeli = function()
	...
	if svars.isQuietRideHeli == true then
		Fox.Log(
			"#### s10050_sequence.PlayerRideHeli ### Heli EnablePullOut!! [ isQuietRideHeli = "
				.. tostring(svars.isQuietRideHeli)
				.. " ]"
		)
		TppMission.UpdateObjective({ objectives = { "complete_missionTask_02" } })
...
this.CloseDoorForNextSeq = function()
	if svars.isQuietRideHeli == true or svars.isQuietDead == true then
		Fox.Log(
			"#### s10050_sequence.CloseDoorForNextSeq ### [ isQuietRideHeli = "
				.. tostring(svars.isQuietRideHeli)
				.. " ], [isQuietDead = "
				.. tostring(svars.isQuietDead)
				.. " ] "
		)
	...
	if svars.isQuietReady then
		...
		if this.CheckPlayerRideSomething() then
			...
			if svars.isPlayerStayInDemoTrap then
				Fox.Log(
					"#### s10050_sequence.CheckPlayDemo_NotKillQuiet ### isPlayerStayInDemoTrap = "
						.. tostring(svars.isPlayerStayInDemoTrap)
				)
...
this.RestrictPlayerActionBeforeDemo = function()
	...
	Player.SetPadMask({
		...
		buttons = PlayerPad.STANCE + PlayerPad.EVADE,
	})
end
...
this.WarpEtceteraForDemo = function()
	...
	if not mvars.executeThisFunc then
		...
		if GameObject.SendCommand(gameObjectId, { id = "IsAlive" }) then
			GameObject.SendCommand(
				gameObjectId,
				{ id = "SetPosition", position = Vector3({ -1825.866, 350.025, -135.594 }), rotY = 90 }
			)
		end
...
this.CheckNeutralizeCauseForMissionTask = function(neutralizeCause)
	if
		neutralizeCause == NeutralizeCause.HANDGUN
		or neutralizeCause == NeutralizeCause.SUBMACHINE_GUN
		or neutralizeCause == NeutralizeCause.ASSAULT_RIFLE
		or neutralizeCause == NeutralizeCause.SHOTGUN
		or neutralizeCause == NeutralizeCause.MACHINE_GUN
		or neutralizeCause == NeutralizeCause.SNIPER_RIFLE
		or neutralizeCause == NeutralizeCause.MISSILE
		or neutralizeCause == NeutralizeCause.GRENADE
		or neutralizeCause == NeutralizeCause.MINE
		or neutralizeCause == NeutralizeCause.ROCKET_ARM
		or neutralizeCause == NeutralizeCause.ASSIST
		or neutralizeCause == NeutralizeCause.QUIET
		or neutralizeCause == NeutralizeCause.D_WALKER_GEAR
		or neutralizeCause == NeutralizeCause.VEHICLE
		or neutralizeCause == NeutralizeCause.SUPPORT_HELI
		or neutralizeCause == NeutralizeCause.NO_KILL
		or neutralizeCause == NeutralizeCause.NO_KILL_BULLET
	then
		return false
	else
		return true
...
sequences.Seq_Game_MainGame = {
	...
	Messages = function(self)
		return StrCode32Table({
			Player = {
				{
					...
					func = function(playerId, rideFlag, vehicleId)
						local getOff = 1
			...
			GameObject = {
				...
				{
					msg = "QuietStartSniping",
					func = function()
						Fox.Log(
							"#### s10050_sequence.Messages #### QuietStartSniping [ isUseDeathBullet = "
								.. tostring(svars.isUseDeathBullet)
								.. " ]"
						)
				...
				{
					msg = "QuietFinishUseDeathBullet",
					func = function()
						Fox.Log(
							"#### s10050_sequence.Messages #### FinishUseDeathBullet [ isLostPlayer = "
								.. tostring(svars.isLostPlayer)
								.. " ]"
						)
...

				{
					...
					func = function()
						Fox.Log(
							"#### s10050_sequence.Messages #### QuietLostPlayer [ isUseDeathBullet = "
								.. tostring(svars.isUseDeathBullet)
								.. " ]"
						)
...

				{
					msg = "Neutralize",
					sender = BOSS_QUIET,
					func = function(quietId, sourceId, neutralizeType, neutralizeCause)
						Fox.Log(
							"#### s10050_sequence.Messages #### Neutralize [ Source : "
								.. tostring(sourceId)
								.. " ] [ Neutralize Type : "
								.. tostring(neutralizeType)
								.. " ] [ Neutralize Cause : "
								.. tostring(neutralizeCause)
								.. " ]"
						)
			...
			Trap = {
				...
				{
					msg = "Exit",
					sender = "trap_playDemo_NotKillQuiet",
					func = function()
						svars.isPlayerStayInDemoTrap = false
					end,
				},
				...
				{
					msg = "Enter",
					sender = "trap_restrictPlayerAction",
					func = function()
				...
				{
					msg = "Exit",
					sender = "trap_checkStayStartPos",
					func = function()
						svars.isStayStartPos = false
					end,
				},
				nil,
			},
			Timer = {
				{
					msg = "Finish",
					sender = "timer_checkStayStartPos",
					func = function()
	...
	OnEnter = function()
		...
		if svars.restoreState == StrCode32("Dying") or svars.restoreState == StrCode32("Dead") then
			...
			s10050_enemy.SetQuietSnipeRoute("rts_snipe_0000", "rts_snipe_0001")
			s10050_enemy.SetQuietExtraRoute("rts_demo_0000", "rts_kill_0000", "rts_recovery_0000", "rts_kill_0000")
			s10050_enemy.SetQuietRelayRoute("rts_relay_0000")
	...
	Messages = function(self)
		return StrCode32Table({
			Timer = {
				{
					msg = "Finish",
					sender = "timer_NotKillQuiet",
					func = function()
						TppSequence.SetNextSequence("Seq_Demo_NotKillQuiet")
					end,
				},
			},
			nil,
			...
			Player = {
				{
					...
					func = function()
						if svars.isPermitFultonRadio then
							s10050_radio.DoNotFulton()
						end
					end,
				},
			},
...
sequences.Seq_Demo_RideHeliWithQuiet = {
	Messages = function(self)
		return StrCode32Table({
			Timer = {
				{
					msg = "Finish",
					sender = "timer_startSE",
					option = { isExecMissionClear = true },
					func = function()
						Fox.Log(
							"#### s10050_sequence.Seq_Demo_RideHeliWithQuiet::timer_startSE ### svars.isHeliClear = "
								.. tostring(svars.isHeliClear)
						)
	...
	OnEnter = function()
		...
		if
			TppBuddyService.CheckBuddyCommonFlag(BuddyCommonFlag.BUDDY_QUIET_LOST) and TppStory.IsMissionCleard(10260)
		then
			...
			if not TppQuest.IsCleard("mtbs_q99011") then
				if
					TppStory.GetElapsedMissionCount(TppDefine.ELAPSED_MISSION_EVENT.QUIET_VISIT_MISSION)
					== TppDefine.ELAPSED_MISSION_COUNT.INIT
				then
					...
					TppStory.StartElapsedMissionEvent(
						...
						TppDefine.INIT_ELAPSED_MISSION_COUNT.QUIET_VISIT_MISSION + 1
					)
				end
			end
		end
