...
function FobUI.UpdateEventTaskView(taskNo, isFobSneak)
	...
	FobUI._EnableMissionTask(
		taskNo,
		isFobSneak,
		isComplete,
		(FobUI.GetCurrentEventTaskDefine(taskNo, isFobSneak) or {})
	)
...
function FobUI._EnableMissionTask(taskNo, isFobSneak, isComplete, eventTaskDefine)
	...
	TppUiCommand.EnableMissionTask({
		...
		value1 = currentValue,
		value2 = eventTaskDefine.threshold,
	})
end
...
function FobUI.InitializeAscendingOrderEventTaskValue(detectType, taskNo, isFobSneak)
	...
	if currentValue == 0 then
		Fox.Log(
			"FobUI.InitializeAscendingOrderEventTaskValue : detectType = "
				.. tostring(detectType)
				.. ", initialValue = "
...
FobUI.OnChangeSVars = function(varName, key)
	...
	if (varName == "sneakEventTaskValue") and isFobSneak then
		FobUI.UpdateEventTaskView(key, isFobSneak)
	end

	if (varName == "defenceEventTaskValue") and not isFobSneak then
		FobUI.UpdateEventTaskView(key, isFobSneak)
	end

	if (varName == "tacticalActionPointClient") and not isFobSneak then
