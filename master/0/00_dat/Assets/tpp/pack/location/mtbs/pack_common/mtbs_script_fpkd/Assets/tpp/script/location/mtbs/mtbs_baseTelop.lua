...
mtbs_baseTelop.OnMessage = function(sender, messageId, arg0, arg1, arg2, arg3, strLogText)
	Tpp.DoMessage(
		mtbs_baseTelop.messageExecTable,
		...
		if missionId == 30050 or missionId == 10115 then
			if mvars.mbBaseTelop_clusterGrade > 1 then
				TppUiCommand.RegistInfoTypingText(
					"separate",
					5,
					mvars.mbBaseTelop_clusterTelopName,
					mvars.mbBaseTelop_plntTelopName
				)
				isTypingText = true
			else
...
mtbs_baseTelop._OnEnterForUpdateFobClearedPlantCount = function(playerId, plantNumber)
	...
	if (vars.fobSneakMode == FobMode.MODE_ACTUAL) and TppServerManager.FobIsSneak() and Tpp.IsLocalPlayer(playerId) then
