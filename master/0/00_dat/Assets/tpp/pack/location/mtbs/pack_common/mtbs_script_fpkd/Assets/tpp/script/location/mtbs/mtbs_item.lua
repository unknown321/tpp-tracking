...
mtbs_item.OnInitialize = function()
	if TppMission.IsHelicopterSpace(vars.missionCode) or vars.missionCode == 30150 then
...
mtbs_item.RegisterGimmick = function(clusterId)
	...
	if mvars.mbItem_funcGetAssetTable then
		local dataSet = string.format(
			"/Assets/tpp/level/location/mtbs/block_area/ly%03d/cl%02d/mtbs_ly%03d_cl%02d_item.fox2",
			layoutCode,
			clusterId,
			layoutCode,
			clusterId
		...
		if assetTable then
			if MotherBaseConstructConnector.SetVariableGimmickList then
				...
				if assetTable.nuclearContainers then
					MotherBaseConstructConnector.SetVariableGimmickList(
						"NuclearContainer",
						assetTable.nuclearContainers,
						dataSet
					)
					Fox.Log(
						"SetVariableGimmickList( NuclearContainer " .. tostring(layoutCode) .. ", " .. dataSet .. " )"
					)
				end
...
function mtbs_item.SwitchingLockGoalDoor(clusterId, doorList, switch)
	local layoutCode = vars.mbLayoutCode
	local dataSetPath = string.format(
		"/Assets/tpp/level/location/mtbs/block_area/ly%03d/cl%02d/mtbs_ly%03d_cl%02d_item.fox2",
		layoutCode,
		clusterId,
		layoutCode,
	...
	if locationName == "mtbs" and missionId == 30050 then
		...
		if locationStationList then
			for clusterId, clusterName in ipairs(locationStationList) do
				...
				local stationName = string.format(
					"ly003_cl%02d_collct0000|cl%02dpl0_uq_00%d0_collct|col_stat_%s",
					clusterData,
					clusterData,
					clusterData,
					clusterName
