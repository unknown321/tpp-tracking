...
function mtbs_enemy.InitSecurityNumTableFromRank()
	for plntName, plntTable in pairs(mtbs_enemy.plntParamTable) do
		...
		if
			TppMotherBaseManagement.GetMbsSecuritySectionRank({ type = "Staff" })
			== TppMotherBaseManagementConst.SECTION_FUNC_RANK_NONE
		then
			for i = 1, 3 do
...
mtbs_enemy.InitDevelopGradeTable = function()
	...
	do
		...
		if level >= 3 then
			mvars.mbEnemy_decyEquipIdList[SHOCK_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11018)] =
				TppEquip.EQP_SWP_ShockDecoy_G02
		end
		if level >= 2 then
			mvars.mbEnemy_decyEquipIdList[SHOCK_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11017)] =
				TppEquip.EQP_SWP_ShockDecoy_G01
		end
		if level >= 1 then
			mvars.mbEnemy_decyEquipIdList[SHOCK_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11016)] =
				TppEquip.EQP_SWP_ShockDecoy
		end
	end
	do
		...
		if level >= 3 then
			mvars.mbEnemy_decyEquipIdList[ACTIVE_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11015)] =
				TppEquip.EQP_SWP_ActiveDecoy_G02
		end
		if level >= 2 then
			mvars.mbEnemy_decyEquipIdList[ACTIVE_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11014)] =
				TppEquip.EQP_SWP_ActiveDecoy_G01
		end
		if level >= 1 then
			mvars.mbEnemy_decyEquipIdList[ACTIVE_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11013)] =
				TppEquip.EQP_SWP_ActiveDecoy
		end
	end
	do
		...
		if level >= 3 then
			mvars.mbEnemy_decyEquipIdList[NORMAL_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11012)] =
				TppEquip.EQP_SWP_Decoy_G02
		end
		if level >= 2 then
			mvars.mbEnemy_decyEquipIdList[NORMAL_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11011)] =
				TppEquip.EQP_SWP_Decoy_G01
		end
		if level >= 1 then
			mvars.mbEnemy_decyEquipIdList[NORMAL_DECY_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(11010)] =
				TppEquip.EQP_SWP_Decoy
		end
	end
	...
	do
		...
		if level >= 4 then
			mvars.mbEnemy_mineEquipIdListSortByEquipGrade[DMINE_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(
				11043
			)] =
	...
	do
		...
		if level >= 3 then
			mvars.mbEnemy_mineEquipIdListSortByEquipGrade[SLEEPING_MINE_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(
				11052
			)] =
				TppEquip.EQP_SWP_SleepingGusMine_G02
		end
		if level >= 2 then
			mvars.mbEnemy_mineEquipIdListSortByEquipGrade[SLEEPING_MINE_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(
				11051
			)] =
				TppEquip.EQP_SWP_SleepingGusMine_G01
		end
		if level >= 1 then
			mvars.mbEnemy_mineEquipIdListSortByEquipGrade[SLEEPING_MINE_INDEX][TppMotherBaseManagement.GetEquipDevelopRank(
				11050
...
mtbs_enemy.SetupUAV = function(clstID)
	...
	for _, plntName in ipairs(mtbs_enemy.plntNameDefine) do
		...
		numUavInPlnt, numUavPlaced =
			mtbs_enemy._GetNumOnPlnt(mtbs_enemy._GetUavNumOnPlnt, plntName, numUavPlaced, numUavMax)
...
mtbs_enemy.SetupSecurityCamera = function()
	...
	for _, plntName in ipairs(mtbs_enemy.plntNameDefine) do
		...
		local enabled = false
		numCameraInPlnt, numCameraPlaced =
			mtbs_enemy._GetNumOnPlnt(mtbs_enemy._GetSecurityCameraNumOnPlnt, plntName, numCameraPlaced, numCameraMax)
...
mtbs_enemy.SetupSortieSoldiers = function(clusterId)
	...
	if not TppGameSequence.IsMaster() and TppEnemy.DEBUG_o50050_memoryDump then
	...
	for i, plntHash in ipairs(plantTables[cpGameObjectId]) do
		...
		if routeSets then
			...
			if DEBUG and #tmpRouteList < sortieSolderNum then
				Fox.Warning(
					"enemy route is very few. enemy on plnt: "
						.. tostring(sortieSolderNum)
						.. " route on plnt: "
						.. tostring(#tmpRouteList)
						.. " :cpName: "
						.. tostring(cpName)
				)
			end
			...
			Fox.Warning(
				"TppEnemy.GetPrioritizedRouteTable(): There is no group! cpGameObjectId:"
					.. tostring(cpGameObjectId)
					.. ", plntName:"
					.. tostring(plntName)
			)
		end
...
mtbs_enemy.OnLoad = function(clusterId, isNoUseRevenge)
	...
	mvars.rad_debugRadioLineTable["DBG_MTBS_ENEMY_FOCUS_CHECK"] =
		{ "各員、特別警戒。所定の位置を防衛せよ" }
end
...
mtbs_enemy._GetRouteNameNextDivision = function(plntName, devIndex, routeType)
	...
	local InsertFreeRouteList = function(routeSets, freeRouteList)
		for _, routeName in ipairs(routeSets) do
			if Tpp.IsTypeString(routeName) and not mvars.mbSoldier_useRouteList[StrCode32(routeName)] then
...
mtbs_enemy._GetRouteGroupName = function(cpId, routeId)
	...
	for _, plntName in pairs(mtbs_enemy.plntNameDefine) do
		...
		for i, devRouteSets in ipairs(searchRouteSets) do
			if hasSameRoute(devRouteSets.inPlnt, routeId) or hasSameRoute(devRouteSets.outPlnt, routeId) then
...
mtbs_enemy.SetStaffAbirity = function(staffId, gameObjectId)
	local abirityRank =
		mtbs_enemy.SoldierCombatAbirityRank[TppMotherBaseManagement.GetMbsStaffCombatRank({ staffId = staffId })]
					...
					if message == StrCode32("FOBRouteChange") then
						...
						if mvars.mbSoldier_PermitRouteChange == true then
							if math.random(0, 99) < mtbs_enemy.routeChangeProbability then
								...
								local nextRouteName =
									mtbs_enemy._GetRouteNameNextDivision(plntName, devIndex, routeType)
...
mtbs_enemy.OnAllocateDemoBlock = function()
	...
	if mvars.f30050demo_fovaPackList == nil then
		return
	end
...
mtbs_enemy.SetSoldierForDemo = function(clusterId, demoSoldirNameList)
	...
	for i = 1, enableLocatorNum do
		table.insert(
			mvars.mbSoldier_enableSoldierLocatorList[clusterId],
			mvars.f30050_soldierListFovaApplyPriority[clusterId][i]
		)
	end
end
