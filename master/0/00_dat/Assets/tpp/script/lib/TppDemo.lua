...
e.PLAY_REQUEST_START_FUNC = {
	...
	gameCameraInterpedToDemo = function(e)
		...
		Player.RequestToInterpCameraToDemo(e, 1, 2, Vector3(0.4, 0.6, -1), true)
		return true
	end,
	...
	playerMoveToPosition = function(n, e)
		...
		Player.RequestToMoveToPosition({
			name = "DemoStartMoveToPosition",
			position = e.position,
			direction = e.direction,
			onlyInterpPosition = true,
			timeout = 10,
		})
...
e.PLAY_REQUEST_START_CHECK_FUNC = {
	missionStateCheck = function(e)
		return true
	end,
	gameCameraInterpedToDemo = function(e)
	...
	playerMoveToPosition = function(e)
		if mvars.dem_waitingMoveToPosition then
			return false
		else
			return true
...
function e.SetDemoTransform(n, e)
	...
	if e.usePlayer == true then
		n = Vector3(vars.playerPosX, vars.playerPosY, vars.playerPosZ)
...
function e.PlayGetIntelDemo(l, d, i, n, t)
	...
	local n, a
	if t then
		n, a = "p31_010026", "p31_010026_001"
	else
		n, a = "p31_010025", "p31_010025_001"
	end
	local t = "_getInteldemo"
...
function e.OnDemoSkip(e, a)
	...
	local t =
		{ p31_010010 = true, p41_030005_000_final = true, p51_070020_000_final = true, p31_050026_000_final = true }
	if t[n] then
	...
	if not r then
		...
		if e then
			TppUI.FadeIn(
				...
				"DemoSkipFadeIn",
				mvars.dem_currentSkippedScdDemoID,
				{ exceptGameStatus = o }
			)
		end
	end
...
function e.MakeNewPlayRequestInfo(e)
	...
	local e = {
		missionStateCheck = false,
		gameCameraInterpedToDemo = i,
		demoBlockLoaded = r,
		playerModelReloaded = o,
		playerActionAllowed = a,
		playerMoveToPosition = n,
		waitTextureLoadOnDemoPlay = t,
	}
	return e
end
...
e.mtbsPriorityFuncList = {
	...
	NuclearEliminationCeremony = function()
		...
		if (a and n) and e then
			if
				vars.mbmIsNuclearDeveloping == 0
				and TppMotherBaseManagement.GetResourceUsableCount({ resource = "NuclearWeapon" }) == 0
			then
				return true
			else
	...
	LongTimeNoSee_DdogHighLikability = function()
		...
		local n = gvars.elapsedTimeSinceLastPlay >= t
		local a = TppBuddyService.CanSortieBuddyType(BuddyType.DOG)
		local t = 25 <= TppBuddyService.GetFriendlyPoint(BuddyFriendlyType.DOG)
			and TppBuddyService.GetFriendlyPoint(BuddyFriendlyType.DOG) < 75
		return ((e and n) and a) and t
	end,
	...
	AttackedFromOtherPlayer_KnowWhereFrom = function()
		if
			e.IsPlayedMBEventDemo("AttackedFromOtherPlayer_KnowWhereFrom")
			or e.IsPlayedMBEventDemo("AttackedFromOtherPlayer_UnknowWhereFrom")
		then
			return false
		end
	...
	DecisionHuey = function()
		...
		if
			TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.DECISION_HUEY)
			and TppRadio.IsPlayed("f2000_rtrg8410")
		then
			return true
		else
			return false
...
function e.UpdateNuclearAbolitionFlag()
	if gvars.f30050_needUpdateNuclearFlag then
		...
		gvars.f30050_discardNuclearCountFromLastAbolition =
			TppMotherBaseManagement.GetResourceUsableCount({ resource = "NuclearWaste" })
