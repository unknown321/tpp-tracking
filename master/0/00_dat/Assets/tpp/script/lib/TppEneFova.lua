...
local T = "/Assets/tpp/parts/chara/prs/prs5_main0_def_v00.parts"
local y = "/Assets/tpp/parts/chara/prs/prs3_main0_def_v00.parts"
local m = "/Assets/tpp/parts/chara/prs/prs6_main0_def_v00.parts"
local I = "/Assets/tpp/parts/chara/dds/dds5_main0_def_v00.parts"
local c = {
	female = {
...
a.S10030_FaceIdList = { 78, 200, 283, 30, 88, 124, 138, 169, 213, 222, 243, 264, 293, 322, 343 }
a.S10030_useBalaclavaNum = 3
a.S10240_FemaleFaceIdList = { 394, 351, 373, 456, 463, 455, 511, 502 }
a.S10240_MaleFaceIdList = {
	195,
	144,
	214,
	6,
...
local function l(e)
	function e:case(e, n)
		local a = self[e] or self.default
		if a then
			a(e, n)
		end
	end
...
function a.CanUseArmorType(n, d)
	local e =
...
function a.SetHostageFaceTable(l)
	local s = a.GetHostageCountAtMissionId(l)
	...
	if s > 0 then
		...
		local r = a.GetHostageIsFaceModelOverlap(l)
		local T = a.GetHostageIgnoreFaceList(l)
		local _ = a.GetHostageFaceModelCount(l)
...
n[10200] = function(t, d)
	...
	TppHostage2.SetDefaultBodyFovaId({ parts = T, bodyId = TppEnemyBodyId.prs5_main0_v00 })
end
n[11200] = n[10200]
n[10120] = function(t, d)
	a.SetHostageFaceTable(d)
	local e = {
		{ TppEnemyBodyId.chd0_v00, e },
		{ TppEnemyBodyId.chd0_v01, e },
...
n[10091] = function(e, a)
	...
	local d = e[t]
	local e = e[a]
	local a = { { TppEnemyFaceId.pfs_balaclava, 2, 2, 0 }, { d, 1, 1, 0 }, { e, 1, 1, 0 } }
	TppSoldierFace.OverwriteMissionFovaData({ face = a, additionalMode = true })
	local t = 265
	local a = 266
	TppSoldierFace.SetSpecialFovaId({ face = { d, e }, body = { t, a } })
	local e = { { t, 1 }, { a, 1 } }
	TppSoldierFace.OverwriteMissionFovaData({ body = e, additionalMode = true })
end
n[11091] = n[10091]
n[10080] = function(d, a)
	local d = l(n)
	d:case("Africa", a)
...
n[10115] = function(a, a)
	...
	TppHostage2.SetDefaultBodyFovaId({ parts = I, bodyId = TppEnemyBodyId.dds5_main0_v00 })
	TppSoldierFace.OverwriteMissionFovaData({ body = e })
end
n[11115] = n[10115]
...
n[11130] = n[10130]
n[10140] = function(a, e)
	local a = l(n)
	a:case("Africa", e)
	TppSoldierFace.SetUseZombieFova({ enabled = true })
end
n[11140] = n[10140]
n[10150] = function(a, a)
	local a = {}
	for e = 0, 9 do
		table.insert(a, e)
	end
...
n[30020] = function(a, d)
	...
	TppHostage2.SetDefaultBodyFovaId({ parts = m, bodyId = TppEnemyBodyId.prs6_main0_v00 })
end
...
function n.Africa(n, p)
	...
	local t = (n % d) * 2
	local o = TppEnemyFaceGroupId.AFRICA_GRP000_B + t
	...
	t = (n % d) * 2
	o = TppEnemyFaceGroupId.AFRICA_GRP000_W + t
	local t = a.GetFaceGroupTableAtGroupType(o)
	TppSoldierFace.OverwriteMissionFovaData({ face = t })
	if l > 0 then
		for e = 1, 2 do
			n = n + 2
...
function n.Mbqf(n, n)
	...
	table.insert(d, { 623, 1, 1, 0 })
	table.insert(d, { TppEnemyFaceId.dds_balaclava2, 10, 10, 0 })
	table.insert(d, { TppEnemyFaceId.dds_balaclava6, 2, 2, 0 })
	table.insert(d, { TppEnemyFaceId.dds_balaclava7, 2, 2, 0 })
	local e =
		{ { 146, e }, { 147, e }, { 148, e }, { 149, e }, { 150, e }, { 151, 1 }, { 152, e }, { 153, e }, { 154, e }, {
			155,
			e,
		}, {
			156,
...
function n.Mb(n, t)
	...
	if TppMission.IsFOBMission(t) then
		...
		if l == TppEnemy.FOB_DD_SUIT_SNEAKING then
			...
			table.insert(n, { TppEnemyFaceId.dds_balaclava0, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava1, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava12, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava3, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava4, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava14, e, e, 0 })
		...
		elseif l == TppEnemy.FOB_PF_SUIT_ARMOR then
			...
			table.insert(n, { TppEnemyFaceId.dds_balaclava0, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava2, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava3, e, e, 0 })
			table.insert(n, { TppEnemyFaceId.dds_balaclava5, e, e, 0 })
		end
		...
		elseif t == 30150 then
		elseif t == 30250 then
			local a = TppMotherBaseManagement.GetOutOnMotherBaseStaffs({
				sectionId = TppMotherBaseManagementConst.SECTION_SECURITY,
			})
			local e = #a
			local e = {}
		...
		elseif t == 10030 then
			...
			table.insert(d, { TppEnemyFaceId.dds_balaclava0, e, e, 0 })
			table.insert(d, { TppEnemyFaceId.dds_balaclava1, e, e, 0 })
			table.insert(d, { TppEnemyFaceId.dds_balaclava2, e, e, 0 })
		end
	end
	...
	if TppMission.IsFOBMission(t) then
		...
		if TppMotherBaseManagement.GetMbsClusterSecurityIsEquipSwimSuit() then
			local n = TppMotherBaseManagement.GetMbsClusterSecuritySwimSuitGrade()
			a = { { c.female[n], e }, { c.male[n], e } }
		end
	else
...
function a.AddTakingOverHostagePack()
	local e = {}
	for d, n in ipairs(TppEnemy.TAKING_OVER_HOSTAGE_LIST) do
		local a = d - 1
		if a >= gvars.ene_takingOverHostageCount then
			break
		end
		local a = { type = "hostage", name = n, faceId = gvars.ene_takingOverHostageFaceIds[a] }
		table.insert(e, a)
	end
...
function a.RegisterUniqueSetting(t, i, p, n)
	local e = EnemyFova.NOT_USED_FOVA_VALUE
	local a, p = a.GetFaceIdAndFlag(t, p)
	if a == nil then
		a = e
	end
	if n == nil then
	...
	table.insert(f, { name = i, faceId = a, bodyId = n, flag = p })
	do
		local i = 1
		local p = 2
		local d = 3
...
function a.AddUniqueSettingPackage(e)
	...
	TppSoldierFace.OverwriteMissionFovaData({ face = o, body = l, additionalMode = true })
	if #d > 0 then
...
function a.ApplyUniqueSetting()
	...
	for n = 0, TppDefine.ENEMY_FOVA_UNIQUE_SETTING_COUNT - 1 do
		...
		if t(d, _) ~= 0 then
			local l = vars.missionCode
			local n = t(d, y)
			local a = a.GetStaffIdForDD(l, n)
			if a ~= T then
...
function a.ApplyMTBSUniqueSetting(e, t, o, i)
	local d = 0
	...
	if TppMission.IsFOBMission(vars.missionCode) then
		if p == TppEnemy.FOB_DD_SUIT_SNEAKING then
			if
				(
					(
						TppEnemy.weaponIdTable.DD.NORMAL.SNEAKING_SUIT
						and TppEnemy.weaponIdTable.DD.NORMAL.SNEAKING_SUIT >= 3
					) and TppMotherBaseManagement.GetMbsNvgSneakingLevel
				) and TppMotherBaseManagement.GetMbsNvgSneakingLevel() > 0
			then
		...
		elseif p == TppEnemy.FOB_DD_SUIT_BTRDRS then
			if
				(
					(
						TppEnemy.weaponIdTable.DD.NORMAL.BATTLE_DRESS
						and TppEnemy.weaponIdTable.DD.NORMAL.BATTLE_DRESS >= 3
					) and TppMotherBaseManagement.GetMbsNvgBattleLevel
				) and TppMotherBaseManagement.GetMbsNvgBattleLevel() > 0
			then
				TppEnemy.AddPowerSetting(e, { "NVG" })
			end
			if l(t) == true then
...
function a.IsUseGasMaskInMBFree(e)
	local e = TppMotherBaseManagement.IsPandemicEventMode()
	local a = mvars.f30050_currentFovaClusterId ~= TppDefine.CLUSTER_DEFINE.Command
	return e and a
end
...
function a.GetUavSetting()
	...
	local f = TppMotherBaseManagement.GetMbsUavSleepingGusGrenadeLevel({})
	local d = TppMotherBaseManagement.GetMbsClusterSecuritySoldierEquipGrade({})
...
function a.GetUavCombatGradeAndEmpLevel(d, o, l, a)
	if d < 9 then
		return nil, 0
	end
	local t = { [9] = { 4, 2 }, [10] = { 5, 3 }, [11] = { 6, 4 } }
	local e, n
	if o then
		n = 2
		e = a
