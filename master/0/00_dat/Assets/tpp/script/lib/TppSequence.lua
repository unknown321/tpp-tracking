	...
	Messages = function(e)
		return Tpp.StrCode32Table({
			...
			Timer = {
				{
					msg = "Finish",
					sender = "Timer_WaitStartingGame",
					func = e.MissionGameStart,
					option = { isExecMissionPrepare = true, isExecMissionClear = true, isExecGameOver = true },
				},
				{
					msg = "Finish",
					sender = "Timer_HelicopterMoveStart",
					func = e.HelicopterMoveStart,
					option = { isExecMissionPrepare = true, isExecMissionClear = true, isExecGameOver = true },
				},
				{
					msg = "Finish",
					sender = "Timer_FadeInStartOnNoTelopHelicopter",
					func = e.FadeInStartOnGameStart,
					option = { isExecMissionPrepare = true, isExecMissionClear = true, isExecGameOver = true },
				},
			},
		})
...
function e.IsFirstLandStart()
	if
		((not mvars.seq_demoSequneceList[mvars.seq_missionStartSequence]) and not mvars.seq_isHelicopterStart)
		and (mvars.seq_missionStartSequence == (e.SYS_SEQUENCE_LENGTH + 1))
	then
		return true
	else
		return false
...
function e.Init(n)
	...
	svars.seq_sequence = e.GetSequenceIndex("Seq_Mission_Prepare")
	if n.sequence then
