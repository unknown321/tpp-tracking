...
if not TppPackList.IsMissionPackLabel("afterMissionClearMovie") then
	this.missionObjectiveDefine = {
		...
		default_area_village = {
			gameObjectName = "s10020_marker_village",
			visibleArea = 4,
		...
		getIntel_area_slopedTown = {
			gameObjectName = "s10020_marker_miller",
			visibleArea = 1,
			goalType = "moving_fix",
			viewType = "all",
			randomRange = 0,
...
if not TppPackList.IsMissionPackLabel("afterMissionClearMovie") then
	this.VARIABLE_TRAP_SETTING = {
		...
		{
			name = "trap_ApproachIntelAtComm",
			type = TppDefine.TRAP_TYPE.NORMAL,
			initialState = TppDefine.TRAP_STATE.ENABLE,
		},
		{
			name = "trap_ApproachIntelAtVillage",
			type = TppDefine.TRAP_TYPE.NORMAL,
			initialState = TppDefine.TRAP_STATE.ENABLE,
		},

		{
			name = "trap_ArriveAtVillageBuild2F",
			type = TppDefine.TRAP_TYPE.NORMAL,
			initialState = TppDefine.TRAP_STATE.ENABLE,
		},
		{
			name = "trap_ArriveAtCommBuild",
			type = TppDefine.TRAP_TYPE.NORMAL,
			initialState = TppDefine.TRAP_STATE.ENABLE,
		},

		{
			name = "trap_ApproachIntelAtVillage_OnAlert",
			type = TppDefine.TRAP_TYPE.NORMAL,
			initialState = TppDefine.TRAP_STATE.ENABLE,
		},
		{
			name = "trap_ApproachIntelAtComm_OnAlert",
			type = TppDefine.TRAP_TYPE.NORMAL,
			initialState = TppDefine.TRAP_STATE.ENABLE,
		},
	}
		...
		OnOutOfMissionArea = function()
			if TppStory.IsMissionCleard(10020) == true then
				...
				TppMission.ReserveGameOver(
					...
					TppDefine.GAME_OVER_RADIO.S10020_OUT_OF_MISSION_AREA
				)
			end
		end,
...
function this.Messages()
	...
	return StrCode32Table({
		...
		Trap = {
			{
				msg = "Enter",
				sender = "trap_CanPlayParasiteDemoArea",
				func = function()
			...
			},
			{
				...
				sender = "trap_CanPlayParasiteDemoArea",
				func = function()
					svars.isCanPlayParasiteDemo = false
				end,
			},
		},
		Throwing = {
...
this.FuncHeliIcon = function() end
		...
		if state == StrCode32("End") then
			if Tpp.IsPlayer(attakerId) == true then
				TppMission.ReserveGameOver(
					...
					TppDefine.GAME_OVER_RADIO.S10020_PLAYER_DESTROY_HELI
				)
			else
				TppMission.ReserveGameOver(
					TppDefine.GAME_OVER_TYPE.HELICOPTER_DESTROYED,
					TppDefine.GAME_OVER_RADIO.S10020_RIDING_HELI_DESTROYED
				)
			end
		end
	end
...
sequences.Seq_Demo_ArrivalInAfghanistan = {
	...
	OnEndMissionTelop = function()
		local funcs = {
			onEnd = function()
				TppSequence.SetNextSequence("Seq_Game_IDroidTutorial")
			end,
		}
...
sequences.Seq_Game_IDroidTutorial = {
			...
			Terminal = {
				{
					msg = "MbDvcActOpenTop",
					func = function()
						TppSequence.SetNextSequence("Seq_Game_IDroidTutorial_opened")
					end,
				},
			},
			Timer = {
	...
	OnEnter = function()
		...
		local command =
			{ id = "ChangeFova", bodyId = TppEnemyBodyId.oce0_main0_v01, demoBodyId = TppEnemyBodyId.oce0_main0_v00 }
		GameObject.SendCommand(gameObjectId, command)
			...
			Terminal = {
				{
					...
					func = function()
						TppSequence.SetNextSequence("Seq_Game_BinocularsTutorial")
					end,
				},
			},
			Timer = {
			...
			Player = {
				{
					...
					func = function()
						TppSequence.SetNextSequence("Seq_Game_ZoomTutorial")
					end,
				},
			},
...
sequences.Seq_Game_ZoomTutorial = {
	...
	FuncHud_Zoom = function(GameObjectId, speechLabel, isSuccess)
		...
		if
			(speechLabel == StrCode32("MRTS_007") and isSuccess ~= 0)
			or (speechLabel == StrCode32("MRTS_013") and isSuccess ~= 0)
		then
...
sequences.Seq_Game_IntelRadioTutorial = {
	...
	FuncHud_EspionageRadio = function(GameObjectId, speechLabel, isSuccess)
		...
		if
			(speechLabel == StrCode32("MRTS_014") and isSuccess ~= 0)
			or (speechLabel == StrCode32("MRTS_015") and isSuccess ~= 0)
		then
			this.FuncTutorial_AllPadMaskReset()
...
sequences.Seq_Game_CustumMarkerTutorial = {
	...
	FuncHud_CustumMarker = function(GameObjectId, speechLabel, isSuccess)
		...
		if
			(speechLabel == StrCode32("MRTS_017") and isSuccess ~= 0)
			or (speechLabel == StrCode32("MRTS_018") and isSuccess ~= 0)
		then
			this.FuncTutorial_AllPadMaskReset()
	...
	FuncCheckCustumMarker = function(markerX, markerY, markerZ)
		if markerX > 384 and markerX < 768 and markerZ > 1024 and markerZ < 1280 then
...
sequences.Seq_Game_RescueMiller = {
	Messages = function(self)
		...
		return StrCode32Table({
			...
			Trap = {
				...
				{
					msg = "Enter",
					sender = "trap_ApproachIntelAtVillage_OnAlert",
					func = self.FuncApproachIntelAtVillage,
				},
				...
				{
					msg = "Enter",
					sender = "trap_ArriveAtVillage_EP2",
					func = function()
						svars.isInVillageEp = true
					end,
				},
				{
					msg = "Exit",
					sender = "trap_ArriveAtVillage_EP2",
					func = function()
						svars.isInVillageEp = false
					end,
				},
				{
					msg = "Enter",
					sender = "trap_ArriveFarEP",
					func = function()
						svars.isInFarEp = true
					end,
				},
				{
					msg = "Exit",
				...
				{
					msg = "Exit",
					sender = "trap_ArriveAtFirstOB",
					func = function()
						svars.isInFirstOB = false
					end,
				},
	...
	OnEnter = function()
		...
		TppRadioCommand.SetEnableEspionageRadioTarget({
			name = { "erl_village_canal", "erl_village_IntelHouse", "erl_flag", "erl_Center" },
			enable = true,
		})
		...
		elseif ContinueCount == 1 or ContinueCount == 2 or ContinueCount >= 6 then
	...
	FuncRescueMiller = function(timing, gameObjectId)
		...
		if gameObjectId == millerId then
			if svars.DayCount >= 4 then
				TppMission.ReserveGameOver(
					TppDefine.GAME_OVER_TYPE.S10020_TARGET_TIMEOVERDEAD,
					TppDefine.GAME_OVER_RADIO.S10020_TARGET_TIMEOVERDEAD
				)
			else
	...
	FuncPutCustumMarker_Hud = function() end,
	...
	FuncLeaveFromVillage = function()
		if svars.isTutorial_CallHorseEnd == false then
			if svars.isArriveAtVillageBuild2F == true then
				if Player.GetGameObjectIdIsRiddenToLocal() == 65535 and Player.IsOnTheLoadingPlatform() == false then
	...
	Messages = function(self)
		...
		return StrCode32Table({
			Player = {
				...
				{
					msg = "PlayerCallEnd_PLM0020_005",
					func = function()
						TimerStart("timer_MillerMonologue3", 1)
					end,
				},
			},
			GameObject = {
			...
			Trap = {
				...
				{
					msg = "Exit",
					sender = "trap_ParasiteBattleStart",
					func = function()
						svars.isInFirstLZonHeli = false
					end,
				},
...
sequences.Seq_Game_EscapeFromParasites = {
	Messages = function(self)
		...
		return StrCode32Table({
			...
			Timer = {
				...
				{
					msg = "Finish",
					sender = "checkMirrorStatus_afterParaDemo",
					func = function()
						if this.FuncCheckStatus_MillerCarried() then
						else
							if svars.isStartedCombat == true then
								...
								if not this.FuncCheckisInRange_Miller(checkRange) then
									...
									GameObject.SendCommand(
										{ type = "TppParasite2" },
										{ id = "StartTargetKill", targetId = targetId }
									)
								end
							end
						end
	...
	OnEnter = function()
		...
		if TppStory.IsMissionCleard(10020) == true then
			...
			vars.playerDisableActionFlag = PlayerDisableAction.REFLEXMODE
	...
	FuncSetDeterrent = function()
		GameObject.SendCommand(
			GameObject.GetGameObjectId("TppParasite2", "Parasite0"),
			{ id = "SetDeterrentEnabled", enabled = "true" }
		)
		GameObject.SendCommand(
			GameObject.GetGameObjectId("TppParasite2", "Parasite1"),
			{ id = "SetDeterrentEnabled", enabled = "true" }
		)
		GameObject.SendCommand(
			GameObject.GetGameObjectId("TppParasite2", "Parasite2"),
			{ id = "SetDeterrentEnabled", enabled = "true" }
		)
		GameObject.SendCommand(
			GameObject.GetGameObjectId("TppParasite2", "Parasite3"),
			{ id = "SetDeterrentEnabled", enabled = "true" }
		)
	end,
	...
	FuncDontFightParaUnit = function(gameObjectId, Id)
		...
		if
			paraUnit0 == gameObjectId
			or paraUnit1 == gameObjectId
			or paraUnit2 == gameObjectId
			or paraUnit3 == gameObjectId
		then
		...
		TppMovie.Play({
			...
			onEnd = function()
				...
				TppMission.MissionFinalize({
					isNoFade = true,
					showLoadingTips = false,
					ignoreMtbsLoadLocationForce = ignoreMtbsLoadLocationForce,
				})
			end,
