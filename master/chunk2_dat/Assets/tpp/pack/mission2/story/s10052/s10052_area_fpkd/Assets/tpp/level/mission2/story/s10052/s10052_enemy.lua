...
this.UniqueInterStart_executioner_remnants = function(soldier2GameObjectId, cpID)
	Fox.Log(
		"#### s10052_enemy.UniqueInterStart_executioner_remnants #### soldier2gameObjectId = " .. soldier2GameObjectId
	)
	...
	if svars.isPermitMarkedHostage_1 then
		Fox.Log(
			"#### s10052_enemy.UniqueInterStart_executioner_remnants #### isPermitMarkedHostage_1 = "
				.. tostring(svars.isPermitMarkedHostage_1)
		)
		interDetail = this.voiceTable.remnants.inter.executioner[1]
	elseif svars.isPermitMarkedHostage_2 then
		Fox.Log(
			"#### s10052_enemy.UniqueInterStart_executioner_remnants #### isPermitMarkedHostage_2 = "
				.. tostring(svars.isPermitMarkedHostage_2)
		)
		interDetail = this.voiceTable.remnants.inter.executioner[2]
	elseif svars.isPermitMarkedHostage_3 then
		Fox.Log(
			"#### s10052_enemy.UniqueInterStart_executioner_remnants #### isPermitMarkedHostage_3 = "
				.. tostring(svars.isPermitMarkedHostage_3)
		)
...
this.SetRouteDrivers = function(id, routeId)
	Fox.Log(
		"#### s10052_enemy.SetRouteDrivers #### NPC[ "
			.. tostring(id)
			.. " ] sent a message from Route[ "
			.. tostring(routeId)
			.. " ] !"
	)
	...
	if not isNeutralizeDriver then
		if id ~= driverId then
			if
				routeId == StrCode32(this.routeTable.travelPlans.toTent[1].remnantsNorth)
				or routeId == StrCode32(this.routeTable.travelPlans.toTent[2].lrrp_21_24)
				or routeId == StrCode32(this.routeTable.travelPlans.toTent[3].tent)
				or routeId == StrCode32(this.routeTable.travelPlans.toTent[4].tent)
				or routeId == StrCode32(this.routeTable.tent.transportVehicle.driver[2])
			then
				Fox.Log(
					"#### s10052_enemy.SetRouteDrivers #### This NPC[ "
						.. tostring(id)
						.. " ] is not a Driver! This Route[ "
						.. tostring(routeId)
						.. " ] is only allowed to Driver(alive)!"
				)
	...
	if this.EVENT_SEQUENCE[svars.eventCount] == "REMNANTS_CONVERSATION_START" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.remnants.transportVehicle.driver.conversation[1]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "REMNANTS_MONOLOGUE_TO_HOSTAGE" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.remnants.transportVehicle.driver.conversation[2]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "REMNANTSNORTH_CONVERSATION_START" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.remnantsNorth.transportVehicle.driver.conversation[1]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "REMNANTSNORTH_CONVERSATION_RESTART_1" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.remnantsNorth.transportVehicle.driver.conversation[2]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "REMNANTSNORTH_CONVERSATION_LAST" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.remnantsNorth.transportVehicle.driver.conversation[3]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "REMNANTSNORTH_MONOLOGUE_TO_HOSTAGE" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.remnantsNorth.transportVehicle.driver.conversation[4]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "BETWEEN_A_MONOLOGUE_START" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.betweenBase_A.transportVehicle.driver.conversation[1]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "BETWEEN_A_MONOLOGUE_FRIEND" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_GUARD,
			this.routeTable.betweenBase_A.transportVehicle.guard.conversation[1]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "BETWEEN_A_MONOLOGUE_LAST" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.betweenBase_A.transportVehicle.driver.conversation[2]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "BETWEEN_A_MONOLOGUE_TO_HOSTAGE" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.betweenBase_A.transportVehicle.driver.conversation[3]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "BETWEEN_B_CONVERSATION_START" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.betweenBase_B.transportVehicle.driver.conversation[1]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "BETWEEN_B_MONOLOGUE_FRIEND" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.betweenBase_B.transportVehicle.driver.conversation[2]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "BETWEEN_B_MONOLOGUE_TO_HOSTAGE" then
		TppEnemy.SetSneakRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.betweenBase_B.transportVehicle.driver.conversation[3]
		)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "TENT_VEHICLE_ARRIVED" then
		...
		elseif svars.isSkipOnEmergency then
			TppEnemy.SetCautionRoute(
				TRANSPORT_VEHICLE_DRIVER,
				this.routeTable.tent.transportVehicle.driver.arrivedOnCaution
			)
			TppEnemy.SetCautionRoute(
				TRANSPORT_VEHICLE_GUARD,
				this.routeTable.tent.transportVehicle.guard.arrivedOnCaution
			)
	...
	elseif this.EVENT_SEQUENCE[svars.eventCount] == "TENT_MONOLOGUE_TO_HOSTAGE" then
		...
		TppEnemy.SetCautionRoute(
			TRANSPORT_VEHICLE_DRIVER,
			this.routeTable.tent.transportVehicle.driver.cautionAfterTransport
		)
...
this.SetRouteExecutionerRemnants = function()
	...
	Fox.Log(
		"#### s10052_enemy.SetRouteExecutionerRemnants #### executeRouteName_S:NOW = "
			.. tostring(svars.executeRouteName_S)
	)
	Fox.Log(
		"#### s10052_enemy.SetRouteExecutionerRemnants #### executeRouteName_C:NOW = "
			.. tostring(svars.executeRouteName_C)
	)
	...
	if
		svars.isDiscoverdEscape_1 == false
		and svars.executeRouteName_S == StrCode32(this.routeTable.remnants.executioner.killHostage.sneak[1])
	then
		Fox.Log(
			"#### s10052_enemy.SetRouteExecutionerRemnants #### KILL_ROUTE = "
				.. this.routeTable.remnants.executioner.killHostage.sneak[1]
		)
	...
	elseif
		svars.isDiscoverdEscape_2 == false
		and svars.executeRouteName_S == StrCode32(this.routeTable.remnants.executioner.killHostage.sneak[2])
	then
		Fox.Log(
			"#### s10052_enemy.SetRouteExecutionerRemnants #### KILL_ROUTE = "
				.. this.routeTable.remnants.executioner.killHostage.sneak[2]
		)
		...
		svars.executeRouteName_C = StrCode32(nextRoute_C)
	elseif
		svars.isDiscoverdEscape_3 == false
		and svars.executeRouteName_S == StrCode32(this.routeTable.remnants.executioner.killHostage.sneak[3])
	then
		Fox.Log(
			"#### s10052_enemy.SetRouteExecutionerRemnants #### KILL_ROUTE = "
				.. this.routeTable.remnants.executioner.killHostage.sneak[3]
		)
	...
	if nextRoute_S == "" then
		...
		Fox.Log(
			"#### s10052_enemy.SetRouteExecutionerRemnants #### executeRouteName_S:NEXT = " .. tostring(nextRoute_S)
		)
		Fox.Log(
			"#### s10052_enemy.SetRouteExecutionerRemnants #### executeRouteName_C:NEXT = " .. tostring(nextRoute_C)
		)
	end
end
...
this.SetRouteExecutionerRemnants_ErrorTracer = function(hostageName)
	...
	Fox.Log("#### s10052_enemy.SetRouteExecutionerRemnants #### !! hostageName = " .. hostageName)
	Fox.Log(
		"#### s10052_enemy.SetRouteExecutionerRemnants #### !! svars.isDiscoverdEscape_1 = "
			.. tostring(svars.isDiscoverdEscape_1)
	)
	Fox.Log(
		"#### s10052_enemy.SetRouteExecutionerRemnants #### !! svars.isDiscoverdEscape_2 = "
			.. tostring(svars.isDiscoverdEscape_2)
	)
	Fox.Log(
		"#### s10052_enemy.SetRouteExecutionerRemnants #### !! svars.isDiscoverdEscape_3 = "
			.. tostring(svars.isDiscoverdEscape_3)
	)
	Fox.Log(
		"#### s10052_enemy.SetRouteExecutionerRemnants #### !! executeRouteName_S:NOW = "
			.. tostring(svars.executeRouteName_S)
...
this.SetEmergencyRide = function()
	...
	if svars.eventCount > this.EVENT_SEQUENCE["TENT_TURNAROUND_HOSTAGE"] then
		Fox.Log(
			"#### s10052_enemy.SetEmergencyRide #### EVENT_SEQUENCE["
				.. this.EVENT_SEQUENCE[svars.eventCount]
				.. "] > TENT_TURNAROUND_HOSTAGE"
		)
	...
	for i, enemyName in pairs(this.soldierDefine[targetCp]) do
		...
		if lifeState == TppEnemy.LIFE_STATUS.NORMAL then
			...
			if actionState == nomalState then
				Fox.Log(
					"#### s10052_enemy.SearchNormalEnemyFrom(" .. targetCp .. ") #### " .. enemyName .. " is NORMAL!"
				)
...
this.SearchNormalEnemyFromSequence = function()
	Fox.Log(
		"#### s10052_enemy.SearchNormalEnemyFromSequence #### EVENT_SEQUENCE = "
			.. this.EVENT_SEQUENCE[svars.eventCount]
	)

	if
		svars.eventCount >= this.EVENT_SEQUENCE["BETWEEN_B_VEHICLE_START"]
		and svars.eventCount <= this.EVENT_SEQUENCE["TENT_GOTO_TORTURE_ROOM"]
	then
		return this.SearchNormalEnemyFrom("afgh_tent_cp")
	elseif
		svars.eventCount >= this.EVENT_SEQUENCE["REMNANTS_VEHICLE_START"]
		and svars.eventCount <= this.EVENT_SEQUENCE["REMNANTSNORTH_MONOLOGUE_TO_HOSTAGE"]
	then
...
this.CheckPhaseChange = function(gameObjectId, phaseName)
	...
	for i, enemyName in pairs(CHECK_PHASE_MEMBER) do
		...
		if checkEnemy == gameObjectId then
			if phaseName >= TppGameObject.PHASE_CAUTION then
				Fox.Log(
					"#### s10052_enemy.CheckPhaseChange #### Enemy ["
						.. checkEnemy
						.. "] and Phase ["
						.. phaseName
						.. "] is OK!!"
	...
	if executeFlag and not svars.isOverlapCheckPhase then
		...
		Fox.Log(
			"#### s10052_enemy.CheckPhaseChange #### Not Execute this Function. Because executeFlag = "
				.. tostring(executeFlag)
				.. ", isOverLapCheckPhase = "
				.. tostring(svars.isOverlapCheckPhase)
		)
	end
end
...
this.EnableRestrictNotice = function(enableFlag)
	...
	for i, enemyName in pairs(RESTRICT_MEMBER) do
		local gameObjectId = GameObject.GetGameObjectId(enemyName)
		Fox.Log(
			"#### s10052_enemy.SetRestrictNotice #### targetId = "
				.. gameObjectId
				.. ", enabled = "
				.. tostring(enableFlag)
		)
...
this.EnableSoldierOnLrrp_21_24 = function(enableFlag)
	...
	for i, enemyName in pairs(this.soldierDefine[targetCp]) do
		...
		if not (targetId == illigalId) then
			...
			Fox.Log(
				"#### s10052_enemy.EnableSoldierOnLrrp_21_24 #### targetId = "
					.. tostring(targetId)
					.. ", enabled = "
					.. tostring(enableFlag)
			)
...
this.SetForceFormationLine = function(enableFlag)
	...
	for i, enemyName in pairs(FORMATION_MEMBER) do
		local gameObjectId = GameObject.GetGameObjectId(enemyName)
		Fox.Log(
			"#### s10052_enemy.SetForceFormationLine #### targetId = "
				.. gameObjectId
				.. ", enabled = "
				.. tostring(enableFlag)
		)
...
this.SetFormationAround = function(target)
	...
	local command =
		{ id = "SetCommandAi", commandType = CommandAi.ESCORT, vehicle = targetVehicle, formationIndex = driverPos }
	GameObject.SendCommand(soldierId, command)

	soldierId = GameObject.GetGameObjectId("TppSoldier2", TRANSPORT_VEHICLE_GUARD)
	command =
		{ id = "SetCommandAi", commandType = CommandAi.ESCORT, vehicle = targetVehicle, formationIndex = guardPos }
	GameObject.SendCommand(soldierId, command)
...
this.JoinToActorToNearestBase = function()
	if
		(svars.eventCount >= this.EVENT_SEQUENCE["BETWEEN_A_VEHICLE_ARRIVED"])
		and (svars.eventCount < this.EVENT_SEQUENCE["BETWEEN_A_VEHICLE_START"])
	then
		s10052_enemy.SetEmergencyRide()
	elseif
		(svars.eventCount >= this.EVENT_SEQUENCE["BETWEEN_B_VEHICLE_ARRIVED"])
		and (svars.eventCount < this.EVENT_SEQUENCE["BETWEEN_B_VEHICLE_START"])
	then
...
this.UnsetTravelPlanForException = function()
	if
		(svars.eventCount >= this.EVENT_SEQUENCE["REMNANTSNORTH_VEHICLE_ARRIVED"])
		and (svars.eventCount <= this.EVENT_SEQUENCE["BETWEEN_A_VEHICLE_START"])
	then
		TppEnemy.SetSneakRoute(GUARD_VEHICLE_DRIVER, this.routeTable.remnantsNorth.guardVehicle.driver[1])
		TppEnemy.SetCautionRoute(GUARD_VEHICLE_DRIVER, this.routeTable.remnantsNorth.guardVehicle.driver[1])
...
this.SetHostageRideVehicle = function(isGetOffVehicle)
	if not svars.isTargetRescue and not svars.tempSvars_010 then
		...
		Fox.Log(
			"#### s10052_enemy.SetHostageRideVehicle #### This function does not execute. Because svars.isTargetRescue = "
				.. tostring(svars.isTargetRescue)
		)
		return
	end
...
this.CallConversation = function(targetA, targetB, targetLabel)
	Fox.Log(
		"#### s10052_enemy.CallConversation #### targetA = "
			.. tostring(targetA)
			.. ", targetB = "
			.. tostring(targetB)
			.. ", targetLabel = "
...
this.CallMonologue = function(speakerId, targetLabel)
	Fox.Log(
		"#### s10052_enemy.CallMonologue #### speakerId = "
			.. tostring(speakerId)
			.. ", targetLabel = "
			.. tostring(targetLabel)
	)
...
this.StartConversation = function()
	Fox.Log(
		"#### s10052_enemy.StartConversation #### EVENT_SEQUENCE= " .. tostring(this.TALK_SEQUENCE[svars.talkCount])
	)
	...
	elseif this.TALK_SEQUENCE[svars.talkCount] == "REMNANTSNORTH_CONVERSATION_ABOUT_MALAK" then
		this.CallConversation(
			conversationEnemy,
			TRANSPORT_VEHICLE_DRIVER,
			this.voiceTable.remnantsNorth.conversation[1]
		)
	...
	elseif this.TALK_SEQUENCE[svars.talkCount] == "BETWEEN_B_MONOLOGUE_TO_MALAK" then
		this.CallConversation(
			TRANSPORT_VEHICLE_DRIVER,
			TRANSPORT_VEHICLE_DRIVER,
			this.voiceTable.betweenBase_B.monologue[2]
		)
...
this.SearchLabelFromConversationEndMessage = function(targetLabel)
	...
	for i, searchLabel in pairs(LABEL_TABLE) do
		if targetLabel == Fox.StrCode32(searchLabel) then
			Fox.Log(
				"#### s10052_enemy.SearchLabelFromConversationEndMessage #### targetLabel ["
					.. tostring(targetLabel)
					.. "] is event label!"
			)
			return true
...
this.DisableOccasionalChatFor = function(targetEnemy, disableFlag)
	...
	if not (gameObjectId == illigalId) then
		...
		GameObject.SendCommand(gameObjectId, command)
		Fox.Log(
			"#### s10052_enemy.DisableOccasionalChatFor #### targetEnemy = "
				.. tostring(targetEnemy)
				.. ", disableFlag = "
				.. tostring(disableFlag)
		)
	end
end
...
this.DisableOccasionalChatForEnemyFrom = function(targetCp, disableFlag)
	if this.soldierDefine[targetCp] == nil then
		Fox.Log(
			"#### s10052_enemy.DisableOccasionalChatForEnemyFrom(" .. tostring(targetCp) .. ") #### targetCp is nil..."
		)
		return
	end
	...
	Fox.Log(
		"#### s10052_enemy.DisableOccasionalChatForEnemyFrom("
			.. tostring(targetCp)
			.. ") #### disableFlag = "
			.. tostring(disableFlag)
	)
...
this.SetRelativeVehicle = function(soldierId, vehicleId)
	Fox.Log(
		"#### s10052_enemy.SetRelativeVehicle #### soldierId = "
			.. tostring(soldierId)
			.. ", vehicleId = "
			.. tostring(vehicleId)
	)
...
this.DeleteConvoySetting = function()
	...
	GameObject.SendCommand(
		{ type = "TppVehicle2" },
		{ id = "UnregisterConvoy", convoyId = GameObject.GetGameObjectId("TppVehicle2", TRANSPORT_VEHICLE) }
	)
end
...
this.SetTravelPlanForEventVehicle = function(soldierId, planName)
	...
	Fox.Log(
		"#### s10052_enemy.SetTravelPlanForEventVehicle #### soldierId = "
			.. soldierId
			.. ", travelPlan = "
			.. planName
			.. ", keepInAlert = "
...
this.CheckFixedPointCombatForEventVehicle = function() end
...
this.EnableForceRailDriveSpeedFAST = function(enableFlag)
	...
	for i, vehicleName in pairs(TARGET_VEHICLES) do
		...
		if this.CheckAliveVehicle(vehicleName) then
			Fox.Log(
				"#### s10052_enemy.EnableForceRailDriveSpeedFAST #### vehicleId = "
					.. gameObjectId
					.. ", enabled = "
					.. tostring(enableFlag)
			)
...
this.CheckRideOnAnyEnemy = function(target)
	...
	for seatIndex, riderId in ipairs(riderIdArray) do
		Fox.Log(
			"#### s10052_enemy.CheckRideOnAnyEnemy #### target ["
				.. target
				.. "], seatIndex ["
				.. seatIndex
				.. "] "
...
this.SetEnableForAnimal = function(enableFlag)
	...
	if doesExist then
		Fox.Log(
			"#### s10052_enemy.SetEnableForAnimal #### targetAnimal = "
				.. targetAnimal
				.. ", enabled = "
				.. tostring(enableFlag)
		)
...
this.EnableNoticeForAnimal = function(enableFlag)
	...
	if doesExist then
		Fox.Log(
			"#### s10052_enemy.EnableNoticeForAnimal #### targetAnimal = "
				.. targetAnimal
				.. ", enabled = "
				.. tostring(enableFlag)
		)
		...
		Fox.Log(
			"#### s10052_enemy.EnableNoticeForAnimal #### This function not execute! Because TppGoat does not exist."
		)
	end
end
...
this.SetEventRouteForAnimal = function(setRoute)
	...
	if doesExist then
		Fox.Log(
			"#### s10052_enemy.SetEventRouteForAnimal #### targetAnimal = "
				.. targetAnimal
				.. ", setRoute = "
				.. setRoute
		)
		...
		Fox.Log(
			"#### s10052_enemy.SetEventRouteForAnimal #### This function not execute! Because TppGoat does not exist."
		)
	end
end
...
this.EnableEscapeAttributeForAnimal = function(enableFlag)
	...
	if doesExist then
		Fox.Log(
			"#### s10052_enemy.EnableEscapeAttributeForAnimal #### targetAnimal = "
				.. targetAnimal
				.. ", enableFlag = "
				.. tostring(enableFlag)
		)
		...
		Fox.Log(
			"#### s10052_enemy.EnableEscapeAttributeForAnimal #### This function not execute! Because TppGoat does not exist."
		)
	end
end
...
this.SetUniqueStaffParam = function(targetName, uniqueTypeId, existParamTable)
	Fox.Log(
		"#### s10052_enemy.SetUniqueStaffParam #### targetName = " .. targetName .. ", uniqueTypeId = " .. uniqueTypeId
	)
	TppEnemy.AssignUniqueStaffType({
		locaterName = targetName,
		uniqueStaffTypeId = uniqueTypeId,
		alreadyExistParam = existParamTable,
	})
end
