...
this.SetUpSahelan = function()
	...
	if mvars.isNormal then
		...
		local command = {
			id = "SetCombatGrade",
			defenseValue = 60000,
			defenseValueForWeakPoint = 20000,
			offenseGrade = 6,
			defenseGrade = 2,
		}
		GameObject.SendCommand(gameObjectId, command)
...
this.SetUpDebugSahelanParam = function() end
...
this.StartHeliAntiSahelan = function()
	...
	GameObject.SendCommand(
		gameObjectId,
		{ id = "StartAntiSahelan", startPosition = Vector3(-363, 573, -1178), pullOutPosition = Vector3(
			-363,
			573,
			-1178
		) }
