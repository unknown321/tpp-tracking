...
this.missionObjectiveDefine = {
	...
	default_area_sovietBase01 = {
		gameObjectName = "Marker_default_area_sovietBase",
		visibleArea = 6,
		randomRange = 0,
		viewType = "map_only_icon",
		setNew = false,
		announceLog = "updateMap",
		mapRadioName = "s0070_mprg0020",
		targetBgmCp = "afgh_sovietBase_cp",
	...
	detail_area_huey = {
		gameObjectName = "Marker_detail_area_huey",
		visibleArea = 2,
		randomRange = 0,
		viewType = "all",
		announceLog = "updateMap",
		mapRadioName = "s0070_mprg0030",
		langId = "marker_info_mission_targetArea",
	},

	target_huey = {
		gameObjectName = "Marker_huey",
		goalType = "defend",
		viewType = "all",
		setNew = true,
		setImportant = true,
		announceLog = "updateMap",
		langId = "marker_chara_huey",
	},

	area_sp_walkergear = {
		gameObjectName = "Marker_area_sp_walkergear",
		visibleArea = 0,
		goalType = "moving",
		viewType = "map_only_icon",
		setNew = false,
	...
	rv_escape = {
		gameObjectName = "Marker_rv_escape",
		visibleArea = 0,
		randomRange = 0,
		viewType = "all",
		setNew = true,
	...
	photo_target = {
		photoId = MISSION_INFO_PHOTO_ID,
		addFirst = true,
		addSecond = false,
		photoRadioName = "s0070_mirg0010",
	},
...
function this.OnEndMissionCredit()
	...
	TppScriptBlock.LoadDemoBlock("Demo_MotherBase", false)
		...
		Player = {
			{
				...
				func = function(playerGameObjectId, collectionUniqueId, collectionTypeId)
					...
					if
						collectionUniqueId
						== TppCollection.GetUniqueIdByLocatorName("col_develop_Semiauto_SR_s10070_0000")
					then
						TppMission.UpdateObjective({
		...
		Block = {
			{
				...
				func = function(blockName, blockState)
					...
					Fox.Log(
						"blockName is " .. tostring(blockName) .. "        / blockState is " .. tostring(blockState)
					)
...
this.s10070_baseOnActiveTable = {
	...
	afgh_sovietBase = function()
		...
		local function currentSeqCheck()
			local currentSequence = TppSequence.GetCurrentSequenceName()
			if
				currentSequence == "Seq_Game_GoToSovietBase"
				or currentSequence == "Seq_Demo_ContactHuey"
				or currentSequence == "Seq_Game_EscapeSovietBase"
			then
				return true
			else
				return false
		...
		local function restoreSeqCheck()
			local restoreSeq = TppSequence.GetMissionStartSequenceName()
			if
				restoreSeq == "Seq_Game_GoToSovietBase"
				or restoreSeq == "Seq_Demo_ContactHuey"
				or restoreSeq == "Seq_Game_EscapeSovietBase"
			then
				return true
			else
				return false
...
function this.SetTargetDistanceCheck()
	...
	local gameObjectId = { type = "TppHuey2", index = 0 }
	local command =
		{ id = "SetPlayerDistanceCheck", enabled = true, near = RANGE_TARGET_CHECK_NEAR, far = RANGE_TARGET_CHECK_FAR }
	GameObject.SendCommand(gameObjectId, command)
end
...
function this.SetHueyRideState()
	...
	if svars.typeHueyRideVehicle and svars.typeHueyRideVehicle ~= 0 then
		...
		GameObject.SendCommand(gameObjectId, {
			id = "RideVehicle",
			vehicleId = svars.idHueyRideVehicle,
			type = svars.typeHueyRideVehicle,
			seatIndex = svars.seatIndexHueyRideVehicle,
			fastRide = true,
		})
	end
end
...
function this.CheckHeliStatusOnLZ()
	...
	if
		aiState == Fox.StrCode32("WaitPoint")
		or aiState == Fox.StrCode32("Descent")
		or aiState == Fox.StrCode32("Landing")
	then
...
function this.CheckHeliStatusOnExit()
	local aiState = GameObject.SendCommand({ type = "TppHeli2", index = 0 }, { id = "GetAiState" })
	if aiState == Fox.StrCode32("PullOut") or aiState == Fox.StrCode32("") then
...
function this.SetLandingZnoeDoor()
	...
	GameObject.SendCommand(
		gameObjectId,
		{ id = "SetLandingZnoeDoorFlag", name = RV01, leftDoor = "Open", rightDoor = "Close" }
	)
	GameObject.SendCommand(
		gameObjectId,
		{ id = "SetLandingZnoeDoorFlag", name = RV02, leftDoor = "Open", rightDoor = "Close" }
	)
end
...
function this.SwitchAntiSahelanMode(flag) end
		...
		if svars.isDummyFlag06 then
			...
			if vehicleId ~= GameObject.NULL_ID then
				...
				if type == Vehicle.type.WESTERN_LIGHT_VEHICLE or type == Vehicle.type.EASTERN_LIGHT_VEHICLE then
					GameObject.SendCommand(
						vehicleId,
						{ id = "SetPosition", position = this.VEHICLE_POS_ATTACK_DEMO, rotY = 245 }
					)
				elseif type == Vehicle.type.WESTERN_TRUCK or type == Vehicle.type.EASTERN_TRUCK then
					GameObject.SendCommand(
						vehicleId,
						{ id = "SetPosition", position = this.TRUCK_POS_ATTACK_DEMO, rotY = 245 }
					)
				else
					GameObject.SendCommand(
						vehicleId,
...
function this.CheckBeforeContactHueyDemo()
	if
		TppScriptBlock.GetCurrentPackListName("npc_block") == "Huey"
		and TppScriptBlock.GetCurrentPackListName("demo_block") == "Demo_ContactHuey"
	then
...
function this.AddTrapSettingForRideMetalDemo(params)
	...
	Fox.Log(
		"s10070_sequence.AddTrapSettingForRideMetalDemo. trapName = "
			.. tostring(trapName)
			.. ", direction = "
			.. tostring(direction)
			.. ", directionRange = "
			.. tostring(directionRange)
	)
...
function this.SwitchSovietBaseHungerAsset_RescueHuey()
	...
	TppDataUtility.SetVisibleDataFromIdentifier(
		"sovietBase_asset_DataIdentifier",
		"ReptileHungerAsset_HueyMission",
		true,
		true
	)
	...
	TppDataUtility.SetVisibleDataFromIdentifier(
		"sovietBase_asset_DataIdentifier",
		"HueyHungerAsset_ReptileMission",
		false,
		true
	)
	...
	TppDataUtility.SetVisibleDataFromIdentifier(
		"sovietBase_asset_DataIdentifier",
		"ReptileHungerAsset_TopRoof_Before",
		true,
		true
	)

	TppDataUtility.SetVisibleDataFromIdentifier(
		"soviet_mission_asset01_DataIdentifier",
		"HueyHungerAsset_HueyMission",
		true,
		true
	)
	TppDataUtility.SetVisibleDataFromIdentifier(
	...
	TppDataUtility.SetVisibleDataFromIdentifier(
		"soviet_mission_asset02_DataIdentifier",
		"ReptileHungerAsset_ReptileMission",
		false,
		true
	)
	TppDataUtility.SetVisibleDataFromIdentifier(
	...
	TppDataUtility.SetVisibleDataFromIdentifier(
		"soviet_mission_asset02_DataIdentifier",
		"ReptileHungerAsset_TopRoof_After",
		false,
		true
	)
end
...
function this.SwitchSovietBaseHungerGimmick_RescueHuey()
	...
	Gimmick.InvisibleGimmick(
		...
		"afgh_cmpt002_gim_n0001|srt_afgh_cmpt002",
		DATASET_PATH_ASSET01,
		false
	)
	Gimmick.InvisibleGimmick(
		TppGameObject.GAME_OBJECT_TYPE_IMPORTANT_BREAKABLE,
		"afgh_cmpt002_gim_n0002|srt_afgh_cmpt002",
		DATASET_PATH_ASSET01,
		false
	)
	Gimmick.InvisibleGimmick(
		TppGameObject.GAME_OBJECT_TYPE_IMPORTANT_BREAKABLE,
		"afgh_cmpt002_gim_n0003|srt_afgh_cmpt002",
		DATASET_PATH_ASSET01,
		true
	)
	Gimmick.InvisibleGimmick(
		TppGameObject.GAME_OBJECT_TYPE_IMPORTANT_BREAKABLE,
		"afgh_cmpt002_gim_n0004|srt_afgh_cmpt002",
		DATASET_PATH_ASSET01,
		true
	)
	...
	Gimmick.InvisibleGimmick(
		TppGameObject.GAME_OBJECT_TYPE_IMPORTANT_BREAKABLE,
		"aip0_main0_gm_gim_n0000|srt_aip0_main0_gm",
		DATASET_PATH_ASSET01,
		false
	)
	Gimmick.InvisibleGimmick(
		TppGameObject.GAME_OBJECT_TYPE_IMPORTANT_BREAKABLE,
		"aip0_main0_gm_gim_n0001|srt_aip0_main0_gm",
		DATASET_PATH_ASSET01,
		true
	)
end
...
function this.SwitchSovietBaseHungerDoor(switchFlag)
	...
	if switchFlag then
		TppDataUtility.SetVisibleDataFromIdentifier(
			"sovietBase_asset_DataIdentifier",
			"HueyHungerDoor_Before",
			false,
			true
		)
		TppDataUtility.SetVisibleDataFromIdentifier(
			"sovietBase_asset_DataIdentifier",
			"HueyHungerDoor_After",
			true,
			true
		)
...
sequences.Seq_Game_GoToSovietBase = {
	...
	Messages = function(self)
		return StrCode32Table({
			Block = {
				{
					...
					func = function(blockName, blockState)
						...
						if
							TppScriptBlock.GetCurrentPackListName("npc_block") == "Huey"
							and blockState == ScriptBlock.TRANSITION_ACTIVATED
						then
						...
						if
							TppScriptBlock.GetCurrentPackListName("demo_block") == "Demo_ContactHuey"
							and blockState == ScriptBlock.TRANSITION_ACTIVATED
						then
						...
						if svars.issovietEnemyHeliStart == false then
							...
							elseif messageId == StrCode32("msg_pwHeli_rt0001_end") then
								if svars.isOpeningDemoRadio == true then
									s10070_enemy02.UpdateEnemyHeliRoute(
										"rts_ptr_e_powerPlant_0003",
										"rts_ptr_c_powerPlant_0000"
									)
									svars.ispowerPlantHeliMove = true
								else
									s10070_enemy02.UpdateEnemyHeliRoute(
										"rts_ptr_e_powerPlant_0002",
										"rts_ptr_c_powerPlant_0000"
									)
			...
			Trap = {
				...
				{
					msg = "Enter",
					sender = "trap_powerPlantEnemyHeliMove",
					func = function()
				...
				{
					msg = "Enter",
					sender = "trap_sovietEnemyHeliStart",
					func = function()
				...
				{
					msg = "Enter",
					sender = "trap_sovietEnemyHeliMove",
					func = function()
				...
				{
					msg = "Enter",
					sender = "trap_Start_ContactHueyDemo",
					func = function()
	...
	OnEnter = function()
		...
		TppInterrogation.AddHighInterrogation(GameObject.GetGameObjectId("afgh_sovietBase_cp"), {
...
sequences.Seq_Game_EscapeSovietBase = {
				...
				},
				{
					...
					func = function()
						if svars.isSahelan_Herald00 == false then
							...
							this.SetUpCommandPostSirenType(
								"afgh_sovietBase_cp",
								TppGameObject.SIREN_TYPE_POWP,
								{ -1864.860, 463.629, -1436.973 }
							)
				...
				},
				{
					...
					func = function()
						if svars.isSahelan_Herald02 == false then
							...
							this.SetUpCommandPostSirenType(
								"afgh_sovietBase_cp",
								TppGameObject.SIREN_TYPE_SOVB,
								{ -2257.874, 451.303, -1516.756 }
							)
	...
	OnEnter = function()
		...
		TppUiCommand.RegisterIconUniqueInformation({
			markerId = GameObject.GetGameObjectId(TARGET_HUEY),
			langId = "marker_chara_huey",
		})
		this.UnlockedHuey()
			...
			Demo = {
				{
					...
					func = function(targetName, gameObjectId)
						...
						GameObject.SendCommand(
							...
							{ id = "Warp", pos = { -2213.135, 443.240, -1312.413 }, rotY = 0 }
						)
					end,
					option = { isExecDemoPlaying = true },
				},
				{
					...
					func = function(targetName, gameObjectId)
						...
						GameObject.SendCommand(
							GameObject.GetGameObjectId("TppPlayer2", "Player"),
							{ id = "Warp", pos = { -2196.855, 443.828, -1252.046 }, rotY = 0 }
						)
	...
	Messages = function(self)
		return StrCode32Table({
			...
			GameObject = {
				...
				{
					msg = "SahelanVulcunStartToHeli",
					sender = "Sahelanthropus",
					func = function(arg0, arg1)
						Fox.Log("!!!!! SahelanVulcunStartToHeli !!!!!")

						if
							mvars.isSahelanAttackHeli1 == false
							and this.CheckDistancePlayerToHeli() > RANGE_HELI_TO_PLAYER_CHECK
						then
...

				{
					...
					func = function(arg0, arg1)
						...
						if sahelanPhase == TppSahelan2.SAHELAN2_PHASE_ALERT then
							...
							if
								this.CheckHeliStatusOnLZ()
								and svars.isRideEscapeHeli == false
								and mvars.isPlayerRideOnHeli == false
								and mvars.isSahelanEvasion == false
								and mvars.isPlayerOnLZ == false
							then
								this.SwitchAntiSahelanMode(true)
						...
						elseif sahelanPhase == TppSahelan2.SAHELAN2_PHASE_SNEAK then
							...
							if this.CheckHeliStatusOnExit() then
								...
								local aiState = GameObject.SendCommand(
			...
			Trap = {
				...
				},
				{
					...
					msg = "Enter",
					sender = "trap_shln_lzs_sovietBase_S_0000",
					func = function()
				...
				},
				{
					...
					msg = "Enter",
					sender = "trap_shln_lzs_sovietSouth_W_0000",
					func = function()
	...
	Messages = function(self)
		return StrCode32Table({
			Timer = {
				...
				{
					msg = "Finish",
					sender = "Timer_SahelanFallingDemo",
					func = function()
			...
			GameObject = {
...

				{
					...
					func = function(arg0, arg1)
						...
						TppMission.ReserveGameOver(
							TppDefine.GAME_OVER_TYPE.HELICOPTER_DESTROYED,
