...
function this.SetMeetingRoute_VIP(nGameObjectID)
	if svars.isMeetingAborted == true then
		...
		if
			nGameObjectID == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.FIELD_VIP)
			or nGameObjectID == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.FIELD_DRIVER)
			or nGameObjectID == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.FIELD_GUARD_00)
		then
		...
		elseif
			nGameObjectID == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.ENEMYBASE_VIP)
			or nGameObjectID == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.ENEMYBASE_DRIVER)
			or nGameObjectID == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.ENEMYBASE_GUARD_00)
		then
...
function this.EliminatedVIP(nGameObjectId, EliminateTypeId)
	...
	elseif nGameObjectId == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.VILLAGE_VIP) then
		...
		if isVillageGuardEliminated == true and svars.isMeetingAborted == false then
...
function this.SetWaitingVIP(nGameObjectId)
	...
	if sequence == "Seq_GameVipAssembleVillage" then
		...
		if isWaitingVipRemain == 0 then
			if
				TppEnemy.GetLifeStatus(this.ENEMY_NAME.FIELD_VIP) == TppEnemy.LIFE_STATUS.NORMAL
				and TppEnemy.GetLifeStatus(this.ENEMY_NAME.ENEMYBASE_VIP) == TppEnemy.LIFE_STATUS.NORMAL
				and TppEnemy.GetLifeStatus(this.ENEMY_NAME.VILLAGE_VIP) == TppEnemy.LIFE_STATUS.NORMAL
				and TppEnemy.GetStatus(this.ENEMY_NAME.FIELD_VIP) == EnemyState.NORMAL
				and TppEnemy.GetStatus(this.ENEMY_NAME.ENEMYBASE_VIP) == EnemyState.NORMAL
				and TppEnemy.GetStatus(this.ENEMY_NAME.VILLAGE_VIP) == EnemyState.NORMAL
			then
...
function this.AbortMonologue(phaseName)
	...
	if TppEnemy.GetLifeStatus(this.ENEMY_NAME.VILLAGE_VIP) == TppEnemy.LIFE_STATUS.NORMAL then
		...
		if
			TppEnemy.GetLifeStatus(this.ENEMY_NAME.FIELD_VIP) == TppEnemy.LIFE_STATUS.NORMAL
			and svars.isWaiting_fieldVip == true
		then
			...
			if
				TppEnemy.GetLifeStatus(this.ENEMY_NAME.ENEMYBASE_VIP) == TppEnemy.LIFE_STATUS.NORMAL
				and svars.isWaiting_enemyBaseVip == true
			then
...
function this.CpRadioVillageMain()
	...
	if svars.isReserve_20 == false then
		...
		local command =
			{ id = "CallRadio", label = "HQSP030", stance = "SquatCaution", voiceType = "ene_a", isForce = true }
...
function this.CpRadioFieldVip()
	...
	local command =
		{ id = "CallRadio", label = "HQSP020", stance = "SquatCaution", voiceType = "ene_d", isForce = true }
...
function this.CpRadioFieldGuard()
	...
	local command =
		{ id = "CallRadio", label = "HQSP040", stance = "SquatCaution", voiceType = "ene_c", isForce = true }
	if svars.isCancelMeetingVillageVip == false then
		GameObject.SendCommand(gameObjectId, command)
	else
...
function this.StartMeeting()
	...
	if svars.isMeetingAborted == true then
		...
		if
			(readyMeetingCount == 3)
			and TppEnemy.GetLifeStatus(this.ENEMY_NAME.FIELD_VIP) == TppEnemy.LIFE_STATUS.NORMAL
			and TppEnemy.GetLifeStatus(this.ENEMY_NAME.ENEMYBASE_VIP) == TppEnemy.LIFE_STATUS.NORMAL
			and TppEnemy.GetLifeStatus(this.ENEMY_NAME.VILLAGE_VIP) == TppEnemy.LIFE_STATUS.NORMAL
		then
			Fox.Log("#### Meeting3Vip ####")
...
function this.CancelMeeting()
	if svars.isMeetingAborted == true then
		...
		if
			TppEnemy.GetLifeStatus(this.ENEMY_NAME.FIELD_VIP) == TppEnemy.LIFE_STATUS.DEAD
			or (svars.isReserve_09 == true)
		then
			Fox.Log(
				"#### fieldVip is Already GONE, Make sure other 2 don't always sit on the car and make them travel back ####"
			)
			this.SetupStepBasedOntravelCountField()
		end

		if
			TppEnemy.GetLifeStatus(this.ENEMY_NAME.ENEMYBASE_VIP) == TppEnemy.LIFE_STATUS.DEAD
			or (svars.isReserve_10 == true)
		then
			Fox.Log(
				"#### enemyBaseVip is Already GONE, Make sure other 2 don't always sit on the car and make them travel back ####"
			)
			this.SetupStepBasedOntravelCountEnemyBase()
		end

		if
			TppEnemy.GetLifeStatus(this.ENEMY_NAME.VILLAGE_VIP) == TppEnemy.LIFE_STATUS.NORMAL
			and TppEnemy.GetStatus(this.ENEMY_NAME.VILLAGE_VIP) == EnemyState.NORMAL
			and svars.isWaiting_villageVip == true
		then
...
function this.conversationMain(speaker, listner, label)
	...
	GameObject.SendCommand(speaker, {
		id = "CallConversation",
		label = label,
		friend = listner,
	})
...
this.TransferCpStepIndividual = function(nObjectId, planName, stepId)
	...
	if
		nObjectId == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.ENEMYBASE_VIP)
		or nObjectId == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.ENEMYBASE_DRIVER)
		or nObjectId == GameObject.GetGameObjectId("TppSoldier2", this.ENEMY_NAME.ENEMYBASE_GUARD_00)
	then
		local command = { id = "StartTravel", travelPlan = planName, step = stepId }
...
function this.EnemyBaseVipBackBase()
	...
	if TppEnemy.IsEliminated(s10041_enemy.ENEMY_NAME.FIELD_VIP) == true or svars.isReturn_fieldVip == true then
...
function this.FieldVipBackBase()
	...
	if TppEnemy.IsEliminated(s10041_enemy.ENEMY_NAME.ENEMYBASE_VIP) == true or svars.isReturn_enemyBaseVip == true then
		TppRadio.SetOptionalRadio("Set_s0041_oprg0040")
	end
...
this.SetNpcStaffParameter = function(soldierName, uniqueTypeId, alreadyExistParam)
	...
	TppEnemy.AssignUniqueStaffType({
		locaterName = soldierName,
		uniqueStaffTypeId = uniqueTypeId,
		alreadyExistParam = alreadyExistParam,
	})
end
...
this.SetVipRelativeVehicleMust = function()
	...
	local enemyBaseVipcommand =
		{ id = "SetRelativeVehicle", targetId = enemyBaseVeh, rideFromBeginning = false, isMust = true }
...
this.SetRelativeVehicleVigilance = function(vehicleName)
	...
	for index, enemyName in ipairs(GRU_VEHICLE_GROUP[vehicleName]) do
		...
		local vehicleId = GameObject.GetGameObjectId("TppVehicle2", vehicleName)
		local command =
			{ id = "SetRelativeVehicle", targetId = vehicleId, rideFromBeginning = false, isVigilance = true }
		Fox.Log("### vehicleId  ###" .. tostring(vehicleName))
		Fox.Log("### soldierId  ###" .. tostring(soldierName))
...
this.SetRelativeVehicleNoVigilance = function(vehicleName)
	...
	for index, enemyName in ipairs(GRU_VEHICLE_GROUP[vehicleName]) do
		...
		local vehicleId = GameObject.GetGameObjectId("TppVehicle2", vehicleName)
		local command =
			{ id = "SetRelativeVehicle", targetId = vehicleId, rideFromBeginning = false, isVigilance = false }
		Fox.Log("### vehicleId  ###" .. tostring(vehicleName))
		Fox.Log("### soldierId  ###" .. tostring(soldierName))
...
this.CheckVipRouteMarker = function() end
...
this.SetUpEnemy = function()
	...
	if DEBUG then
		if svars.isSkipToConversation == true then
			...
			GameObject.SendCommand(
				GameObject.GetGameObjectId("TppVehicle2", "veh_s10041_VipField"),
				{ id = "SetPosition", position = Vector3(526.034, 320.619, 1190.245), rotY = 0 }
			)
			GameObject.SendCommand(
				GameObject.GetGameObjectId("TppVehicle2", "veh_s10041_VipEnemyBase"),
				{ id = "SetPosition", position = Vector3(514.948, 321.339, 1190.875), rotY = 0 }
			)
		else
			this.SetVipRoute()
		end
	...
	this.SetNpcStaffParameter(
		this.ENEMY_NAME.FIELD_VIP,
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_FIELD_COMMANDER,
		{ staffTypeId = 2, randomRangeId = 6, skill = "Grappler" }
	)
	this.SetNpcStaffParameter(
		this.ENEMY_NAME.VILLAGE_VIP,
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_VILLAGE_COMMANDER,
		{ staffTypeId = 15, randomRangeId = 6, skill = nil }
	)
	this.SetNpcStaffParameter(
		this.ENEMY_NAME.ENEMYBASE_VIP,
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_ENEMY_BASE_COMMANDER,
		{ staffTypeId = 5, randomRangeId = 6, skill = nil }
	)

	this.SetNpcStaffParameter(
		this.ENEMY_NAME.FIELD_DRIVER,
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_FIELD_DRIVER,
		{ staffTypeId = 58, randomRangeId = 4, skill = nil }
	)
	this.SetNpcStaffParameter(
		this.ENEMY_NAME.FIELD_GUARD_00,
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_FIELD_BODYGUARD,
		{ staffTypeId = 2, randomRangeId = 6, skill = nil }
	)

	this.SetNpcStaffParameter(
		this.ENEMY_NAME.ENEMYBASE_DRIVER,
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_ENEMY_BASE_DRIVER,
		{ staffTypeId = 3, randomRangeId = 6, skill = "RadarEngineer" }
	)
	this.SetNpcStaffParameter(
		this.ENEMY_NAME.ENEMYBASE_GUARD_00,
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_ENEMY_BASE_BODYGUARD,
		{ staffTypeId = 41, randomRangeId = 4, skill = nil }
	)

	this.SetNpcStaffParameter(
		"hos_subTarget_0000",
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_HOSTAGE_A,
		{ staffTypeId = 62, randomRangeId = 4, skill = nil }
	)
	this.SetNpcStaffParameter(
		"hos_subTarget_0001",
		TppDefine.UNIQUE_STAFF_TYPE_ID.S10041_HOSTAGE_B,
		{ staffTypeId = 62, randomRangeId = 4, skill = "Botanist" }
	)
