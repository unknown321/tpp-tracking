			...
			},
			{
				...
				func = function(playerId, gameObjectId)
					...
					if gameObjectId == GameObject.GetGameObjectId("TppHostage2", ESCAPE_HOSTAGE_NAME) then
						...
						local isInRange01 = GameObject.SendCommand(GameObject.GetGameObjectId(ESCAPE_HOSTAGE_NAME), {
							id = "IsInRange",
							range = 3,
							target = { -626.664, 342.623, 470.654 },
						})
						local isInRange02 = GameObject.SendCommand(GameObject.GetGameObjectId(ESCAPE_HOSTAGE_NAME), {
							id = "IsInRange",
							range = 3,
								...
								for index, labelName in ipairs(this.longMonologueList) do
									...
									if actionStatus == TppGameObject.NPC_STATE_CARRIED then
										if svars[labelName] == false then
											...
											Fox.Log(
												"s10033_mission Monologue labelName Played!!!" .. tostring(labelName)
											)
										end
									else
										break
