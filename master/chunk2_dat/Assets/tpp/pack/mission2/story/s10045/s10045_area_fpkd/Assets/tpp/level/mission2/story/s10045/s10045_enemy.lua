...
this.InterCall_targetNextLocation = function(soldier2GameObjectId, cpID, interName)
	...
	TppInterrogation.RemoveHighInterrogation(
		GameObject.GetGameObjectId("afgh_fieldWest_ob"),
		{ { name = "enqt1000_101528", func = this.Interrogation_LocateFlowers } }
	)
	TppInterrogation.RemoveHighInterrogation(
...
this.SetRelativeVehicle_recovery = function()
	...
	local command =
		{ id = "SetRelativeVehicle", targetId = vehicleId, rideFromBeginning = true, isMust = true, isVigilance = true }
	GameObject.SendCommand(soldierId, command)
end
...
this.setfova_executioner = function()
	...
	local command =
		{ id = "ChangeSpecialFova", faceIndex = 0, bodyIndex = 0, balaclavaFaceId = TppEnemyFaceId.svs_balaclava }
	GameObject.SendCommand(gameObjectId, command)
end
