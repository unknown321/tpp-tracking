...
this.CheckEnemyAllDead = function()
	...
	for i, soldiersName in pairs(this.ImportantNPCList) do
		lifeState =
			GameObject.SendCommand(GameObject.GetGameObjectId("TppSoldier2", soldiersName), { id = "GetLifeStatus" })
...
this.HostageRideOnVehicle = function()
	...
	local gameObjectId = { type = "TppHostageUnique", index = 0 }
	local vehicleId = GameObject.SendCommand(gameObjectId, {
		id = "RideVehicle",
...
this.ChangeRouteSetsTake = function()
	...
	if
		this.CheckHostagePosition(IDEN_ENE_POS, IDEN_KEY_HOS_POS)
		and this.CheckEnemyOnRoute(this.TakeRoute.Wait1) > 0
	then
...
this.SetNotShiftEnemy = function() end
...
this.SetUpEnemy = function()
	GameObject.SendCommand(
		{ type = "TppVehicle2" },
		{ id = "Spawn", locator = VEHICLE_NAME, type = Vehicle.type.EASTERN_LIGHT_VEHICLE }
	)
