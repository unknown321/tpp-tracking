...
this.missionObjectiveDefine = {
	...
	default_area_fort = {
		gameObjectName = "default_area_fort",
		visibleArea = 5,
		randomRange = 0,
		viewType = "all",
		setNew = false,
	...
	marker_area_bridge = {
		gameObjectName = "marker_area_bridge",
		visibleArea = 4,
		randomRange = 2,
		viewType = "all",
		setNew = false,
	...
	marker_honey_bee = {
		gameObjectName = "marker_honey_bee",
		visibleArea = 0,
		randomRange = 0,
		viewType = "all",
		setNew = true,
	...
	marker_about_bridge = {
		gameObjectName = "marker_about_bridge",
		visibleArea = 8,
		randomRange = 3,
		viewType = "all",
		setNew = true,
...
function this.MissionPrepare()
	...
	TppMission.RegiserMissionSystemCallback({
		...
		OnEndDeliveryWarp = function(stationUniqueId)
			...
			elseif
				stationUniqueId == TppPlayer.GetStationUniqueId("bridge")
				and currentSequence == "Seq_Game_BossBattle"
			then
...
this.CallGameOver = function()
	...
	TppMission.ReserveGameOver(
		TppDefine.GAME_OVER_TYPE.S10040_ARRIVAL_HONEY_BEE,
		TppDefine.GAME_OVER_RADIO.S10040_ARRIVAL_HONEY_BEE
	)
end
...
function this.Messages()
	return StrCode32Table({
		GameObject = {
			...
			{
				msg = "RouteEventFaild",
				sender = { "sol_s10040_0000", "sol_s10040_0001", "sol_s10040_0002" },
				func = function(gameObjectId, routeId, failureType)
					...
					elseif svars.eventSequenceNum < this.EVENT_SEQUENCE.WAIT_FORT then
						...
						elseif
							failureType == TppGameObject.ROUTE_EVENT_FAILED_TYPE_LOST_VEHICLE_HOSTAGE_PUT_IN
							or failureType == TppGameObject.ROUTE_EVENT_FAILED_TYPE_LOST_VEHICLE_HOSTAGE_TAKE_OUT_OF
						then
							Fox.Log("vehicle is gone")
							s10040_enemy.StartTravelWalk()
						elseif
							failureType == TppGameObject.ROUTE_EVENT_FAILED_TYPE_LOST_HOSTAGE_TAKE_OUT_OF_VEHICLE
						then
						end
					end
				end,
		...
		Trap = {
			...
			{
				msg = "Enter",
				sender = "trap_fort_candle",
				func = function()
					Gimmick.EnableCandleWind(true)
				end,
			},
			{
				msg = "Exit",
				sender = "trap_fort_candle",
				func = function()
					Gimmick.EnableCandleWind(false)
				end,
			},
		},
		Timer = {
...
this.ChangeWeatherForBoss = function(enable)
	...
	if enable == "battle" then
		Fox.Log("FOG start")
		TppWeather.ForceRequestWeather(
			TppDefine.WEATHER.FOGGY,
			0.01,
			{ fogDensity = 0.15, fogType = WeatherManager.FOG_TYPE_EERIE }
		)
	elseif enable == "beforeDemo" then
		...
		TppWeather.ForceRequestWeather(
			TppDefine.WEATHER.FOGGY,
			4,
			{ fogDensity = 5, fogType = WeatherManager.FOG_TYPE_EERIE }
		)
	elseif enable == "getHoneyBee" then
		...
		TppWeather.ForceRequestWeather(
			TppDefine.WEATHER.FOGGY,
			9,
			{ fogDensity = 0.005, fogType = WeatherManager.FOG_TYPE_EERIE }
		)
	else
		Fox.Log("FOG stop")
...
this.ChangeRouteEndTalk = function()
	...
	elseif svars.eventSequenceNum >= this.EVENT_SEQUENCE.ATTACK0 then
		...
		if s10040_enemy.ChangeRouteSetsTake() then
			if svars.isGetMissile == false and this.CheckLookHostage() == true then
		...
		return StrCode32Table({
			GameObject = {
						...
						if svars.eventSequenceNum <= this.EVENT_SEQUENCE.DRIVE then
							if phase >= TppGameObject.PHASE_CAUTION then
								...
								GameObject.SendCommand(
									GameObject.GetGameObjectId("sol_s10040_0000"),
									{ id = "SetRestrictNotice", enabled = flag }
								)
								GameObject.SendCommand(
									GameObject.GetGameObjectId("sol_s10040_0001"),
									{ id = "SetRestrictNotice", enabled = flag }
								)
								GameObject.SendCommand(
									GameObject.GetGameObjectId("sol_s10040_0002"),
									{ id = "SetRestrictNotice", enabled = flag }
								)
							end
						end
				...
				},
				{
					...
					func = function(gameObjectId, flag)
						...
						if
							flag == 0
							and TppEnemy.GetLifeStatus(HOSTAGE_NAME) ~= TppEnemy.LIFE_STATUS.DEAD
							and svars.isGetInfoHoneyBee == false
						then
							svars.isFlag04 = true
			...
			Trap = {
				...
				},
				{
					...
					sender = "trap_bgm_cave",
					func = function()
						s10040_sound.BGMPhaseReset()
					end,
				},
				{
					msg = "Enter",
					sender = "trap_demo_bridge",
					func = self.StartAttackDemo,
				},
				{
					msg = "Enter",
					sender = "trap_radio_bridge",
					func = function()
				...
				{
					msg = "Enter",
					sender = "trap_radio_taking",
					func = function()
						if
							s10040_enemy.CheckHostageInRoom1()
							and this.CheckLookHostage() == true
							and svars.isGetMissile == false
						then
				...
				{
					msg = "Enter",
					sender = "trap_demo_honeyBee",
					func = function()
						if svars.isFlag02 == false and svars.eventSequenceNum < this.EVENT_SEQUENCE.GET_MISSILE then
							svars.isFlag02 = true

							local demoBlockName = TppScriptBlock.GetCurrentPackListName("demo_block")

							if
								demoBlockName == "Demo_FindHoneyBee1"
								and s10040_demo.CheckIsPlayHostageAttack() == false
								and Tpp.IsNotAlert()
							then
			...
			Timer = {
				...
				{
					msg = "Finish",
					sender = "Timer_RadioRescueHostage",
					func = function()
						if s10040_enemy.CheckHostageInRoom1() and this.CheckLookHostage() == true then
							s10040_radio.PlayGoToRescueHostage()
						end
					end,
				},
...
sequences.Seq_Game_BossBattle = {
				...
				},
				{
					...
					func = function(gameObjectId, attackId)
						if s10040_enemy.CheckParasiteMessage(gameObjectId) == true then
							...
							if
								attackId == TppDamage.ATK_80002
								or attackId == TppDamage.ATK_80004
								or attackId == TppDamage.ATK_80006
								or attackId == TppDamage.ATK_80103
								or attackId == TppDamage.ATK_80104
								or attackId == TppDamage.ATK_80116
								or attackId == TppDamage.ATK_80124
								or attackId == TppDamage.ATK_80125
								or attackId == TppDamage.ATK_80203
								or attackId == TppDamage.ATK_80204
								or attackId == TppDamage.ATK_80205
								or attackId == TppDamage.ATK_80206
								or attackId == TppDamage.ATK_80303
								or attackId == TppDamage.ATK_80304
								or attackId == TppDamage.ATK_80305
								or attackId == TppDamage.ATK_80306
								or attackId == TppDamage.ATK_HoneyBee
							then
								svars.isFlag01 = true
				...
				},
				{
					...
					func = function(gameObjectId, lzName)
						if lzName == Fox.StrCode32("lzs_fort_S_000") then
							...
							GameObject.SendCommand(
								gameObjectId,
								{ id = "CallToLandingZoneAtName", name = "lzs_fort_W_0000" }
							)
						end
					end,
				},
	...
	SetRealizeParasite = function(block, state)
		...
		if blockName == "Demo_Parasite" and state == ScriptBlock.TRANSITION_ACTIVATED then
			...
			GameObject.SendCommand(
				{ type = "TppParasite2" },
				{ id = "StartCombat", position = Vector3(2119.874, 456.582, -1753.959), radius = 30.0 }
			)
	...
	Messages = function(self)
		return StrCode32Table({
			...
			Timer = {
				...
				{
					msg = "Finish",
					sender = "Timer_HeliCall",
					func = function()
						local gameObjectId = GameObject.GetGameObjectId("TppHeli2", "SupportHeli")
						GameObject.SendCommand(
							gameObjectId,
							{ id = "CallToLandingZoneAtName", name = "lz_fort_W0000|lz_fort_W_0000" }
						)
