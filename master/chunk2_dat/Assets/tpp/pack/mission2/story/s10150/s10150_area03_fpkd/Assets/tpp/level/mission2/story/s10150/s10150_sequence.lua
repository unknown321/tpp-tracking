...
function this.MissionPrepare()
	...
	TppLocation.RegistMissionAssetInitializeTable(this.s10150_baseOnActiveTable, this.s10150_OnActiveSmallBlockTable)
...
this.OnEstablishMissionClear = function(missionClearType)
	...
	TppScriptBlock.LoadDemoBlock("Demo_SahelanAppearance", false)
		...
		GameObject = {
			{
				...
				func = function(s_gameObjectId, containerName)
					if Tpp.IsFultonContainer(s_gameObjectId) then
						if
							TppMission.IsEnableAnyParentMissionObjective("clear_thirdBonus_sub_MissionTask") == false
						then
							...
							for i, gimmickId in pairs(this.MISSION_TASK_TARGET_LIST.CONTAINER) do
								...
								if gameObjectId == NULL_ID then
									...
									if s_gameObjectId == gameObjectId then
										...
										Fox.Log(
											"___________________svars.getTaskContainer : " .. svars.getTaskContainer
										)
...
this.missionObjectiveDefine = {
	...
	default_area_Okb = {
		gameObjectName = "10150_marker_heliport",
		viewType = "all",
		visibleArea = 1,
		randomRange = 0,
		setNew = false,
		announceLog = "updateMap",
		mapRadioName = "s0150_mprg0010",
			...
			Demo = {
				{
					...
					func = function()
						...
						GameObject.SendCommand(
							...
							{ id = "SetDemoToSendEnabled", enabled = true, route = "rts_demo_citadelSouth_S_0000" }
						)
					end,
	...
	Messages = function(self)
		return StrCode32Table({
			Trap = {
				...
				{
					msg = "Enter",
					sender = "trap_ChangeModeWelcomeTrue",
					func = function()
				...
				{
					msg = "Enter",
					sender = "trap_ChangeModeWelcomeFalse",
					func = function()
	...
	FuncNextSequenceSkullFaceAppearance = function()
		TppMission.Reload({
			...
			OnEndFadeOut = function()
				...
				TppScriptBlock.LoadDemoBlock("Demo_SkullFaceAppearance", false)
...
sequences.Seq_Game_TalkSkullFace = {
	...
	PlayerToSkullFacecDistance = function()
		...
		local SquarePlayerToSkullDistance =
	...
	Messages = function(self)
		return StrCode32Table({
			...
			Trap = {
				...
				{
					msg = "Enter",
					sender = "trap_Demo_SahelanAppearance",
					func = self.FuncNextSequenceSahelanAppearance,
				},
			...
			Timer = {
				...
				{
					msg = "Finish",
					sender = "SOFDelayTimer",
					func = function()
						TppSound.SetSceneBGM("bgm_skull_drive")
					end,
				},
				{
					msg = "Finish",
