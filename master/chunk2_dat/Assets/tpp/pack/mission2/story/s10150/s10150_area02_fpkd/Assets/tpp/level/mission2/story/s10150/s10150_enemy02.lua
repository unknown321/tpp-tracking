...
this.routeChangeTableRoot[Fox.StrCode32("rideCar")] = {

	{
		func = function()
			...
			Fox.Log(
				"rideCar //// mvars.RideMemberNum: "
					.. mvars.RideMemberNum
					.. " svars.isPlayerRidingOn:"
					.. tostring(svars.isPlayerRidingOn)
			)
...
function this.Messages()
	local messageTable = {
		...
		Timer = {
			...
			{ msg = "Finish", sender = "StartVehicleTimer", func = function() end },
			{
				msg = "Finish",
				sender = "SkullRouteFirstTalkTimer",
				func = function()
					this.TalkSkullFace("SFT_01")
				end,
			},
			{
				msg = "Finish",
				sender = "SkullWarnsPlayerTimer20",
				func = function()
			...
			{
				msg = "Finish",
				sender = "SkullWarnsPlayerTimer30",
				func = function()
			...
			{
				msg = "Finish",
				sender = "SkullWarnsPlayerTimer40",
				func = function()
			...
			{
				msg = "Finish",
				sender = "SkullWarnsPlayerTimer50",
				func = function()
			...
			{
				msg = "Finish",
				sender = "SkullWarnsPlayerTimer110",
				func = function()
		...
		Trap = {
			...
			{
				msg = "Enter",
				sender = "trap_EvOurPlayer",
				func = function()
					this.OnRoutePoint(nil, nil, nil, Fox.StrCode32("outEV2"))
				end,
			},
			{
				msg = "Enter",
				sender = "trap_rideVehiclePlayerCheck",
				func = function()
...
this.StartEv = function()
	if mvars.RideEvNum == RIDE_EV_MAX_NUM then
		...
		Gimmick.BreakGimmick(
			GameObjectType,
			"afgh_elvt001_gim_n0000|srt_afgh_elvt001",
			"/Assets/tpp/level/location/afgh/block_large/citadel/afgh_citadel_gimmick.fox2",
			0
		)
	end
end
...
this.WarpSetHeli = function(trapName)
	...
	for i, heliName in ipairs(heliGroup) do
		...
		local cmdSetRoute = {
			id = "SetForceRoute",
			route = this.EventHeliTable[trapName].routeList[i],
			point = 0,
			warp = this.EventHeliTable[trapName].warpFlag,
		}
...
this.SetUpHeliSkull = function()
	...
	GameObject.SendCommand(
		...
		{ id = "SetForceRoute", route = "rts_h_heli_skull_talk_beforeEV" }
	)
end
...
this.UpdateProgressSkullFace = function(id, sendM)
	Fox.Log(
		"_____________s10150_enemy02.UpdateProgressSkullFace() / ID : "
			.. tostring(id)
			.. " / sendM : "
			.. tostring(sendM)
	)
...
this.SetSkullFaceWarnsPlayerSpecialAction20 = function()
	...
	Fox.Log(
		"_____________s10150_enemy02.SetSkullFaceWarnsPlayerSpecialAction20() randomIndex:"
			.. randomIndex
			.. " stateName:"
			.. tostring(stateName)
			.. " labelName:"
...
this.SetSkullFaceWarnsPlayerSpecialAction50 = function()
	...
	Fox.Log(
		"_____________s10150_enemy02.SetSkullFaceWarnsPlayerSpecialAction50() randomIndex:"
			.. randomIndex
			.. " stateName:"
			.. tostring(stateName)
			.. " labelName:"
...
this.UpdateBringerRoute = function(solProgress, solName, solRouteType)
	...
	for i, tName in pairs(bringerRouteTable) do
		if tName.progress == solProgress then
			Fox.Log(
				"_____________routeName[routeA]/ "
					.. tostring(tName["routeA"])
					.. " /_____________routeName[routeB]/ "
					.. tostring(tName["routeB"])
			)
...
this.ResisterVehicleSE = function(vehicleId, soundId)
	GameObject.SendCommand(
		{ type = "TppVehicle2" },
		{ id = "RegisterSpecialEngineSfx", targetId = GameObject.GetGameObjectId(vehicleId), eventName = soundId }
	)
end

this.ConstrainVehicleOnRail = function(vehicleId)
	GameObject.SendCommand(GameObject.GetGameObjectId(vehicleId), { id = "Constrain" })
end
...
this.SpawnVehicleOnInitialize = function()
	local despawnList = {
		{
			id = "Despawn",
			locator = "vehs_citadel_tank_0000",
			type = Vehicle.type.EASTERN_WHEELED_ARMORED_VEHICLE,
			index = 1,
		},
		{
			id = "Despawn",
			locator = "vehs_citadel_tank_0001",
			type = Vehicle.type.EASTERN_WHEELED_ARMORED_VEHICLE,
			index = 2,
		},
		{
			id = "Despawn",
			locator = "vehs_citadel_tank_0002",
			type = Vehicle.type.EASTERN_WHEELED_ARMORED_VEHICLE,
			index = 3,
		},
	}
