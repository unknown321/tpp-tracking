	...
	for locatorName, table in pairs(posList) do
		Fox.Log(
			"locatorName : "
				...
				.. " / x :"
				.. tostring(table.x_pos)
				.. " / y :"
				.. tostring(table.y_pos)
				.. " / z :"
				.. tostring(table.z_pos)
		)
		local name = locatorName
		local command =
			{ id = "SetLocatorPosition", name = name, x = table.x_pos, y = table.y_pos, z = table.z_pos, r = table.rad }
...
this.SetUpWalkerGear = function()
	...
	for i, wgName in pairs(WalkerGear_Ride_Group) do
		Fox.Log(
			"____________________________________WalkerGearName is : "
				.. tostring(i)
				.. "____________________________________"
		)
		Fox.Log(
			"solName : "
				.. tostring(wgName["solName"])
				.. " /isBeginning : "
				.. tostring(wgName["isBegining"])
				.. " /sneakRouteName : "
				.. tostring(wgName["sneakRouteName"])
				.. " /cautionRouteName : "
				.. tostring(wgName["cautionRouteName"])
		)
		...
		local cmdSetRelativeVehicle =
			{ id = "SetRelativeVehicle", targetId = walkerId, rideFromBeginning = isBeginning }
