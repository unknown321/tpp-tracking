	...
	hostageList = {
		{
			...
			bodyId = TppDefine.QUEST_BODY_ID_LIST.Q99080_02,
			uniqueTypeId = TppDefine.UNIQUE_STAFF_TYPE_ID.QUEST_KANTOKU,
			voiceType = "dummy",
		},
	},
...
function this.OnAllocate()
	...
	TppQuest.RegisterQuestSystemCallbacks({
		...
		OnTerminate = function()
			this.SwitchEnableQuestHighIntTable(false, "afgh_slopedTown_cp", this.questCpInterrogation)

			TppEnemy.OnTerminateQuest(this.QUEST_TABLE)
		end,
	})

	mvars.isDemoPlay = false
...
quest_step.QStep_Start = {
	OnEnter = function()
		this.SwitchEnableQuestHighIntTable(true, "afgh_slopedTown_cp", this.questCpInterrogation)

		local gameObjectId = GetGameObjectId(TARGET_HOSTAGE_NAME)
			...
			Trap = {
				{
					...
					func = function()
						TppScriptBlock.Load("animal_block", "slopedTown_S_RescueSpecial", true)
					end,
				},
			},
			Player = {
				{
					msg = "HostageUnlock",
					func = function(state, s_gameObjectId)
						if mvars.isDemoPlay == false then
							local gameObjectId = GetGameObjectId(TARGET_HOSTAGE_NAME)
							if s_gameObjectId == gameObjectId then
								TppDemo.Play("Demo_RescueSpecial", {
