	...
	enemyList = {
		{
			...
			bodyId = TppDefine.QUEST_BODY_ID_LIST.MSF_10,
			uniqueTypeId = TppDefine.UNIQUE_STAFF_TYPE_ID.QUEST_MSF_10,
...
function this.OnAllocate()
	...
	TppQuest.RegisterQuestSystemCallbacks({
		OnActivate = function()
			...
			local command = { id = "SetMsfCombatLevel", level = 9 }
...
this.SetEscapePoint = function()
	...
	local identifierName = "identifier_q80040_esc_pos"
	local startPos = Tpp.GetLocatorByTransform(identifierName, "start")
	local underPos = Tpp.GetLocatorByTransform(identifierName, "under")
	local pollPos = Tpp.GetLocatorByTransform(identifierName, "poll")
	local planePos = Tpp.GetLocatorByTransform(identifierName, "plane")
	...
	local EscapePosList = {
		{
			idx = EscapeIdx.Start,
			pos = startPos,
			link = { EscapeIdx.Under, EscapeIdx.Poll, EscapeIdx.Plane, EscapeIdx.Tree },
		},
		{ idx = EscapeIdx.Under, pos = underPos, link = { EscapeIdx.Start, EscapeIdx.Poll, EscapeIdx.Tree } },
		{ idx = EscapeIdx.Poll, pos = pollPos, link = { EscapeIdx.Start, EscapeIdx.Under, EscapeIdx.Plane } },
		{ idx = EscapeIdx.Plane, pos = planePos, link = { EscapeIdx.Start, EscapeIdx.Tree, EscapeIdx.Poll } },
		{ idx = EscapeIdx.Tree, pos = treePos, link = { EscapeIdx.Start, EscapeIdx.Under, EscapeIdx.Plane } },
	}
		...
		return StrCode32Table({
			Marker = {
				{
					...
					func = function(arg0, arg1, arg2, arg3)
						...
						if
							arg0 == StrCode32("sol_quest_0000")
							and arg3 == StrCode32("Player")
							and mvars.isPlayChangeToEnableRadio == false
						then
			...
			GameObject = {
				...
				},
				{
					...
					func = function(gameObjectId)
						local isClearType =
							TppEnemy.CheckQuestAllTarget(this.QUEST_TABLE.questType, "Vanished", gameObjectId)
						TppQuest.ClearWithSave(isClearType)
					end,
				},
				{
