...
this.missionObjectiveDefine = {
	...
	default_area_ruinsNorth = {
		gameObjectName = "marker_ruinsNorth",
		visibleArea = 4,
		randomRange = 0,
		viewType = "map_only_icon",
		setNew = false,
		langId = "marker_enemyarea_trans",
		mapRadioName = "f1000_rtrg0700",
...
function this.OnRestoreSVars()
	...
	if TppMission.IsMissionStart() then
		svars.isBrokenGimmick_s10043_01 =
			TppGimmick.IsBroken({ gimmickId = S10043_GIMMICK_DATA_LIST.ANTN001.gimmickId, searchFromSaveData = true })
		svars.isBrokenGimmick_s10043_02 =
			TppGimmick.IsBroken({ gimmickId = S10043_GIMMICK_DATA_LIST.ANTN002.gimmickId, searchFromSaveData = true })
		svars.isBrokenGimmick_s10043_03 =
			TppGimmick.IsBroken({ gimmickId = S10043_GIMMICK_DATA_LIST.ANTN003.gimmickId, searchFromSaveData = true })
		svars.isBrokenGimmick_s10043_04 =
			TppGimmick.IsBroken({ gimmickId = S10043_GIMMICK_DATA_LIST.CMMN001.gimmickId, searchFromSaveData = true })

		gvars.s10043_forceMissionClear = false

		if
			(
				svars.isBrokenGimmick_s10043_01 == true
				and svars.isBrokenGimmick_s10043_02 == true
				and svars.isBrokenGimmick_s10043_03 == true
			) or (svars.isBrokenGimmick_s10043_04 == true)
		then
	...
	local systemCallbackTable = {
			...
			if missionClearType == TppDefine.MISSION_CLEAR_TYPE.FREE_PLAY_ORDER_BOX_DEMO then
				if orderBoxName then
					...
					GkEventTimerManager.Start(
						"Timer_LoadOpeningDemoBlock",
						START_OPENING_DEMO_FADE_SPEED + START_MISSION_GAME_END_TIME
					)
		...
		OnDisappearGameEndAnnounceLog = function(missionClearType)
			...
			if
				...
				or (missionClearType == TppDefine.MISSION_CLEAR_TYPE.QUEST_INTRO_RESCUE_EMERICH_END)
			then
		...
		OnEndMissionReward = function()
			...
			elseif missionClearType == TppDefine.MISSION_CLEAR_TYPE.FORCE_GO_TO_MB_ON_SIDE_OPS_CLEAR then
				...
				local radioList = TppStory.GetForceMBDemoNameOrRadioList(
					"clearSideOpsForceMBRadio",
					{ clearSideOpsName = mvars.qst_currentClearQuestName }
				)
...
function this.OnCheckMBReturnRadio()
	...
	if
		TppMotherBaseManagement.IsBuiltMbMedicalClusterSpecialPlatform
		and currentStorySequence < TppDefine.STORY_SEQUENCE.CLEARD_RESCUE_HUEY
		and TppQuest.IsActive("sovietBase_q99020") == false
	then
		if
			not (TppRadio.IsPlayed("f2000_rtrg3005"))
			and not (playerRideState == rideHeli)
			and (svars.acceptMissionId == 0)
		then
		end
	end
end
...
function this.Messages()
	return StrCode32Table({
		...
		Timer = {
			...
			{
				msg = "Finish",
				sender = "Timer_LoadOpeningDemoBlock",
				func = f30010_demo.LoadOpeningDemoBlock,
				option = { isExecMissionClear = true },
			},
			{
				msg = "Finish",
				sender = "Timer_StartMissionGamEndOnOpeningDemo",
				func = function()
				...
				func = function()
					if this.IsQuietQuestOK() then
						if not mvars.q99010_questActive then
							...
							Fox.Log(
								"######### f30010_sequence:Message::trap_BossQuiet_QuestInsurance ######### waterway_q99010 / !!!NOT ACTIVE!!! waiting activation..."
							)
...
this.IsQuietQuestOK = function()
	if
		TppQuest.IsActive("waterway_q99010")
		and TppQuest.IsOpen("waterway_q99010")
		and not (TppQuest.IsCleard("waterway_q99010"))
	then
		return true
	end
	return false
...
this.RequestActiveForBossQuiet = function(place)
	...
	Fox.Log(
		"######### f30010_sequence:Message::trap_QuietShoot_Insurance ######### waterway_q99010 / Insurance processing! ["
			.. tostring(place)
			.. "]"
	)
...
sequences.Seq_Game_FreePlay = {
	...
	OnEnter = function()
		...
		if TppQuest.IsActive("waterway_q99010") then
			...
			TppUiCommand.RegisterDisableDeliveryStation({
				stationId = TppCollection.GetUniqueIdByLocatorName("col_stat_sovietBase"),
			})
			TppUiCommand.RegisterDisableDeliveryStation({
				stationId = TppCollection.GetUniqueIdByLocatorName("col_stat_powerPlant"),
			})
		else
			TppUiCommand.UnRegisterDisableDeliveryStation({
				stationId = TppCollection.GetUniqueIdByLocatorName("col_stat_sovietBase"),
			})
			TppUiCommand.UnRegisterDisableDeliveryStation({
				stationId = TppCollection.GetUniqueIdByLocatorName("col_stat_powerPlant"),
