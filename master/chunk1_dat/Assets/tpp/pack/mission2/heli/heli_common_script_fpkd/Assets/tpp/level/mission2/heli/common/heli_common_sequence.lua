...
function this.AddCommonHeliSpaceMessage(messageTable)
	...
	table.insert(
		messageTable.UI,
		{ msg = "PopupClose", sender = TppDefine.ERROR_ID.NOW_INSTALLING, func = this.OnPopupCloseChunkInstalling }
	)
...
function this.OnUpdateChunkInstalling()
	...
	if DebugText then
		...
		DebugText.Print(
			context,
			{ 1.0, 0.0, 0.0 },
			"Select GoToMGO : " .. tostring(mvars.heliSpaceMGOChunkInstallCheckingState)
		)
	end
