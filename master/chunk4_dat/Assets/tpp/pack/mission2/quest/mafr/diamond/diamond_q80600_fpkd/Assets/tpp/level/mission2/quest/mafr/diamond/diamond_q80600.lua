	...
	enemyList = {
		{
			...
			bodyId = TppDefine.QUEST_BODY_ID_LIST.MSF_05,
			uniqueTypeId = TppDefine.UNIQUE_STAFF_TYPE_ID.QUEST_MSF_05,
...
function this.OnAllocate()
	...
	TppQuest.RegisterQuestSystemCallbacks({
		OnActivate = function()
			...
			local command = { id = "SetMsfCombatLevel", level = 4 }
...
this.SetEscapePoint = function()
	...
	local identifierName = "identifier_q80600_esc_pos"
	local smokePos = Tpp.GetLocatorByTransform(identifierName, "smoke")
	local waterPos = Tpp.GetLocatorByTransform(identifierName, "water")
	local tentPos = Tpp.GetLocatorByTransform(identifierName, "tent")
	local hillPos = Tpp.GetLocatorByTransform(identifierName, "hill")

	local EscapePosList = {
		{ idx = EscapeIdx.Smoke, pos = smokePos, link = { EscapeIdx.Water, EscapeIdx.Hill } },
		{ idx = EscapeIdx.Water, pos = waterPos, link = { EscapeIdx.Tent, EscapeIdx.Smoke } },
		{ idx = EscapeIdx.Tent, pos = tentPos, link = { EscapeIdx.Water, EscapeIdx.Hill } },
		{ idx = EscapeIdx.Hill, pos = hillPos, link = { EscapeIdx.Tent, EscapeIdx.Smoke } },
	}
