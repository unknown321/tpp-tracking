...
this.missionObjectiveDefine = {
	default_Target_pfCamp = {
		gameObjectName = "10121_marker_pfCamp",
		visibleArea = 5,
		randomRange = 0,
		viewType = "all",
		setNew = false,
	...
	default_Target_Area = {
		gameObjectName = "10121_marker_Leader",
	...
	default_Sub_Target = {
		...
		setNew = true,
		announceLog = "updateMap",
		langId = "marker_ene_wp_supplier",
	},

	default_photo_target_10 = {
		photoId = 10,
		addFirst = true,
		photoRadioName = "s0121_mirg2010",
	},

	default_photo_target_20 = {
		photoId = 20,
		addFirst = true,
		photoRadioName = "s0121_mirg2020",
	},
	...
	default_viewpoint = {
		gameObjectName = "10121_marker_viewpoint",
		mapRadioName = "s0121_mprg2010",
		visibleArea = 2,
		randomRange = 0,
		viewType = "map_only_icon",
		setNew = true,
...
function this.ContinueMission()
	...
	elseif svars.sub_Flag_0002 == false then
		if
			(TppMarker.GetSearchTargetIsFound(TARGET_ENEMY_NAME) == false)
			and (TppMarker.GetSearchTargetIsFound(SUB_TARGET_ENEMY_NAME) == false)
		then
		...
		elseif
			(TppMarker.GetSearchTargetIsFound(TARGET_ENEMY_NAME) == true)
			and (TppMarker.GetSearchTargetIsFound(SUB_TARGET_ENEMY_NAME) == false)
		then
			s10121_radio.ContinueVipFound()
		elseif
			(TppMarker.GetSearchTargetIsFound(TARGET_ENEMY_NAME) == false)
			and (TppMarker.GetSearchTargetIsFound(SUB_TARGET_ENEMY_NAME) == true)
		then
...
this.OffHighInterrogation = function()
	...
	TppInterrogation.RemoveHighInterrogation(GameObject.GetGameObjectId("mafr_pfCamp_cp"), {
		...
		Subtitles = {
			{
				...
				func = function(speechLabel, status)
					...
					elseif speechLabel == SubtitlesCommand:ConvertToSubtitlesId("wrec6000_3b1010_0_ened_af") then
						...
						if
							(svars.isListen1stTalk == true)
							and (svars.isListen2ndTalk == true)
							and (svars.isListen3rdTalk == true)
						then
		...
		GameObject = {
...

			{
				...
				func = function(GameObjectId, ContainerName)
					if
						(ContainerName == StrCode32(this.characterIdList.ContainerA))
						or (ContainerName == StrCode32(this.characterIdList.ContainerB))
						or (ContainerName == StrCode32(this.characterIdList.ContainerC))
					then
				...
				func = function(gameObjectId, routeId, routeNode, messageId)
					...
					elseif messageId == StrCode32("Kari_vip_evacuated") then
						TppMission.ReserveGameOver(
							TppDefine.GAME_OVER_TYPE.TARGET_ESCAPE,
				...
				func = function(gameObjectId, routeId, routeNode, messageId)
					...
					elseif messageId == StrCode32("EnemyHeliEscaped") then
						if svars.sub_Flag_0004 == true then
							TppMission.ReserveGameOver(
								TppDefine.GAME_OVER_TYPE.TARGET_ESCAPE_BY_HELI,
								TppDefine.GAME_OVER_RADIO.OTHERS
							)
						end
					end
			...
			GameObject = {
				{
					...
					func = function(GameObjectId, phaseName)
						...
						if
							phaseName ~= TppEnemy.PHASE.SNEAK
							and svars.isEnemyHeliStart == false
							and svars.isEnemyHeliArrived == false
						then
							this.CallCpRadio("CPR0560")
						elseif
							phaseName ~= TppEnemy.PHASE.SNEAK
							and svars.isEnemyHeliStart == true
							and svars.isEnemyHeliArrived == false
						then
							this.CallCpRadio("CPR0560")
