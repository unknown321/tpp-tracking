...
this.InterCall_common = function(soldier2GameObjectId, cpID, interName) end
	...
	if svars.eventSequenceIndex >= mvars.interEventSequenceIndex then
		...
		for i, params in ipairs(cpNameTable) do
			...
			if gameObjectId == NULL_ID then
				...
				elseif
					eventSequenceIndex >= params.eventSequenceIndexMin
					and eventSequenceIndex < params.eventSequenceIndexMax
				then
					interrogationTable = {}
...
this.SetUpEnemy = function()
	...
	TppEnemy.InitialRouteSetGroup({
		cpName = "mafr_pfCamp_cp",
		soldierList = { "sol_pfCamp_0000" },
		groupName = "groupConversation",
	})
	TppEnemy.InitialRouteSetGroup({
		cpName = "mafr_swamp_cp",
		soldierList = { "sol_swamp_0000" },
		groupName = "groupConversation",
	})

	TppEnemy.InitialRouteSetGroup({
		cpName = "mafr_pfCamp_cp",
		soldierList = { "sol_pfCamp_0001", "sol_pfCamp_0002", "sol_pfCamp_0003" },
		groupName = "groupSniper",
	})

	TppEnemy.InitialRouteSetGroup({
		cpName = "mafr_swamp_cp",
		soldierList = { this.ENEMY_NAME.ZRS_SWAMP01, this.ENEMY_NAME.ZRS_SWAMP02 },
		groupName = "groupZRS",
	})
	TppEnemy.InitialRouteSetGroup({
		cpName = "mafr_flowStation_cp",
		soldierList = { this.ENEMY_NAME.ZRS_FLOWSTATION01, this.ENEMY_NAME.ZRS_FLOWSTATION02 },
		groupName = "groupZRS",
	})
...
this.SetRelativeVehicle = function(enemyName, vehicleName, isrideFromBeginning, isVigilance)
	...
	if isrideFromBeginning then
		GameObject.SendCommand(
			enemyName,
			{
				id = "SetRelativeVehicle",
				targetId = vehicleName,
				rideFromBeginning = isrideFromBeginning,
		...
		GameObject.SendCommand(
			enemyName,
			{ id = "SetRelativeVehicle", targetId = vehicleName, isVigilance = isVigilance }
		)
	end
end
