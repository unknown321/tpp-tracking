...
this.ContinueMissionStart = function()
	if
		s10090_sequence.GetVehicleValue(
			s10090_sequence.ENUM_VEHICLE_VALUE_TYPE.MARKINGFLAG,
...
this.SearchEscortVehicle = function(type, isAppearanceParasites, isPunkTargetVehicle)
	...
	if svars.isMarkingVehicle01 == false and svars.isAppearanceParasites == false then
		if svars.eventSequenceIndex >= s10090_sequence.ENUM_EVENT_VEHICLE_SEQUENCE.ENTER_PFCAMP then
			...
			local isFound01 = s10090_sequence.GetVehicleValue(
				s10090_sequence.ENUM_VEHICLE_VALUE_TYPE.PRESENCE,
				s10090_enemy.VEHICLE_NAME.WEST_WAV01
			)
			local isFound02 = s10090_sequence.GetVehicleValue(
				s10090_sequence.ENUM_VEHICLE_VALUE_TYPE.PRESENCE,
...
function this.SetOptionalRadioUpdate(name, isForceUpdate)
	if name then
		...
		if s10090_sequence.IsMainSequence() == true then
			local isFound = s10090_sequence.GetVehicleValue(
				s10090_sequence.ENUM_VEHICLE_VALUE_TYPE.MARKINGFLAG,
				s10090_enemy.VEHICLE_NAME.TRUCK_TARGET
			)
			if isFound == true then
				...
				if isParasites == false then
					local isDrive =
						s10090_sequence.IsSequenceEnemyVehicleAction(s10090_sequence.ENUM_VEHICLE_CHECK_ACTION.DRIVE)
					local isStop =
						s10090_sequence.IsSequenceEnemyVehicleAction(s10090_sequence.ENUM_VEHICLE_CHECK_ACTION.STOP)
...
function this.SetIntelUpdateTruck(isDrive, isStop)
	local isWavFound01 = s10090_sequence.GetVehicleValue(
		s10090_sequence.ENUM_VEHICLE_VALUE_TYPE.PRESENCE,
		s10090_enemy.VEHICLE_NAME.WEST_WAV01
	)
	local isWavFound02 = s10090_sequence.GetVehicleValue(
		s10090_sequence.ENUM_VEHICLE_VALUE_TYPE.PRESENCE,
		s10090_enemy.VEHICLE_NAME.WEST_WAV02
	)
	if svars.eventSequenceIndex <= s10090_sequence.ENUM_EVENT_VEHICLE_SEQUENCE.STOP_PFCAMP then
