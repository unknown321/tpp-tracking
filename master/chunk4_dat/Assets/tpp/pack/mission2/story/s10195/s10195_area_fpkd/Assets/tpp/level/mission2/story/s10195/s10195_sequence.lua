...
this.missionObjectiveDefine = {
	...
	default_area_savannahEast = {
		gameObjectName = "10195_marker_VIP",
		viewType = "all",
		visibleArea = 4,
		mapRadioName = "s0195_mprg0020",
		randomRange = 0,
		setNew = false,
	...
	default_photo_VIP = {
		photoId = 10,
		addFirst = true,
		photoRadioName = "s0195_mirg0010",
	},

	default_photo_Daaler = {
		photoId = 20,
		addFirst = true,
		photoRadioName = "s0195_mirg0020",
	},
	...
	about_area_meeting = {
		gameObjectName = "10195_marker_dealer",
		visibleArea = 4,
		randomRange = 0,
		setNew = false,
		announceLog = "updateMap",
	...
	rv_missionClear = {
		gameObjectName = "10195_marker_RV",
		goalType = "moving",
		viewType = "all",
		announceLog = "updateMap",
	},
...
function this.Messages()
	return StrCode32Table({
			...
			},
			{
				...
				func = function(GameObjectId, phaseName)
					...
					if phaseName == TppGameObject.PHASE_ALERT then
						...
						elseif
							GameObjectId == GameObject.GetGameObjectId("sol_dealer")
							or GameObject.GetGameObjectId("mafr_diamondSouth_ob")
						then
		...
		Trap = {
			...
			{
				msg = "Enter",
				sender = { "trap_nearSvannahEast", "trap_nearHillWest", "trap_nearHillNorth", "trap_nearMeetingArea" },
...
this.IsBGMFollow = function()
	if
		svars.isKnowDealer == false
		and svars.isEndMeeting == false
		and svars.isKnowVIP == true
		and svars.isVipDriveCar == true
		and svars.isCancelMeeting == false
	then
		return true
	end
	return false
...
this.routePointHintTableRoot[Fox.StrCode32("heliOut")] = {
	...
	func = function()
		TppMission.ReserveGameOver(
			...
			TppDefine.GAME_OVER_RADIO.S10195_TARGET_ESCAPE
		)
	end,
}
...
this.OnRoutePointHint = function(gameObjectId, routeId, routeNodeIndex, messageId)
	Fox.Log(
		"s10195_sequence.OnRoutePointHint( gameObjectId:"
			.. tostring(gameObjectId)
			.. ", routeId:"
			.. tostring(routeId)
			.. ", routeNodeIndex:"
...
sequences.Seq_Game_FollowCommander = {
	...
	FuncVIPstart = function()
		if svars.isVipRouteChange == false and svars.isCancelMeeting == false then
			svars.isVipRouteChange = true, s10195_enemy.VipStartMove()
		end
	end,
