...
function this.MissionPrepare()
	...
	TppMarker.SetUpSearchTarget({
		{
			gameObjectName = "TppLiquid2",
			gameObjectType = "TppLiquid2",
			messageName = "TppLiquid2",
...
this.FuncDeadChildSoldier = function(gameObjectId)
	...
	if soldierType == EnemyType.TYPE_CHILD then
		...
		TppMission.ReserveGameOver(
			...
			TppDefine.GAME_OVER_RADIO.S10120_CHILD_DEAD
		)
	end
end
...
this.FuncUnconsciousLiquid = function(gameObjectId)
	...
	GameObject.SendCommand(
		...
		{ id = "SetPhase", phase = TppEnemy.PHASE.SNEAK }
	)
...
function this.Messages()
	return StrCode32Table({
		...
		Timer = {
			{
				msg = "Finish",
				sender = "Timer_LiquidLZGo",
				func = function()
					local gameObjectId = GameObject.GetGameObjectId("TppHeli2", "SupportHeli")
					GameObject.SendCommand(
						gameObjectId,
						{ id = "CallToLandingZoneAtName", name = "lzs_outland_Liquid" }
					)
				end,
			},
		},
...
sequences.Seq_Game_MainGame = {
	...
	Messages = function(self)
		return StrCode32Table({
			GameObject = {
				{
					msg = "LiquidChangePhase",
					func = self.FuncLiquidChangePhase,
				},
			},
			Trap = {
				{
					msg = "Enter",
					sender = "trap_boat_out",
				...
				},
				{
					...
					func = function()
						...
						Fox.Log(
							"entered stairs trap "
								.. tostring(liquidLifeState)
								.. ","
								.. tostring(liquidPhase)
								.. ","
								.. tostring(liquidChair)
								.. ","
								.. tostring(cpPhase)
						)
...
sequences.Seq_Game_LiquidFight = {
	...
	Messages = function(self)
		return StrCode32Table({
			GameObject = {
				{
					msg = "LiquidDefeatedByCqcInStartRoom",
					sender = "TppLiquid2",
					func = function()
...
sequences.Seq_Game_CallHeli = {
	...
	OnEnter = function()
		...
		GameObject.SendCommand(
			gameObjectId,
			{ id = "SetLandingZnoeDoorFlag", name = "lzs_outland_Liquid", leftDoor = "Open", rightDoor = "Close" }
		)
