...
this.OnRestoreSVars = function()
	...
	if svars.isEnemyInterrogation == false then
		if svars.isMapGet == false then
			if
				(TppMission.IsEnableAnyParentMissionObjective("add_s10093_Container_A") == false)
				or (TppMission.IsEnableAnyParentMissionObjective("add_s10093_Container_B") == false)
			then
				TppInterrogation.AddHighInterrogation(GameObject.GetGameObjectId("mafr_lab_cp"), {
				...
				TppInterrogation.AddHighInterrogation(GameObject.GetGameObjectId("mafr_labJungle_cp"), {
					{ name = "enqt1000_106980", func = s10093_enemy.InterCall_container_pos01 },
					{ name = "enqt1000_106973", func = s10093_enemy.InterCall_captain_pos01 },
				})
			else
	...
	if
		(svars.PreliminaryFlag03 == false)
		and (svars.PreliminaryFlag03 == false)
		and (svars.PreliminaryFlag05 == false)
	then
		TppInterrogation.AddHighInterrogation(GameObject.GetGameObjectId("mafr_lab_cp"), {
			{ name = "enqt1000_106980", func = s10093_enemy.InterCall_container_pos01 },
	...
	Gimmick.SetFultonableContainerForMission(
		TARGET_GIMMICK_NAME.CNTN001,
		TARGET_SEARCHTARGET_NAME_LIST.CNTN001.FOX2NAME,
		0,
		false
	)
	Gimmick.SetFultonableContainerForMission(
		TARGET_GIMMICK_NAME.CNTN002,
		TARGET_SEARCHTARGET_NAME_LIST.CNTN002.FOX2NAME,
		0,
		false
	)
	...
	if (svars.isNoContainerANotice == false) and (svars.isNoContainerBNotice == false) then
		TppEnemy.RegisterCombatSetting(s10093_enemy.combatSetting)
	elseif (svars.isNoContainerANotice == true) and (svars.isNoContainerBNotice == false) then
		TppEnemy.RegisterCombatSetting(s10093_enemy.combatSetting01)
	elseif (svars.isNoContainerANotice == false) and (svars.isNoContainerBNotice == true) then
		TppEnemy.RegisterCombatSetting(s10093_enemy.combatSetting02)
	elseif (svars.isNoContainerANotice == true) and (svars.isNoContainerBNotice == true) then
	...
	return StrCode32Table({
					...
					if markType == Fox.StrCode32("TYPE_ENEMY") then
						if MalkerById == StrCode32("Player") then
							if svars.isEnemyInterrogation == false then
								...
								if
									(TppMission.IsEnableMissionObjective("add_s10093_Container_A") == false)
									or (TppMission.IsEnableMissionObjective("add_s10093_Container_B") == false)
									or (TppMission.IsEnableMissionObjective("get_s10093_Container_A") == false)
									or (TppMission.IsEnableMissionObjective("get_s10093_Container_B") == false)
								then
									...
									Fox.Log(
										"s10093_sequence add_s10093_Container_A and add_s10093_Container_B On !!!!!"
									)
								end
							else
		...
		Timer = {
...

			{
				...
				sender = "HeliCamera",
				func = function()
					TppPlayer.SetTargetHeliCamera({
						gameObjectName = "EnemyHeli",
						announceLog = "target_extract_failed",
					})
				end,
				option = { isExecGameOver = true },
			},
...
this.commonMissionMapGet = function()
	...
	local sequence = TppSequence.GetCurrentSequenceName()
	if
		(sequence == "Seq_Game_ContainerGetBefore")
		or (sequence == "Seq_Game_ContainerAGetAfter")
		or (sequence == "Seq_Game_ContainerBGetAfter")
	then
...
this.commonContainerNotice = function()
	if (svars.isNoContainerANotice == true) and (svars.isNoContainerBNotice == false) then
...
function this.CheckSearchTarget(gameObjectId)
	...
	for key, SearchTargetSetting in pairs(gimmickSettingTable) do
		...
		Fox.Log(
			"s10093_sequence gimmickSettingTable LookingTarget: " .. tostring(svars[SearchTargetSetting.markFlagName])
		)
...
this.MissionPrepare = function()
	...
	TppMission.RegisterMissionSystemCallback({
		OnSetMissionFinalScore = function(missionClearType)
			Fox.Log(
				"!!!! s10093_mission.clear_missionTask_03 check !!!! isNoContainerANotice = "
					.. tostring(svars.isNoContainerANotice)
			)
			Fox.Log(
				"!!!! s10093_mission.clear_missionTask_03 check !!!! isNoContainerBNotice = "
					.. tostring(svars.isNoContainerBNotice)
			)
		...
		CheckMissionClearOnRideOnFultonContainer = function()
			...
			Fox.Log(
				"CheckMissionClearOnRideOnFultonContainer : isGetTargetContainer001 = "
					.. tostring(getContainer001)
					.. ", isGetTargetContainer002 = "
					.. tostring(getContainer002)
			)
...
this.OnSkipUpdateCommon = function() end

this.OnSkipLeaveCommon = function() end

this.commonMissionFailure = function(esmanager, messageId, message) end
		...
		return StrCode32Table({
			GameObject = {

				{
					...
					func = function(GameObjectId, ContainerName)
						if
							(ContainerName == StrCode32(TARGET_GIMMICK_NAME.CNTN001))
							or (ContainerName == StrCode32(TARGET_GIMMICK_NAME.CNTN002))
						then
							TppMission.UpdateObjective({
