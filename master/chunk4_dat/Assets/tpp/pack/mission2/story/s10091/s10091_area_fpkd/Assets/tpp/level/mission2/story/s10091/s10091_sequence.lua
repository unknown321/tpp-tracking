	...
	TppMission.RegisterMissionSystemCallback({
		...
		OnRecovered = function(gameObjectId)
			...
			if this.DoesIncludeTarget(gameObjectId, this.missionTask_3_bonus_TARGET_LIST) then
				svars.countMissionTask03TargetsNum =
					this.CountRecoveredMissionBonusTask3(this.missionTask_3_bonus_TARGET_LIST)
...
function this.Messages()
	return StrCode32Table({
		...
		GameObject = {
...

			{
				...
				func = function(gameObjectId, label)
					...
					local abductHostage_monologue_25 =
						{ id = "CallMonologue", label = "speech091_carry025", carry = true }

					if
						gameObjectId == GameObject.GetGameObjectId("hos_s10091_0000")
						and status_abductHostage01 == TppGameObject.NPC_STATE_CARRIED
					then
...

			{
				...
				func = function(arg0, arg1)
					if arg1 == GameObject.GetGameObjectId("Player") then
						...
						mvars.deadNPCId =
							TARGET_HOSTAGE.NORTH_FOREST,
							TppMission.ReserveGameOver(
								TppDefine.GAME_OVER_TYPE.TARGET_DEAD,
								TppDefine.GAME_OVER_RADIO.PLAYER_KILL_TARGET
							)
					else
						mvars.deadNPCId = TARGET_HOSTAGE.NORTH_FOREST, this.OnTargetDead()
					end
				end,
			},

			{
				...
				func = function(arg0, arg1)
					if arg1 == GameObject.GetGameObjectId("Player") then
						Fox.Log("###CommonRadio_PlayerKilledTarget_f8000_gmov2500###")
						mvars.deadNPCId =
							TARGET_HOSTAGE.SWAMP,
							TppMission.ReserveGameOver(
								TppDefine.GAME_OVER_TYPE.TARGET_DEAD,
								TppDefine.GAME_OVER_RADIO.PLAYER_KILL_TARGET
							)
					else
						mvars.deadNPCId = TARGET_HOSTAGE.SWAMP, this.OnTargetDead()
					end
				end,
			},
		...
		Timer = {
				...
				func = function()
					if svars.isSearchUnit01OnMoving == false then
						...
						GameObject.SendCommand(
							GameObject.GetGameObjectId("sol_SwampWest_0002"),
							{ id = "StartTravel", travelPlan = "Travel_First_Car" }
						)
						GameObject.SendCommand(
							GameObject.GetGameObjectId("sol_SwampWest_0003"),
							{ id = "StartTravel", travelPlan = "Travel_First_Car" }
						)
						GameObject.SendCommand(
							GameObject.GetGameObjectId("sol_SwampWest_0004"),
							{ id = "StartTravel", travelPlan = "Travel_First_Car" }
						)

						GameObject.SendCommand(
							GameObject.GetGameObjectId("sol_Truck_Driver"),
							{ id = "StartTravel", travelPlan = "Travel_Truck" }
						)
...

			{
				...
				func = function()
					if svars.isExecuteUnitRouteChanged == false then
						...
						if
							TppEnemy.GetLifeStatus("sol_ExecuteUnit_0000") == TppEnemy.LIFE_STATUS.NORMAL
								and TppEnemy.GetStatus("sol_ExecuteUnit_0000") == EnemyState.NORMAL
							or TppEnemy.GetLifeStatus("sol_ExecuteUnit_0001") == TppEnemy.LIFE_STATUS.NORMAL
								and TppEnemy.GetStatus("sol_ExecuteUnit_0001") == EnemyState.NORMAL
						then
			...
			{
				msg = "Finish",
				sender = "Go_SurroundCar",
				func = function()
					Fox.Log("###Go_SurroundCar!!!!###")
					if svars.isSearchUnit02OnMoving == false then
						GameObject.SendCommand(
							GameObject.GetGameObjectId("sol_SwampWest_0005"),
							{ id = "StartTravel", travelPlan = "Travel_Surround_Car" }
						)
			...
			{
				msg = "Finish",
				sender = "Target01_EnemyNotFoundInTime",
				func = function()
				...
				func = function()
					if vars.isAfrikaansTranslatable == 1 then
						...
						if svars.isExecuteRadioStarted == false then
							...
							if
								(No19isPlayed == false)
								and (svars.isTarget02Marked == false)
								and (svars.isTarget02Rescue == false)
							then
			...
			{
				msg = "Finish",
				sender = "CountDown_Target01_Monologue",
				func = function()
			...
			{
				msg = "Finish",
				sender = "CountDown_Target02_Monologue",
				func = function()
...
this.PlayRadio001 = function()
	...
	if is_s0091_rtrg4010_Played == false then
		...
		if
			(svars.isTarget01Rescue == false)
			and (svars.isTarget02Rescue == true)
			and (svars.isTarget01Unlocked == true)
		then
		...
		elseif
			(svars.isTarget01Rescue == false)
			and (svars.isTarget02Rescue == true)
			and (svars.isTarget01Unlocked == false)
		then
		...
		elseif
			...
			and (svars.isTarget02Rescue == false)
			and (svars.isTarget02Unlocked == true)
		then
			Fox.Log("#### Radio_s0091_rtrg2010")
		...
		elseif
			(svars.isTarget01Rescue == true)
			and (svars.isTarget02Rescue == false)
			and (svars.isTarget02Unlocked == false)
		then
...
function this.OnTargetRescued(s_characterId)
	...
	if
		s_characterId == GameObject.GetGameObjectId("TppHostageUnique", TARGET_HOSTAGE.NORTH_FOREST)
		and (svars.isTarget01Rescue == false)
	then
		svars.isTarget01Rescue = true
		svars.isTarget01Marked = true
	...
	elseif
		s_characterId == GameObject.GetGameObjectId("TppHostageUnique", TARGET_HOSTAGE.SWAMP)
		and (svars.isTarget02Rescue == false)
	then
	...
	OnEnter = function()
		...
		if TppSequence.GetContinueCount() > 0 then
			...
			if (svars.isTarget01Rescue == false) and (svars.isTarget02Rescue == false) then
				...
				this.SetUpTimer06()
			elseif
				(svars.isTarget02Marked == false)
				and (svars.isTarget02Rescue == false)
				and (svars.isTarget01Rescue == true)
			then
		...
		return StrCode32Table({
			...
			GameObject = {

				{
					msg = "Fulton",
					sender = TARGET_HOSTAGE.SWAMP,
					func = function(gameObjectId) end,
				},
