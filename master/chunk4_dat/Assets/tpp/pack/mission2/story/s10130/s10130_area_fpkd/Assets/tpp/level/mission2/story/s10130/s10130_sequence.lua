...
this.saveVarsList = {
	...
	parasiteSquadMarkerFlag = {
		name = "parasiteSquadMarkerFlag",
...
function this.MissionPrepare()
	...
	TppMission.RegisterMissionSystemCallback({
		OnSetMissionFinalScore = function(missionClearType)
			...
			Fox.Log(
				"!!!! s10130_mission.clear_missionTask_03 check !!!! isCodeTalkerDamage = "
					.. tostring(svars.isCodeTalkerDamage)
			)
...
this.GimmickRestore = function()
	Gimmick.ResetGimmick(TppGameObject.GAME_OBJECT_TYPE_DOOR, GIMMICK_NAME.DOOR001, GIMMICK_PATH.DOOR001)
end
...
this.ParasiteDisablementEvaluate = function(gameObjectId, seq)
	...
	if
		(svars.wmu_lab_0000Disablement == true)
		and (svars.wmu_lab_0001Disablement == true)
		and (svars.wmu_lab_0002Disablement == true)
		and (svars.wmu_lab_0003Disablement == true)
	then
...
this.FuncPlayerRideOnHeliWithHuman = function() end
						...
						if GameObjectId == GameObject.GetGameObjectId(TARGET_HOSTAGE_NAME) then
							...
							for index, labelName in ipairs(this.longMonologueList) do
								...
								elseif speechLabel == Fox.StrCode32("speech130_CTV020_03") then
									...
									local gameObjectId =
										GameObject.GetGameObjectId("TppCodeTalker2", TARGET_HOSTAGE_NAME)
									local command = { id = "GetStatus" }
									local actionStatus = GameObject.SendCommand(gameObjectId, command)
			...
			Player = {
				{
					...
					func = function(playerId, doorId)
						...
						local result, checkAlert, checkEnemy =
							TppDemo.CheckEventDemoDoor(doorId, CHECK_ENEMY_POS01, CHECK_ENEMY_RANGE01)
...

				{
					...
					func = function()
						if svars.isLabInRoom == false then
							...
							TppInterrogation.RemoveHighInterrogation(GameObject.GetGameObjectId("mafr_lab_cp"), {
...
sequences.Seq_Game_Escape_A = {
					...
					func = function()
						...
						if actionStatus == TppGameObject.NPC_STATE_CARRIED then
							if svars.isCodetalkerDemo001 == false then
								...
								for index, labelName in ipairs(this.longMonologueList01) do
									Fox.Log(
										"s10130_mission Monologue Stop longMonologueList01 "
											.. labelName
											.. " bool= "
											.. tostring(svars[labelName])
									)
									svars[labelName] = true
								end
								...
								local checkEnemy =
									TppEnemy.IsActiveSoldierInRange(CHECK_ENEMY_POS02, CHECK_ENEMY_RANGE02)
			...
			GameObject = {
				...
				},
				{
					...
					func = function(gameObjectId, arg01, arg02, arg03)
						...
						if
							(svars.wmu_lab_0000Fulton == true)
							and (svars.wmu_lab_0001Fulton == true)
							and (svars.wmu_lab_0002Fulton == true)
							and (svars.wmu_lab_0003Fulton == true)
						then
							svars.isAllKillParasite = true
						end
					end,
	...
	OnEnter = function()
		...
		TppUiCommand.RegisterIconUniqueInformation({
			markerId = GameObject.GetGameObjectId(TARGET_HOSTAGE_NAME),
			langId = "marker_chara_codetalker",
		})
