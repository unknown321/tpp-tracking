...
this.CalculateEspionageTotal = function(numClearType)
	...
	local sneakTotal = svars.espt_of_alert_count
		+ svars.espt_of_plant_count
		- (svars.alertCount * ESPT_OF_ALERT_COUNT)
		+ svars.espt_of_no_kill
		+ svars.espt_of_no_reflex
	...
	local collectTotal = svars.espt_of_fluton_cntn
		+ svars.espt_of_fluton_ptwp
		+ svars.espt_of_pick_scgj
		+ svars.espt_of_fluton_dds
	svars.espt_of_total_collect = fnc_SetESPTotal(collectTotal, OF_TOTAL_COLLECT_MIN, svars.espt_of_total_collect_max)

	local destroyTotal = svars.espt_of_destroy_ptwp
		+ svars.espt_of_destroy_scgj
		+ svars.espt_of_kill_dds
		- (svars.cnt_down_of * ESPT_OF_DOWNED)
	...
	local collectTotalDF = svars.espt_df_save_cntn
		+ svars.espt_df_save_ptwp
		+ svars.espt_df_save_sec
		+ svars.espt_df_save_dds
	svars.espt_df_total_defense = fnc_SetESPTotal(collectTotalDF, DF_TOTAL_COLLECT_MIN, svars.espt_df_total_defense_max)

	local versusTotalDF = svars.espt_df_kill_ofp
		+ svars.espt_df_fulton_ofp
		- svars.espt_df_fultoned
		- svars.espt_df_dead
		+ (svars.cnt_down_of * ESPT_DF_DOWN_OFP)
...
this.CalculateResult = function(numClearType)
	...
	if vars.fobSneakMode == FobMode.MODE_ACTUAL then
		...
		svars.espt_df_save_ptwp = (numTotalPutWeaponCount - svars.cnt_fltn_ptwp - svars.cnt_dstr_ptwp)
			* ESPT_DF_SAVE_PTWP
	...
	if vars.fobSneakMode == FobMode.MODE_ACTUAL then
		...
		this.SwitchExecByIsHost(function()
			this.SetAnnounceEspPoint({
				upPoint = ESPT_OF_FLUTON_CNTN,
				sbj = "cmmn_name_container",
				upLangId = "espFultonContainer_a",
			})
		end, function()
			this.SetAnnounceEspPoint({
				downPoint = ESPT_DF_SAVE_CNTN,
				sbj = "cmmn_name_container",
...
function this.SetAndAnnounceEspPoint_FultonPtwp(strFultoned)
	if vars.fobSneakMode == FobMode.MODE_ACTUAL then
		...
		this.SwitchExecByIsHost(function()
			this.SetAnnounceEspPoint({
				upPoint = ESPT_OF_FLUTON_PTWP,
				sbj = strFultoned,
				upLangId = "espFultonContainer_a",
			})
		end, function()
			this.SetAnnounceEspPoint({
				downPoint = ESPT_DF_SAVE_PTWP,
				sbj = strFultoned,
				downLangId = "espFultonContainer_d",
			})
		end)
...
function this.SetAndAnnounceEspPoint_FultonDds()
	if vars.fobSneakMode == FobMode.MODE_ACTUAL then
		...
		this.SwitchExecByIsHost(function()
			this.SetAnnounceEspPoint({
				upPoint = ESPT_OF_FLUTON_DDS,
				sbj = "cmmn_name_sec_staff",
				upLangId = "espFultonContainer_a",
			})
		end, function()
			this.SetAnnounceEspPoint({
				downPoint = ESPT_DF_SAVE_DDS,
				sbj = "cmmn_name_sec_staff",
				downLangId = "espFultonContainer_d",
			})
		end)
...
function this.SetAndAnnounceEspPoint_FultonDF()
	if vars.fobSneakMode == FobMode.MODE_ACTUAL then
		...
		this.SwitchExecByIsHost(function()
			this.SetAnnounceEspPoint({
				upPoint = ESPT_OF_FLUTON_DFP,
				sbj = "result_fob_guardian",
				upLangId = "espFulton_a",
			})
		end, function() end)
	end
end
...
this.OnPlayerDamagedOffence = function(isAlertDamage)
	...
	this.SwitchExecByIsGameMode(fnc_vsCommon, fnc_vsCommon, function() end, isAlertDamage)
end
