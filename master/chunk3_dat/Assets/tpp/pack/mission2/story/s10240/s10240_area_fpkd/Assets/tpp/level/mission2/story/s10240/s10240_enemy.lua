	...
	if #soldiers > 0 then
		for i, soldier in ipairs(soldiers) do
			GameObject.SendCommand(
				soldier,
				{ id = "SetSneakRoute", route = afterRoute, point = routePointNum, isRelaxed = true }
			)
...
this.SetUpEnemy = function()
	...
	if
		...
		== TppSequence.GetSequenceIndex("Seq_Demo_Funeral")
	then
		...
		for i, enemyId in pairs(this.soldierDefine.mbqf_mtbs_cp) do
			...
			GameObject.SendCommand(
				gameObjectId,
				{ id = "ChangeFova", balaclavaFaceId = TppEnemyFaceId.dds_balaclava2, seed = faceSeed[i] }
			)
		end
	else
		...
		for i, enemyId in pairs(this.soldierDefine.mbqf_mtbs_cp) do
			...
			GameObject.SendCommand(
				gameObjectId,
				{ id = "ChangeFova", balaclavaFaceId = TppEnemyFaceId.dds_balaclava6, seed = seed[seedNum] }
			)
