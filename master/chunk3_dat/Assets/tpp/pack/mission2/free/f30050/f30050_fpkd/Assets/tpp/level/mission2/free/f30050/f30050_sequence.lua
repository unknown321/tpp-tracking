...
local PlayerDisableActionFlagInPazRoom = PlayerDisableAction.CQC
...
this.missionStartPosition = {
	...
	IsUseRoute = function()
		...
		if
			demoName
			and this.IsRideOnHeliDemo(demoName)
			and gvars.heli_missionStartRoute == Fox.StrCode32(DROP_ROUTE_USE_SORTIE_DEMO)
		then
		...
		Fox.Log(
			"missionStartPosition.IsUseRoute: isStartFromHelispace:"
				.. tostring(isStartFromHelispace)
				.. " :notPlayDemo:"
				.. tostring(isNotPlayOpeningDemo)
		)
...
function this.AcceptMission(missionId)
	...
	local s10115_heliRouteTable = {
		[1] = {
			"ly003_cl02_10115_heli0000|cl02pl0_mb_fndt_plnt_heli_10115|rt_apr",
			Vector3(878.28747558594, -3.498596906662, 323.64611816406),
		},
		[2] = {
			"ly003_cl02_10115_heli0000|cl02pl1_mb_fndt_plnt_heli_10115|rt_apr",
			Vector3(974.13403320313, -3.498596906662, 326.39437866211),
		},
		[3] = {
			"ly003_cl02_10115_heli0000|cl02pl2_mb_fndt_plnt_heli_10115|rt_apr",
			Vector3(1043.8511962891, -3.498596906662, 260.56399536133),
		},
		[4] = {
			"ly003_cl02_10115_heli0000|cl02pl3_mb_fndt_plnt_heli_10115|rt_apr",
			Vector3(1191.4196777344, -3.498596906662, 259.04104614258),
		},
	}
				...
				sequences[debugDemoSequenceName] = {
					OnEnter = function()
						...
						TppDataUtility.SetVisibleDataFromIdentifier(
							"uq_0040_paz_room_AssetIdentifier",
							"paz_room_phantom",
							true,
							false
						)
						...
						TppDataUtility.SetVisibleDataFromIdentifier(
							"uq_0040_paz_room_bed_AssetIdentifier",
							"paz_room_bed",
							true,
							false
						)
...
function this.SetupStaffList()
	...
	for i = 1, MAX_STAFF_NUM_ON_CLUSTER do
		for j = TppMotherBaseManagementConst.SECTION_COMBAT, TppMotherBaseManagementConst.SECTION_SECURITY do
			...
			if #commandStaffList >= MAX_STAFF_NUM_ON_CLUSTER then
				break
			end
		end
		if #commandStaffList >= MAX_STAFF_NUM_ON_CLUSTER then
	...
	if mvars.f30050_demoName then
		...
		for i = 1, addSolNum do
			table.insert(
				staffListOnDemoCluster,
				TppMotherBaseManagement.GenerateStaffParameter({ staffType = "Unique", uniqueTypeId = uniqueTypeId })
			)
...
this.RegisterFovaFpk = function(clusterId)
	...
	if TppEneFova.IsUseGasMaskInMBFree(mvars.f30050_currentFovaClusterId) then
		Tpp.ApendArray(
			fovaPackList,
			TppSoldierFace.GetFaceFpkFileCodeList({ face = {
				TppEnemyFaceId.dds_balaclava6,
				TppEnemyFaceId.dds_balaclava7,
			} })
		)
		...
		Tpp.ApendArray(
			mvars.f30050_soldierBalaclavaLocatorList,
			mvars.f30050_soldierListFovaApplyPriority[clusterId + 1]
		)
		mvars.f30050_soldierFaceIdList = {}
	elseif #mvars.f30050_soldierBalaclavaFaceIdList > 0 then
		Tpp.ApendArray(
			fovaPackList,
			TppSoldierFace.GetFaceFpkFileCodeList({ face = {
				TppEnemyFaceId.dds_balaclava5,
				TppEnemyFaceId.dds_balaclava2,
			} })
		)
	end
	...
	local faceFovaFpk = TppSoldierFace.GetFaceFpkFileCodeList({
		face = mvars.f30050_soldierFaceIdList,
		deco = mvars.f30050_soldierBalaclavaFaceIdList,
		useHair = true,
	})
	if faceFovaFpk then
			...
			},
			{
				...
				func = function(gameObjectId, gameObjectName, name, switchFlag)
					if
						gameObjectName
						== StrCode32(
							"ly003_cl04_item0000|cl04pl0_uq_0040_gimmick2|gntn_swtc001_vrtn001_gim_n0001|srt_gntn_swtc001_vrtn001"
						)
					then
						do
							local pos = Tpp.GetLocator("mtbs_pazRoom_Locator", "enter")
							local distance = (pos[1] - vars.playerPosX) * (pos[1] - vars.playerPosX)
								+ (pos[2] - vars.playerPosY) * (pos[2] - vars.playerPosY)
								+ (pos[3] - vars.playerPosZ) * (pos[3] - vars.playerPosZ)
		...
		Player = {
			{
				...
				func = function(playerGameObjectId, switchGameObjectId)
					if TppStory.GetBattleGearDevelopLevel() >= 0 then
						do
							local _, gameObjectId = Gimmick.GetGameObjectId(
								TppGameObject.GAME_OBJECT_TYPE_IMPORTANT_BREAKABLE,
								"gntn_swtc001_vrtn001_gim_n0000|srt_gntn_swtc001_vrtn001",
								"/Assets/tpp/level/location/mtbs/block_large/mtbs_hanger_gimmick.fox2"
							)
						...
						do
							...
							local dataSetName = "/Assets/tpp/level/location/mtbs/block_area/ly00"
								.. tostring(vars.mbLayoutCode)
								.. "/cl02/mtbs_ly00"
								.. tostring(vars.mbLayoutCode)
								.. "_cl02_item.fox2"
							local _, gameObjectId = Gimmick.GetGameObjectId(
					...
					if this.CanEnterPazRoom() then
						do
							...
							local dataSetName = "/Assets/tpp/level/location/mtbs/block_area/ly00"
								...
								.. tostring(layoutCode)
								.. "_cl04_item.fox2"
							local switchOnEnterPazRoom =
								"ly003_cl04_item0000|cl04pl0_uq_0040_gimmick2|gntn_swtc001_vrtn001_gim_n0000|srt_gntn_swtc001_vrtn001"
							local _, gameObjectId = Gimmick.GetGameObjectId(
								TppGameObject.GAME_OBJECT_TYPE_IMPORTANT_BREAKABLE,
								switchOnEnterPazRoom,
								dataSetName
							)
							if gameObjectId == switchGameObjectId then
								this.SetCameraPushSwitch()
...
sequences.Seq_Demo_End_Warp_Cluster = {
	...
	OnEnter = function()
		mvars.f30050_prevScriptBlockState =
			ScriptBlock.GetScriptBlockState(ScriptBlock.GetScriptBlockId(SCRIPT_BLOCK_NAME))
	end,
	OnUpdate = function()
			...
			Demo = {
				{
					...
					func = function()
						TppDataUtility.SetVisibleDataFromIdentifier(
							"id_20150116_131237_710",
							"p61_010030_demo_hide",
							false,
							false
						)
						TppDataUtility.SetVisibleDataFromIdentifier(
							"id_20150116_174108_802",
							"p61_010030_demo_show",
							true,
							false
						)
					end,
				...
				},
				{
					...
					func = function()
						if
							mvars.f30050_demoName == "SnakeHasBadSmell_WithoutQuiet"
							or mvars.f30050_demoName == "QuietOnHeliInRain"
						then
...
function this.ShowTelopInDemo()
	if not TppMission.IsStartFromHelispace() and mvars.f30050_demoTelopLangIdList ~= nil then
		local index = 4
...
local EpigraphTable = {
	...
	{
		time = 15,
		epigraphLangid = "tpp_epigraph_obama_04",
		quoteLangid = "tpp_epigraph_obama_quote",
		quoteOffsetY = 15,
	},
...
function this.StartEpigraph()
	...
	for _, epigraph in ipairs(EpigraphTable) do
		TppUiCommand.RegistTelopCast(
			"Epigraph",
			epigraph.time,
			epigraph.epigraphLangid,
			epigraph.quoteLangid,
			1.0,
			0,
			epigraph.quoteOffsetY
		)
	...
	Messages = function(self)
		return StrCode32Table({
			Player = {
				...
				},
				{
					...
					func = function(gameObjectId, collectionUniqueId, collectionTypeId)
						if
							collectionUniqueId
							== TppCollection.GetUniqueIdByLocatorName(
								"ly003_cl04_collct0000|cl04pl0_uq_0040_collct|col_photo_paz"
							)
						then
			...
			UI = {
				{
					msg = "EndFadeOut",
					sender = "OnEndFadeOutPlayRideOnHeliDemo",
					func = function()
						f30050_demo.PlayMtbsEventDemo({
			...
			Terminal = {
				{
					...
					func = this.AcceptMission,
				},
				{
					msg = "MbDvcActCallRescueHeli",
					func = this.CallRescueHeli,
				},
				{
					msg = "MbDvcActSelectLandPointTaxi",
					func = this.OnSelectLandPointTaxi,
				},
			},
			MotherBaseStage = {
				{
			...
			Trap = {
				{
					...
					func = function()
						if
							not TppQuest.IsActive("mtbs_q99011")
							and TppStory.CanArrivalQuietInMB(false)
							and not isPlayedAfterDeathFactory
						then
							GameObject.SendCommand(
								{ type = "TppMbQuiet", index = 0 },
								{ id = "StartWakeUp", isLoop = true, isAll = false }
							)
						end
					end,
				},
...
function this.PushSwitchOnEnterBattleHanger()
	...
	if TppStory.GetBattleGearDevelopLevel() >= 0 then
		...
		local dataSetName = "/Assets/tpp/level/location/mtbs/block_area/ly00"
			...
			.. "_cl02_item.fox2"
		local dataName =
			"ly003_cl02_item0000|cl02pl0_uq_0020_gim_onlymb2|mtbs_door006_door002_ev_gim_n0000|srt_mtbs_door006_door002_ev"
...
function this.DisableLandingZoneForSeparationPlatform()
	...
	GameObject.SendCommand(
		gameObjectId,
		{ id = "DisableLandingZone", name = "ly003_cl07_30050_heli0000|cl07pl0_mb_fndt_plnt_heli_30050|lz_plnt" }
	)
end
...
function this.EnableAssetsOnCluster(clusterId)
	...
	if clusterId >= #TppDefine.CLUSTER_NAME or MotherBaseStage.GetCurrentCluster() >= #TppDefine.CLUSTER_NAME then
...
function this.LockGoalDoor(clusterId)
	local layoutCode = vars.mbLayoutCode
	local dataSetPath = "/Assets/tpp/level/location/mtbs/block_area/ly00"
		.. tostring(layoutCode)
		.. "/cl0"
		.. tostring(clusterId)
		.. "/mtbs_ly00"
...
function this.UpdatePazPhotoCollection()
	if
		this.GetPazLookedPictureCount() >= 10
		and not TppMotherBaseManagement.IsGotDataBase({ dataBaseId = TppMotherBaseManagementConst.PHOTO_1010 })
	then
...
function this.UpdateCommonCollection()
	if gvars.f30050_missionPackIndex == 0 then
		if TppQuest.IsCleard("commFacility_q80060") then
			TppCollection.RepopCountOperation(
				"SetAt",
				"ly003_cl00_collct0000|cl00pl0_uq_0000_collct|col_POSTER_MTBS_H_0000",
				0
			)
			TppCollection.RepopCountOperation(
				"SetAt",
				"ly003_cl00_collct0000|cl00pl0_uq_0000_collct|col_POSTER_MTBS_V_0000",
				0
			)
		else
			TppCollection.RepopCountOperation(
		...
		if TppQuest.IsActive("mtbs_q99011") and (not TppUiCommand.HasEmblemTexture("front9")) then
			TppCollection.RepopCountOperation(
				"SetAt",
				"ly003_cl04_collct0000|cl04pl0_uq_0040_collct|col_emblem_quiet",
				0
			)
		else
			TppCollection.RepopCountOperation(
		...
		TppCollection.RepopCountOperation(
			"SetAt",
			"ly003_cl00_collct0000|cl00pl0_uq_0000_collct|col_develop_MTBS_0000",
			nShowflag
		)
		...
		TppCollection.RepopCountOperation(
			"SetAt",
			"ly003_cl00_collct0000|cl00pl0_uq_0000_collct|col_develop_MTBS_0001",
			nShowflag
		)
		...
		TppCollection.RepopCountOperation(
			"SetAt",
			"ly003_cl00_collct0000|cl00pl0_uq_0000_collct|col_develop_MTBS_0002",
			nShowflag
		)
		...
		TppCollection.RepopCountOperation(
			"SetAt",
			"ly003_cl00_collct0000|cl00pl0_uq_0000_collct|col_develop_MTBS_0003",
			nShowflag
		)
	end
end
...
function this.EnableBiggBossPosters()
	if
		TppDefine.STORY_SEQUENCE.CLEARD_OKB_ZERO <= TppStory.GetCurrentStorySequence()
...
local requestToDropCboxTable = {
	{
		trapName = "trap_RequestToDropCbox0000",
		time = 5,
		clusterConstruct = 2,
		position = Vector3(-27.911, 0.000, 2.630),
		height = 25.0,
	},
	{
		trapName = "trap_RequestToDropCbox0001",
		time = 5,
		clusterConstruct = 2,
		position = Vector3(-16.651, 0.000, 12.570),
		height = 25.0,
	},
	{
		trapName = "trap_RequestToDropCbox0002",
		time = 5,
		clusterConstruct = 3,
		position = Vector3(16.883, 0.002, -11.817),
		height = 25.0,
	},
}
...
local pazPhaseList = {
	...
	[8] = {
		Update = function()
			if
				TppDemo.IsPlayedMBEventDemo(PAZ_PACK_NAME_LIST[4])
				or TppDemo.IsPlayedMBEventDemo(PAZ_PACK_NAME_LIST[5])
			then
				return 9
			else
				return 8
...
function this.EnablePazRoom(isEnable)
	...
	TppDataUtility.SetVisibleDataFromIdentifier(
		"uq_0040_paz_room_AssetIdentifier",
		"paz_room_phantom_door",
		true,
		false
	)

	local PAZ_PHOTO_LIST =
...
function this.GetPazCassette()
	...
	if TppDemo.IsPlayedMBEventDemo(PAZ_PACK_NAME_LIST[4]) or TppDemo.IsPlayedMBEventDemo(PAZ_PACK_NAME_LIST[5]) then
			...
			f30050_demo.PlayMtbsEventDemo({
				...
				onEnd = function()
					if packName == PAZ_PACK_NAME_LIST[4] or packName == PAZ_PACK_NAME_LIST[5] then
						...
						if DEBUG then
							if packName ~= PAZ_PACK_NAME_LIST[1] and packName ~= PAZ_PACK_NAME_LIST[2] then
...
sequences.Seq_Demo_PazRoomOnEnter = {
	...
	OnUpdate = function(self)
		if mvars.f30050_waitPazBlockLoad then
			...
			local blockId = ScriptBlock.GetScriptBlockId(SCRIPT_BLOCK_NAME)
			if
				ScriptBlock.GetScriptBlockState(blockId) == ScriptBlock.SCRIPT_BLOCK_STATE_ACTIVE
				and not TppSoundDaemon.IsAssetLoading()
			then
				this.SetPazMode()
...
sequences.Seq_Game_PazRoom = {
	...
	OnUpdate = function(self)
		if mvars.paz_isSkipPlayerInit and self.IsAnyStickDown() then
	...
	Messages = function(self)
		return StrCode32Table({
			...
			Timer = {
				{
					msg = "Finish",
					sender = "TimerWaitFadeInPazRoomOnLeave",
					func = function()
			...
			Demo = {
				{
					...
					func = function()
						TppDataUtility.SetVisibleDataFromIdentifier(
							"id_20150225_190602_103",
							"btg01_demo_hide_group",
							true,
							false
						)
					end,
				...
				},
				{
					...
					func = function()
						TppDataUtility.SetVisibleDataFromIdentifier(
							"id_20150225_191810_465",
							"btg02_demo_hide_group",
							true,
							false
						)
					end,
				...
				},
				{
					...
					func = function()
						TppDataUtility.SetVisibleDataFromIdentifier(
							"id_20150225_192737_376",
							"btg04_demo_hide_group",
							true,
							false
						)
					end,
				...
				},
				{
					...
					func = function()
						TppDataUtility.SetVisibleDataFromIdentifier(
							"id_20150225_193056_215",
							"btg05_demo_hide_group",
							true,
							false
						)
					end,
...
function this.SetupPadMaskInBattleHanger()
	Player.SetPadMask({
		...
		buttons = PlayerPad.MB_DEVICE + PlayerPad.HOLD + PlayerPad.RELOAD,
	})
end
		...
		if clusterId == TppDefine.CLUSTER_DEFINE.Command then
			...
			if Tpp.IsVehicle(vars.playerVehicleGameObjectId) then
				GameObject.SendCommand(
					...
					{ id = "SetPosition", position = Vector3(9, 0.8, -42), rotY = 0 }
				)
			else
...
function this.NeedPlayQuietWishGoMission()
	...
	Fox.Log(
		"NeedPlayQuietWishGoMission:isClearedSideOps: "
			.. tostring(isClearedSideOps)
			.. " isNotPlayDemo: "
			.. tostring(isNotPlayDemo)
			.. " canArrival: "
			.. tostring(isCanArrival)
	)
